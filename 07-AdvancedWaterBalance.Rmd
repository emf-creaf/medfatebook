...
Optionally, the model can simulate additional transpiration caused by hemi-parasitic plants (mistletoe) attached to specific plant cohorts, as done in the basic water model. This causes water update from the corresponding soil water pools to be higher than cohort transpiration. The model does not try to simulate mistletoe hydraulics, because the focus is on estimating the effect of mistletoe transpiration on the host plant cohort. Mistletoe hydraulic conductance (or water potential drop) is not explicitly represented. Instead, the water potential driving transpiration in mistletoe is assumed to be *equal to the leaf water potential of the host plant*. In other words, it is assumed that the drop in water potential in mistletoe branches and leaves is comparable to the drop from the mistletoe branch insertion point to the host leaf (but see different water potential values in @sancho-knapik_viscum_2026). Another limitation is that mistletoe transpiration flow is not extracted from plant tissues directly, as in @zweifel_pine_2012, but added to the water uptake from the soil layers (or soil water pools) where the plant is connected. Therefore, increased stress effects on the host plant are better represented when soil water pools are considered. 
...
+ *If cavitation-induced defoliation is considered* (i.e. `cavitationInducedDefoliation = TRUE`), then leaf area index ($LAI_i$) will become a state variable for all plant cohorts, including evergreen species (see \@ref(cavitationinduceddefoliationadvanced)).
...
\end{equation}
where all the other terms were defined in \@ref(soilwaterbalance). Soil water extraction ($Ex_{cohorts}$) and plant transpiration ($Tr_{cohorts}$) will be equal if plant capacitance effects or transpiration by hemi-parasitic (mistletoe) plants ($Tr_{mistletoe}$) are not considered. More generally, the amount of water stored in the plant tissues changes according to:
\begin{equation}
\Delta{V_{plant}} = Ex_{cohorts} - Tr_{cohorts} - Tr_{mistletoe}
...
7. Determine instantaneous plant cohort transpiration, photosynthesis rates and close soil/canopy energy balance at sub-daily steps (chapters \@ref(transpirationsperrysureau) and \@ref(closecanopysoilenergybalance)). This involves the following actions for each sub-daily step:
    ...
    b. If `transpirationMode = "Sperry"`, for each plant cohort:
        ...
        iv. If necessary, determine mistletoe transpiration ($Tr_{mistletoe}$) and the corresponding additional soil water uptake (section \@ref(mistletoetranspirationadvanced)).
    ...
    c. If `transpirationMode = "Sureau"`, for each plant cohort leaf energy balance, stomatal and cuticular conductances, transpirational flows, photosynthesis and plant hydraulics are computed iteratively in small temporal sub-steps (e.g. 10 min):
        ...
        vi. If necessary, determine mistletoe transpiration ($Tr_{mistletoe}$) and the corresponding additional soil water uptake (section \@ref(mistletoetranspirationadvanced)).
...
Vegetation input requirements were fully described in section \@ref(vegetationinput). Trees and shrubs do not need to be characterized with different variables in soil water balance calculations, since both kinds of cohort have a $LAI$ value. In most cases, users only need to estimate the leaf area index corresponding to live leaves, i.e. $LAI_{live}$, because normally at the starting point all leaves are expanded (i.e. $LAI_{\phi} = LAI_{live}$) and one can assume no dead leaves in the canopy (i.e., $LAI_{dead} = 0$). Vegetation characteristics stay constant during simulations using function `spwb()`, although the actual expanded leaf area ($LAI_{\phi}$) and dead leaf area may vary if the species is winter deciduous. For each plant cohort with hemi-parasitic (mistletoe) plants living on crowns, a corresponding value of mistletoe leaf area index ($LAI_{mistletoe}$) needs to be defined.
...
| $T_{gsw, optim, jarv}$ |  $^{\circ} \mathrm{C}$ | `Gsw_Toptim_Jarvis`| Temperature corresponding to maximal stomatal conductance (Jarvis stomatal sub-model) |
| $T_{gsw, sens, jarv}$ | | `Gsw_Tsens_Jarvis` | Stomatal sensitivity to temperature (Jarvis stomatal sub-model) |
| $m_{wue, bald}$ | | `Gsw_AC_slope_Baldocchi` | Slope of the Gsw vs Ac/Cs relationship (Baldocchi stomatal sub-model, see \@ref(stomatalconductancesureau)) |
| $\Psi_{50,gsw, bald}$ | MPa | `Gsw_P50_Baldocchi`| Water potential causing 50% reduction in stomatal conductance (Sureau sub-model) |
| $slope_{gsw, bald}$ | %/MPa | `Gsw_slope_Baldocchi` | Rate of decrease in stomatal conductance at $\Psi_{50,gsw,bald}$ (Baldocchi stomatal sub-model) |
...
+  `mistletoeParams`: A list with the following elements:
    ...
    + `Gsw_AC_slope_Baldocchi [=8]`: Slope of the Gsw vs Ac/Cs relationship ($m_{wue, bald, mistletoe}$  of the Baldocchi model).
    + `Vmax298 [= 80]`: Maximum Rubisco carboxylation rate at 298K.
    + `Jmax298 [= 120]`: Maximum rate of electron transport at 298K.
    + `LeafWidth [=1]`: Mistletoe leaf width (cm).
...
| `Plants`| Plant-level variables, such as $LAI_{\phi,i}$, plant transpiration, plant water potential, etc. Only returned if `plantResults = TRUE`.|
