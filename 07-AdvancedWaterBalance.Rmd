...
Steps 1-4 are the same as described in the basic model, and steps 10-11 here correspond to steps 6-7 in the basic model. The set of steps corresponds to simulations if plant water pools *are not* considered, so that only the water balance of the overall soil matters. If plant water pools *are* considered, steps 1, 2 and 11 are the same, because they apply to the whole forest stand (note that at present infiltration does not depend on antecedent conditions). However, steps 3 and 4 are performed for each water pool separately because they depend on antecedent soil moisture. Steps 7-9 also involve defining woody plant water uptake from different plant water pools depending on the proportion of fine roots of each woody cohort in each pool (see \@ref(transpirationpools)). Soil water flows from these three steps are then averaged at the stand level. After that, Step 10 is performed on the soil corresponding to each pool, and the resulting drainage estimates are also averaged. Finally, the soil moisture of the overall soil is estimated by averaging the soil moisture of water pools.
...
+ `cavitationRecoveryStem/cavitationRecoveryLeaves [= "annual"]`: A string indicating how refilling of embolized conduits is done  (see section \@ref(cavitationrecovery). Allowed values are: 
  + `"none"` - no refilling.
  + `"annual"` - every first day of the year.
  + `"total"` - instantaneous complete refilling.
  + `"rate"` - following a rate of new sapwood formation.
+ `cavitationRecoveryMaximumRate [= 0.05]`: Maximum rate of conduit refilling (see section \@ref(cavitationrecovery)).
