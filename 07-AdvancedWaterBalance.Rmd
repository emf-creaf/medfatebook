...
While plant water content is normally determined by changes in water potential values corresponding to changes in soil moisture and transpiration rates, the model can also consider an explicit water balance within the plant. In this case, the amount of water in compartments is explicitly tracked and they decouple the extraction rate of soil water from the rate of leaf transpiration. 
...
+ If irreversible cavitation is activated, the model also tracks $P_{embolized,i}$, the proportion of stem embolized xylem conduits of each plant cohort $i$.
...
| `dEdP` | $dE/d\Psi_{i}$ | $mmol \cdot s^{-1} \cdot m^{-2} \cdot MPa^{-1}$ | Mean values of soil-plant conductance (derivative of the supply function) for each plant cohort. |
...
| `LeafPsiMin_SL`|$\Psi_{md, i}^{sunlit}$ | MPa | Minimum (midday) sunlit leaf water potential. |
| `LeafPsiMax_SL`| $\Psi_{pd, i}^{sunlit}$| MPa | Maximum (predawn) sunlit leaf water potential. |
| `LeafPsiMin_SH`| $\Psi_{md, i}^{shade}$ | MPa | Minimum (midday) shade leaf water potential. |
| `LeafPsiMax_SH`| $\Psi_{pd, i}^{shade}$| MPa | Maximum (predawn) shade leaf water potential. |
