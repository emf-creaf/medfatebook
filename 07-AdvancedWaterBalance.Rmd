...
Hydrological processes are the same as in chapter \@ref(basicwaterbalance). Thus, the model includes water interception loss [@Gash1995], plant transpiration, evaporation from soil [@Ritchie1972], transpiration of the herbaceous layer, partition between infiltration and runoff [@Boughton1989], Darcian vertical water movement [@bonan_climate_2019] and deep drainage. 
...
Steps 1-4 are the same as described in the basic model, and steps 10-11 here correspond to steps 6-7 in the basic model. The set of steps corresponds to simulations if plant water pools *are not* considered, so that only the water balance of the overall soil matters. If plant water pools *are* considered, steps 1-3 are the same, because they apply to the whole forest stand (note that at present infiltration does not depend on antecedent conditions). However, step 4 is performed for each water pool separately, steps 7 and 9 also involves defining woody plant water uptake from different plant water pools depending on the proportion of fine roots of each woody cohort in each pool (see \@ref(transpirationpools)). Step 10 is first performed on the soil corresponding to each pool and, finally, overall soil moisture is updated using pool proportions as weights. In this case, deep drainage for the overall soil is determined so as to force the water balance closure.
