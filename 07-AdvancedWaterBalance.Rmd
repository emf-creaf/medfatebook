...
+ If irreversible cavitation is activated, the model also tracks $P_{embolized,i}$, the proportion of embolized xylem conduits for stem hydraulic segments.
...
Like the canopy, the soil absorbs short- and longwave radiation from the atmosphere ($K_{abs,sa}$ and $L_{abs,sa}$). It also absorbs longwave radiation released by the canopy ($L_{abs,sc} = L_{em,c}$) and emmits longwave radiation ($L_{em,s}$). Note that its counterpart, the longwave energy emmitted by the soil and absorbed by the canopy, $L_{abs,cs}$, is equal or lower than $L_{em,s}$ because part of the radiation emmitted by the soil is sent to the atmosphere without being intercepted by the canopy. Finally, the soil also exchanges convective energy with the canopy ($H_{cs}$) and releases energy in form of latent heat ($LE_{s}$), which includes snow melt ($Sm$) and evaporation from the soil surface ($Es$). The energy balance equation for the soil is:
\begin{equation}
  C_{soil} \cdot \frac{\delta T_{soil}}{\delta t} = K_{abs,sa} + L_{abs,sa} - L_{em,s} + L_{abs,sc} + H_{cs} - LE_{s} 
