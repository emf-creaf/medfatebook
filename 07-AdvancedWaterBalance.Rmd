...
Like the canopy, the uppermost soil layer absorbs short-wave radiation from the atmosphere ($K_{abs,soil}$) and has a net long-wave radiation balance ($L_{net,soil}$). The uppermost soil layer also exchanges heat with the canopy ($H_{can,soil}$) and releases energy in the form of latent heat, which includes fusion latent heat due to snow melt ($LE_{fus, soil}$) and vaporization latent heat due to evaporation from the soil surface ($LE_{vap, soil}$). Finally, the soil layer exchanges thermal energy by conduction with the layer below ($G_{1,2}$) The energy balance equation for the uppermost soil layer is:
\begin{equation}
  TC_{soil, 1} \cdot \frac{\delta T_{soil, 1}}{\delta t} = K_{abs,soil} + L_{net,soil} + H_{can,soil} - LE_{fus, soil} - LE_{vap, soil} - G_{1,2}
