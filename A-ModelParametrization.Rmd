...
| $WUE_{decay}$ | `WUE_decay` |  Exponent describing the decay of water use efficiency with low light levels  | No | No | \@ref(imputationtranspwue) |
...
When maximum water use efficiency ($WUE_{\max}$) is missing, it is determined from taxonomic family using an internal data set (`medfate:::trait_family_means`):
```{r echo = FALSE}
tfm = medfate:::trait_family_means
knitr::kable(tfm[!is.na(tfm$WUE),
                 c("WUE"),drop=FALSE],
             caption = 'Default maximum water use efficiency by family.')
```
...
By default, the coefficient describing the decay of water use efficiency with lower light levels is given a default value of $WUE_{decay} = 0.2812$.
