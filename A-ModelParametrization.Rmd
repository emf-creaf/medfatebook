...
Different kinds of vegetation functional parameters can be distinguished according to whether inbuilt parameter estimation is possible and how it is conducted:
 ...
 + **Strictly-required parameters** are those for which there are no inbuilt estimation procedures implemented in the initialization functions. Hence, either values in the species parameter table input are non-missing or suitable values need to be specified before running simulation models.
...
| $r_{6.35}$ | `r635` | Ratio between the weight of leaves plus branches and the weight of leaves alone for branches of 6.35 mm | Yes | No | No |
...
| $LD$ | `LeafDuration` | Average duration of leaves | Yes | No | No |
| $S^*_{eco}$ | `Sgdd` | Degree days corresponding to leaf budburst | Yes | No | No |
...
| Symbol | R    | Description                          | Strict | Scaled | Imputable | 
|--------|------|:----------------------------------- |:-------:|:-------:|:-------:|
| $k_{PAR}$ | `kPAR` | PAR extinction coefficient   | Yes | No | No |
| $\alpha_{SWR}$ | `alphaSWR` | Short-wave radiation leaf absorbance coefficient | Yes | No | No |
| $\gamma_{SWR}$ | `gammaSWR` | Short-wave radiation leaf reflectance (albedo) |   Yes | No | No |
| $s_{water}$ | `g` | Crown water storage capacity | Yes | No | No |
...
| $\Psi_{extract}$ | `Psi_Extract` |  The water potential at which plant transpiration is 50% of its maximum | Yes | No | No |
| $\Psi_{critic,i}$ | `Psi_Critic` |  The water potential corresponding to 50% of stem xylem cavitation |  Yes | No | No |
...
| Symbol | R    | Description                          | Strict | Scaled | Imputable |
|--------|------|:----------------------------------- |:-------:|:-------:|:-------:|
| $a_{ash}$ | `Aash` | Coefficient relating the square of shrub height with shrub area | Yes | No | No |
| $a_{bsh}$, $b_{bsh}$ | `Absh`, `Bbsh` | Coefficients relating crown volume with dry weight of shrub individuals | Yes | No | No |
| $a_{cr}$, $b_{1cr}$, $b_{2cr}$, $b_{3cr}$, $c_{1cr}$, $c_{2cr}$ | `B1cr`, `B2cr`, `B3cr`, `C1cr`, `C2cr` | Coefficients to calculate crown ratio of trees | Yes | No | No |
| $a_{cw}$, $b_{cw}$ | `Acw`, `Bcw` | Regression coefficients used to calculate the crown width of trees | Yes | No | No |
...
At present, default values for phenological parameters are the same regardless of the leaf phenology type:

|  Phenology type | `Tbgdd` |  `Ssen` |  `Phsen` |  `Tbsen` |
|:--------------- |:-----:|:-----:|:-----:|:-----:|
| One-flush evergreen | 5 | 8268 | 12.5 | 28.5 |
| Winter deciduous | 5 | 8268 | 12.5 | 28.5 |
| Winter semi-deciduous | 5 | 8268 | 12.5 | 28.5 |
...
### Xylem vulnerability curves {#imputationVCs}
Vulnerability curves in **medfate** need to be specified for leaves, stem and root segments via the two parameters of the Weibull function (see \@ref(vulnerabilitycurves)). 

When any of the parameters of the stem vulnerability curve is missing, A suitable estimate of $\Psi_{50,stem}$ the water potencial corresponding to 50\% of conductance loss, can be obtained from @Maherali2004 according to combinations of taxon group (either Angiosperm or Gymnosperm), growth form (either tree or shrub) and leaf phenology:
...
A regression equation using data from @Choat2012 can be used to estimate $\Psi_{88,stem}$ from $\Psi_{50,stem}$:
\begin{equation}
...
Default values for the surface-area-to-volume ratio ($\sigma$) and fuel heat content ($h$) are defined from leaf size and leaf shape as follows:

|  Leaf shape | Leaf size |  $\sigma$ |  $h$ |
|:--------------- |:-----:|:-----:|:-----:|
| Broad | Large |   5740   |  19740    |
| Broad | Medium |   4039   |  19825    |
| Broad | Small |   4386   |   20062   |
| Linear/Needle | Large |   3620   |   18250   | 
| Linear/Needle | Medium |   4758    |  21182    |
| Linear/Needle | Small |   6697   |  21888    |
| Spines | [any] |   6750   |   20433   |
| Scale | [any] |   1120   |   20504   |


When missing from the species parameter table, the percent of lignin ($LI$) is assumed equal to 20\%. Default value for the density of fuel particles ($\rho_p$) is 400 $kg\cdot m^{-3}$.
