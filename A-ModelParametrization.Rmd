...
knitr::kable(tfm[!is.na(tfm$Al2As),
                 c("Al2As", "Hv"),drop=FALSE],
             caption = 'Default leaf area to sapwood area (m2/m2) / Huber value (cm2/m2) by taxonomic family.')
```


If there is no information derived from taxonomic family, a default value is given depending on leaf shape and leaf size:
...
When water use efficiency coefficient is missing, it is determined from taxonomic family using an internal data set (`medfate:::trait_family_means`):
```{r echo = FALSE}
tfm = medfate:::trait_family_means
knitr::kable(tfm[!is.na(tfm$WUE),
                 c("WUE"),drop=FALSE],
             caption = 'Default water use efficiency by family.')
```


If there is no information derived from taxonomic family, the following default values are assigned depending on leaf shape and leaf size:
...
\end{equation}
while the apoplastic fraction of stem is assumed $f_{apo,stem} = f_{conduits}$ (see \@ref(imputationcond2sap)).
...
If family-level values are missing, suitable $K_{stem,max,ref}$ values are decided according to combinations of taxon group (either Angiosperm or Gymnosperm), growth form (either tree or shrub) and leaf phenology [@Maherali2004]:
...
If family-level values is missing, a suitable estimate of $\Psi_{50,stem}$ the water potential corresponding to 50\% of conductance loss, can be obtained from @Maherali2004 according to combinations of taxon group (either Angiosperm or Gymnosperm), growth form (either tree or shrub) and leaf phenology:
...
In turn, imputation for $SLA$ is explained in \@ref(imputationLWSLA), whereas missing values for $N_{area}$ are determined from taxonomic family using an internal data set (`medfate:::trait_family_means`):
```{r echo = FALSE}
tfm = medfate:::trait_family_means
knitr::kable(tfm[!is.na(tfm$Narea),
                 c("Narea"),drop=FALSE],
             caption = 'Default nitrogen concentration per leaf area by taxonomic family.')
```
If family-level values is missing, a default average value $N_{area} = 1.89$ is given. 

Would $N_{area}$ and $SLA$ values be both unavailable, a default value of 100 $\mu mol CO_2 \cdot s^{-1} \cdot m^{-2}$ is used for $V_{max, 298}$ imputation. 
...
When missing at the species parameter table, senescence rates for sapwood and fine roots are taken from `control` parameters. [JUSTIFICATION MISSING]
