...
First, $\Delta LA_{alloc}$ is the increase in leaf area dictated by allocation. In plants with *progressive evergreen* phenology (see \@ref(leafphenology)), organogenesis is not separated from leaf expansion and $\Delta LA_{alloc}$ is the maximum leaf area increment allowed by the leaf area target ($LA_{target}$; see \@ref(updateallocationtargets)), minus the current expanded leaf area ($LA_{\phi}$):
\begin{equation}
\Delta LA_{alloc} = \max(LA_{target} - LA_{\phi},0)
\end{equation}
In plants with other phenology types, $\Delta LA_{alloc}$ is equal to the preformed (in buds) leaf area that remains to be expanded $LA_{preformed}$:
\begin{equation}
\Delta LA_{alloc} = LA_{preformed}
\end{equation}
In turn, $LA_{preformed}$ is set at the end of the previous period of *bud formation* as follows:
\begin{equation}
LA_{preformed} = (LA_{target} \cdot (PCAB/100)  - 1/LD) \cdot org_{eff}
\end{equation}
where $PCAB$ is the percentage of crown active buds, $LD$ is leaf duration in years, and $org_{eff}$ is the  organogenesis efficiency, i.e. the ratio of cell expansion rate accounting for turgor effects relative to the maximum cell expansion rate (i.e. $r_{cell}(30, \Psi_{symp,leaf})/ r_{cell,max}$), averaged across the duration of the *bud formation* period.
...
Finally, $\Delta B_{fineroot,sink}$ represents the biomass increment expected by taking into account maximum  tissue growth rate ($RGR_{fineroot, max}$; in $g\,dry \cdot g\,dry^{-1} \cdot day^{-1}$) and the relative cell expansion rate given temperature ($T_{day}$) and water potential in the rhizosphere ($\Psi_{rhizo,s}$): 
\begin{equation}
