...
where $\beta_{surv, 0}$ and $\beta_{surv, 1}$ are species-specific coefficients using repeated forest inventory plot data. 
...
\begin{equation}
D_{stem} = \max(RWC_{stem}, 1.0 - PLC_{stem})
\end{equation}
...
```{r, echo = FALSE}
df <- SpParamsMED[SpParamsMED$Name %in% c("Abies alba","Cistus albidus", "Juniperus communis","Populus spp.", "Quercus ilex", "Quercus suber", "Quercus pubescens", "Pinus halepensis", "Pinus sylvestris"), c("Name", "StemPI0", "StemEPS", "WoodDensity", "VCstem_c", "VCstem_d")]
...
for(i in 1:nrow(df)) {
  ...
  df$Psi_PLC60[i] = hydraulics_xylemPsi(0.4, 1.0, df$VCstem_c[i], df$VCstem_d[i])
}
```
...
```{r, fig = TRUE, echo = FALSE, fig.width=8, fig.height=6, fig.cap="Annual (top) and daily (bottom) probability of dessication as a function of the maximum value of stem relative water content and stem relative hydraulic conductance."}
...
for(is in 1:nrow(df)) {
  ...
  for(i in 1:nrow(df_psi)) {
    ...
    df_psi$PLC[i] = 100 - hydraulics_xylemConductance(df_psi$Psi[i], 1.0, df$VCstem_c[is], df$VCstem_d[is])*100
    df_psi$x[i] <- max(df_psi$RWC[i], 100 - df_psi$PLC[i])
...
g1 <- ggplot(df3)+
  ...
  xlab("Maximum of stem relative water content and stem relative conductance (%)")+ ylab("Annual dessication probability [0-1]")+
  geom_vline(xintercept = 40, linetype = "dashed")+
...
g2 <- ggplot(df3)+
  ...
  xlab("Maximum of stem relative water content and stem relative conductance  (%)")+ ylab("Daily dessication probability [0-1]")+
  scale_color_brewer( type ="qual", palette = "Set1")+
  geom_vline(xintercept = 40, linetype = "dashed")+
  scale_x_reverse()+
  theme_bw()
...
g3 <- ggplot(df3)+
  ...
  scale_color_brewer("", type ="qual", palette = "Set1")+
  scale_x_reverse()+
  theme_bw()+ theme(legend.position = c(0.87, 0.4))
g4 <- ggplot(df3)+
...
\begin{equation}
P_{mort, daily} = \max( P_{mort, base},\, P_{starv, daily}, \, P_{dessic, daily})
\end{equation}
