...
```{r, fig = TRUE, echo = FALSE, fig.width=8, fig.height=14}
...
g1 <- ggplot(df3)+
  ...
  xlab("Maximum of relative water content and relative stem conductance (%)")+ ylab("Annual dessication probability [0-1]")+
...
g2 <- ggplot(df3)+
  ...
  xlab("Maximum of relative water content and relative stem conductance  (%)")+ ylab("Daily dessication probability [0-1]")+
  scale_color_brewer( type ="qual", palette = "Set1")+
  geom_vline(xintercept = 40, linetype = "dashed")+
  scale_x_reverse()+
  theme_bw()
g3 <- ggplot(df3)+
  ...
  scale_color_brewer("", type ="qual", palette = "Set1")+
  scale_x_reverse()+
  theme_bw()+ theme(legend.position = c(0.87, 0.5))
g4 <- ggplot(df3)+
...
plot_grid(g1, g2, g3, g4, nrow = 4, ncol = 1)
```


```{r, fig = TRUE, fig.height=5, fig.width=8, echo = FALSE}
df_NSC <- data.frame(NSC_rel = seq(100, 0.0, by = -0.01))
df_NSC$Pstarvation = NA
for(i in 1:nrow(df_NSC)) {
  # pday = mortality_dailyProbability(0.00001, stressValue = df_NSC$NSC_rel[i]/100, stressThreshold = 0.3)
  x <- (df_NSC$NSC_rel[i]/100 - 0.3)
  fact <- 40
...
g1 <- ggplot(df_NSC)+
  ...
  xlab("Percent of maximum starch capacity (%)")+ ylab("Annual starvation probability [0-1]")+
  geom_vline(xintercept = 30, linetype = "dashed")+
  scale_x_reverse()+
  theme_bw()
g2 <- ggplot(df_NSC)+
  geom_line(aes(x=NSC_rel, y = Pstarvation_daily), size = 1)+
  xlab("Percent of maximum starch capacity (%)")+ ylab("Daily starvation probability [0-1]")+
  geom_vline(xintercept = 30, linetype = "dashed")+
  scale_x_reverse()+
  theme_bw()
