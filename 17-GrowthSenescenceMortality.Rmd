...
$\Delta LA_{alloc}$ is the increase in leaf area dictated by allocation. In plants with *evergreen-progressive* phenology (see \@ref(leafphenology)), organogenesis is not separated from leaf expansion and $\Delta LA_{alloc}$ is the maximum leaf area increment allowed by the leaf area target ($LA_{target}$; see \@ref(updateallocationtargets)), minus the current expanded leaf area ($LA_{\phi}$):
\begin{equation}
\Delta LA_{alloc} = \max(LA_{target} - LA_{\phi},0)
\end{equation}
In plants with other phenology types, allocation is often dictated by preformed leaves within buds. If there is leaf area to be elongated (i.e. $LA_{preformed} > 0.0$) then $\Delta LA_{alloc}$ is equal to the preformed (in buds) leaf area that remains to be elongated:
\begin{equation}
\Delta LA_{alloc} = LA_{preformed}
\end{equation}
where $LA_{preformed}$ is set at the end of the previous period of *bud formation* as follows:
\begin{equation}
LA_{preformed} = (LA_{target} \cdot (PCAB/100)  - 1/LD) \cdot org_{eff}
\end{equation}
where $PCAB$ is the percentage of crown active buds, $LD$ is leaf duration in years, and $org_{eff}$ is the organogenesis efficiency, i.e. the ratio of cell expansion rate accounting for turgor effects relative to the maximum cell expansion rate (i.e. $r_{cell}(30, \Psi_{symp,leaf})/ r_{cell,max}$), averaged across the duration of the *bud formation* period. In phenology types that allow additional leaves to be formed after the elongation of preformed leaves (i.e. in *evergreen-polycyclic*, *winter-deciduous* and *winter-semideciduous*), when $LA_{preformed} = 0.0$ then $LA_{alloc} = \max(LA_{target} - LA_{\phi},0)$ as with *evergreen-progressive* phenology. 
...
$\Delta LA_{sink}$ represents the leaf area increment expected by taking into account the maximum leaf tissue growth rate relative to sapwood area ($RGR_{leaf, max}$; in $m^2 \cdot cm^{-2} \cdot day^{-1}$), the percentage of the crown with active buds ($PCAB$) and the relative cell expansion rate given $T_{day}$, $\Psi_{symp,leaf}$ and $\pi_{0,leaf}$: 
\begin{equation}
...
Sapwood area increment $\Delta SA$ can only occur if $LA_{live}>0$. Newly assimilated carbon is preferentially allocated to leaves and fine roots whenever storage levels are low. $\Delta SA$ is defined as the minimum of two values expressing source and sink constraints:
\begin{equation}
