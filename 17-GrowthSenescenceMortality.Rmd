...
\end{equation}
where $SS_{leaf}$ is the current concentration of metabolic sugar, $SS_{leaf,growth}$ is the minimum concentration required for growth (specified in control vector `minimumSugarForGrowth`), $V_{storage,leaf}$ is the leaf storage volume, $m_{gluc}$ is glucose molar mass and the denominator contains the construction costs per leaf area unit, see eq. \@ref(eq:leafconstructioncost). Note that for deciduous species there is no leaf area before budburst, so that $SS_{leaf} = 0$. The model solves this problem by drawing carbon from $SS_{sapwood}$ when $LA^{live} = 0$.
...
[TO DO revise relationship between StemPLC, SA changes and functional sapwood]
...
\end{equation}
When simulating growth with the basic water balance model, leaf area index and Huber value are the only traits affected by changes in the size of compartments. It is only when we simulate growth with the advanced water balance model that we get a real sense of plant acclimation. Fine root biomass changes in each soil layer ($B_{fineroot,s}$) lead to updates in the proportion of fine roots in each layer ($FRP_s$). Stem maximum conductance per leaf area unit ($k_{stem, max}$; in $mmol·m^{-2}·s^{-1}·MPa^{-1}$) is modelled as a function of species-specific xylem conductivity ($k_{xylem, max}$; in $kg·m^{-1}·s^{-1}·MPa^{-1}$), leaf area, sapwood area and tree height [@Christoffersen2016]:
\begin{equation}
...
[Update of $\pi_{0,leaf}$ and $\pi_{0,stem}$]
