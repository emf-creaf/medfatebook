...
Let $i$ be the stand corresponding to seed sink location (i.e. red dot in Fig. \@ref(fig:seeddispersal), we first compute the geographic distances between $i$ and all the other stands using geographic coordinates $x$ and $y$ (assuming these are in meters).
\begin{equation}
d(i,j) = \sqrt{(x_i - x_j)^2 + (y_i - y_j)^2}
\end{equation}

Then we consider a given distance value $r > 0$ from $i$. We define the probability of choosing a given stand $j$ for a location at this distance as follows:
\begin{equation}
...
\end{equation}
Note that the former equation assumes that the density of spatial points is even across space. If not the sum should be lower than one.

Given the above definitions and assuming that seed bank should be refilled to 100% when if and only if all plots are seed sources, the percentage of seeds reaching the target location $i$, i.e.  $P_{seeds, i}$, is estimated using:
\begin{equation}
P_{seeds, i} = 100 \cdot 2 \pi \cdot \int_{r > 0}^{r_{\max}}{r \cdot f(r) \cdot \left( \sum_{j=1}^{n}{p_i(r,j)} \right) \cdot \delta r}
\end{equation}

where $r_{\max}$ is the maximum allowed dispersal distance