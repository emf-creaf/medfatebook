...
An important difference between `fordyn` and the previous simulation functions is that it does not require a specific input object, as in `spwb`or `growth`. In other words, soil, vegetation, meteorology and control parameters are directly introduced to the model function call.
...
+ `minFPARRecr [= 30]`: Default threshold of minimum fraction of PAR (in %) reaching the ground necessary for recruiting (when species parameter `MinFPARRecr` is missing).
