...
Snag fall based on a simplification of the model by Vanderwel et al. [-@vanderwel_snag_2006], where 5-yr. probability of snag fall depends on DBH, decay class and species identity. Following the results of Oberle et al. [-@oberle_when_2018], we defined species identity effects as a function of wood durability. Following the distribution of years of death determined by Aakala et al. [-@aakala_standing_2008] for snags corresponding to different age classes, we defined snag decay class as a function of age.
...
The `fordyn()` model divides the period to be simulated in years, which is the top-level time step of simulations. Given an input `forest` object, the function first initializes the input for function `growth()`. For each year to be simulated the model the performs the following steps:
 ...
 3. If required, simulates seed production, seed bank mortality, seed rain, recruitment from seeds and/or resprouting (see chapter \@ref(regeneration)). In a spatial context, seed rain will include seeds dispersed from other forest stands \@ref(seeddispersal). 
