...
Transpiration decreases leaf temperature, whereas radiation increases it and wind speed makes it more similar to air temperature. The following figures illustrate the effect of varying wind speed and flow rate on $T_{leaf}$ for two contrasted leaf sizes (see function `biophysics_leafTemperature`):
...
Leaf conductance to water vapour ($g_{sw}$; in $mol\, H_2O \cdot s^{-1} \cdot m^{-2}$) and to carbon dioxide ($g_{sc}$; in $mol\,CO_{2} \cdot s^{-1} \cdot m^{-2}$) are obtained for each value of transpiration flow $E$ (in $mol\, H_2O \cdot s^{-1} \cdot m^{-2}$) and $VPD_{leaf}$ using:
\begin{eqnarray}
...
(ref:leafGWfunction-cap) Examples of conductance to water vapour ($g_{sw}$) functions for a hydraulic network, corresponding to fig. \@ref(fig:supplynetwork) and for different soil textures. Left/right panel shows values for uncavitated/cavitated supply functions. Minimum and maximum conductance values ($g_{swmin} = 0.0045$ and $g_{swmax} = 0.3$) are indicated using dashed lines.
...
\end{equation}
$J_{max}$ and $J$ are the maximum and actual rate of electron transport (both in $\mu mol\,e \cdot m^{-2} \cdot s^{-1}$) and $c=0.9$ defines the curvature of the light-response curve. The gross assimilation rate $A$ at a given $C_i$ is the minimum of $A_e$ and $A_c$. To obtain a smooth $A$-vs-$C_i$ curve we used [@Collatz1991]:
\begin{equation}
...
\begin{equation}
V_{max,298}(L_j) =V_{max,298}\cdot \exp(-0.713\cdot L_j/LAI^{live})   
\end{equation}
where $L_j$ is the cumulative LAI value at a given canopy layer $j$ and $LAI^{live}$ is the overall LAI of the plant cohort. The following figure illustrate this decrease for the single-species canopy example of section \@ref(shortwaveradiationcanopyabs):
