...
\begin{eqnarray}
I^{sunlit}_{abs,i} &=& \frac{\sum_{j=1}^{l}{K^{sunlit}_{abs,i,j}}}{LAI^{sunlit}_{i}} \\
I^{shade}_{abs,i} &=& \frac{\sum_{j=1}^{l}{K^{shade}_{abs,i,j}}}{LAI^{shade}_{i}}
\end{eqnarray}
where $K^{sunlit}_{abs,i,j}$ and $K^{shade}_{abs,i,j}$ are the light absorbed per ground area unit by sunlit/shade leaves of cohort $i$ at layer $j$ (see section \@ref(shortwaveradiationcanopyabs)).
...
d. Using $I^{sunlit}_{SWR,i}$ as $I^{leaf}_{SWR}$ and $L^{sunlit}_{net,i}$ as $L^{leaf}_{net}$ in eq. \@ref(eq:leaftemperatureequation); and $I^{sunlit}_{PAR,i}$ as $I_{PAR}^{leaf}$ in \@ref(eq:leafQequation) to estimate sunlit leaf photosynthesis, which can be up-scaled to the crown level multiplying by $LAI^{sunlit}_{i}$. The same would be done for shade leaves. In a sun-shade canopy model one then calls the photosynthesis function twice (i.e. once for shade leaves and once for sunlit leaves) for each plant cohort $i$.
...
When presenting the sun-shade photosynthesis model, we assumed constant wind, temperature, water vapor pressure and $CO_2$ concentration values through the canopy. If a single-layer canopy energy balance is used, wind speed can still be different at different canopy heights. If a multi-layer canopy energy balance is used, all four environmental variables can differ between canopy layers. While @DePury1997 showed how to aggregate absorbed radiation and photosynthetic capacity, one has to choose specific values of environmental factors when calculating sunlit or shade photosynthesis. This can be done by determining the height corresponding to the mass center of sunlit/shade leaves for each cohort $i$. The canopy layer where this mass center height is contained is chosen as the layer from which environmental conditions will be taken. For any given plant cohort $i$, sunlit leaves will take their environmental conditions from layers above (or equal to) those corresponding to shade leaves.