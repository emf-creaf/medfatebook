...
Since $vp_{leaf}$ decreases when leaf temperature decreases in eq. \@ref(eq:leafvpequation), increasing transpiration decreases leaf VPD as a result of decreasing leaf temperature. To illustrate this effect, let us assume the following values of relative humidity, yielding a $vp_{air} = 1.91\, kPa$:
```{r}
RHmin = 60
RHmax = 75
vpa = meteoland::utils_averageDailyVP(Tmin, Tmax, RHmin, RHmax)
...
legend("bottomleft", bty="n", legend=textures, lwd=1, lty=1:3, cex=0.8)
```
Note that the VPD decreasing curves do not start at the same $VPD_{leaf}$ value despite corresponding to the same $T_{leaf}$ value, because of the effect of $\Psi_{leaf}$ on $vp_{leaf}$ in eq. \@ref(eq:leafvpequation).
...
In the previous subsection we showed how to calculate photosynthesis at the leaf level, but the function $A(\Psi_{leaf})$ needs to be up-scaled to a whole crown. 

### Multi-canopy photosynthesis model
Essentially we need to repeat the calculations of leaf temperature, leaf VPD, leaf gas conductance and photosynthesis for every leaf to be considered in the crown. Gross and net photosynthesis values can be then aggregated across the crown for each value of $\Psi_{leaf}$, so that the function $A(\Psi_{leaf})$ is obtained. 
...
### Vertical variation in photosynthetic capacity

Following @DePury1997, we further assume that maximum carboxylation and electron transport rates are highest for leaves at the top of the canopy and there is a exponential decrease from there towards the bottom, where maximum rates are 50% of those at the top: 
\begin{eqnarray}
V_{max,298,i,j} &=& V_{max,298,i}\cdot \exp(-0.713\cdot L^{\phi}_{i,j}/LAI_i^{\phi}) \\  
J_{max,298,i,j} &=& J_{max,298,i}\cdot \exp(-0.713\cdot L^{\phi}_{i,j}/LAI_i^{\phi})    
\end{eqnarray}
