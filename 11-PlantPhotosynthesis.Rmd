...
Leaf temperature ($T_{leaf}$; in $^\circ C$) can be calculated for any given flow rate $E_{leaf}$ using an energy balance equation [@Campbell1998]:
\begin{equation}
T_{leaf} = T_{canopy}+\frac{I_{abs}-\epsilon\cdot\sigma\cdot(T_{canopy}+273.15)^4-\lambda_v(T_{canopy})\cdot E_{leaf}}{C_p\cdot(g_r+g_{Ha})}
\end{equation}
...
Leaf conductance to water vapour ($g_{sw}$; in $mol\, H_2O \cdot s^{-1} \cdot m^{-2}$) and to carbon dioxide ($g_{sc}$; in $mol\,CO_{2} \cdot s^{-1} \cdot m^{-2}$) are obtained for each value of transpiration flow $E_{leaf}$ (here expressed in $mol\, H_2O \cdot s^{-1} \cdot m^{-2}$) and $VPD_{leaf}$ using:
\begin{eqnarray}
g_{sw} &=& E_{leaf} \cdot \frac{P_{atm}}{VPD_{leaf}}\\
g_{sc} &=& g_{sw}/1.6
\end{eqnarray}
...
As in section \@ref(shortwaveradiationcanopyabs) we consider a crown of one species divided into 10 layers, with constant leaf density. Many aspects may vary across the crown, including environmental conditions (such as direct/diffuse light or wind speed) and photosynthesis parameters (e.g. `Vmax298`). The previous crown definition and light parameters lead to a percentage of the above-canopy irradiance reaching each layer [@Anten2016]. Furthermore, it is generally accepted that sunlit and shade leaves need to be treated separately [@DePury1997]. Under these conditions, the amount of SWR and PAR absorbed per unit of leaf area at each canopy layer was shown in fig. \@ref(fig:absorbanceperlayer). For simplicity, here we will assume constant windspeed in all layers:
```{r}
ulayer = rep(2, 10)
```
