...
+ *Pre-dawn shade leaf water potential* ($\Psi_{pd,shade}$): the maximum of $\Psi_{leaf,shade,t}$ values.
+ *Pre-dawn sunlit leaf water potential* ($\Psi_{pd,sunlit}$): the maximum of $\Psi_{leaf,sunlit,t}$ values.
+ *Mid-day leaf water potential* ($\Psi_{md}$): the minimum of $\Psi_{leaf,t}$ values.
+ *Mid-day shade leaf water potential* ($\Psi_{md,shade}$): the minimum of $\Psi_{leaf,shade,t}$ values.
+ *Mid-day sunlit leaf water potential* ($\Psi_{md,sunlit}$): the minimum of $\Psi_{leaf,sunlit,t}$ values.
...
## Cavitation-induced defoliation and recovery {#cavitationinduceddefoliationadvanced}

By default, the water-balance model assumes that high levels of leaf hydraulic impairment will lead to leaf hydraulic disconnection and defoliation (although this process can be deactivated if `cavitationInducedDefoliation = FALSE`). When cavitation-induced defoliation is considered, the model assumes that crowns are made of a population of leaves whose hydraulic vulnerability follows a Gaussian distribution centered on the water potential corresponding to 50% loss of conductance (i.e. $\Psi_{50,leaf}$, or the corresponding value dictated by $c_{leaf}$ and $d_{leaf}$) and with a known coefficient of variation set to 10 % (by default), a modelling strategy developed by Hervé Cochard. The $slope_{leaf}$  parameter of the vulnerability curve, or the corresponding $c_{leaf}$ exponent in the case of a Weibull function, is considered constant. Leaves are hydraulically disconnected and die when their embolism rate exceeds a critical value set to 88 % of percent loss of conductance, by default (see defoliation control parameters as well as functions  `hydraulics_proportionDefoliationWeibull()` and `hydraulics_proportionDefoliationSigmoid()`). Ultimately, the assumed variability of hydraulic vulnerability and the current leaf water potential lead to a proportion of cavitation-induced crown defoliation.
