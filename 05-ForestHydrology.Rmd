...
\end{equation}
where $S_{canopy}$ is the canopy water storage capacity (in mm) –  i.e. the minimum amount of water needed to saturate the canopy –, $C_{canopy}$ is the canopy cover and $ER_{ratio}$ is the ratio of evaporation rate to rainfall rate during the rainfall event. Although interception models are normally applied to single-canopy stands, we apply the sparse Gash model to the whole stand (including shrubs and the herbaceous layer). Moreover, in our implementation stem interception is lumped with crown interception, so that $S_{canopy}$ represents both. Following @Watanabe1996 we estimate $S_{canopy}$, the canopy water storage capacity, from adjusted LAI values:
\begin{equation}
...
The amount of rainfall water that reaches the soil is the sum of net rainfall ($Pr_{net}$) and runon ($Ro$, in mm). The amount of rainfall water infiltrating into the soil is $Pr_{net} + Ro - Ru$, where $Ru$ (in mm) is the rainfall water lost by surface runoff (see function `hydrology_infiltrationAmount()`). Note that snow melt ($Sm$) does not participate from infiltration/runoff partitioning, because melting rates are assumed to be slow enough to infiltrate completely.
...
Note that percolation of rainfall infiltrating into the soil Following @Granier1999, part of the water reaching one soil layer percolates quickly through the macropores. The amount of water reaching each soil layer through macropores is modelled using an extinction function that depends on macroporosity (see function `hydrology_infiltrationRepartition`). The remaining water is retained by the micropores refilling the current soil layer.
...
Finally, actual herbaceous transpiration ($Tr_{herb}$) is reduced according to the water potential in the topmost soil layer ($\Psi_1$), assuming that transpiration reduction follows Weibull function with fixed coefficients:

\begin{equation}
Tr_{herb}=Tr_{herb,\max} \cdot \exp \left \{\ln{(0.5)}\cdot \left[ \frac{\Psi_1}{-1.5} \right] ^2 \right \} 
...
\begin{equation}
S_1 = I_1 + Sm - Es_1 - Tr_{herb} - Tr_{woody,1}
\end{equation}
and for $1 < i \leq N$:
\begin{equation}
S_i = I_i - Es_i - Tr_{woody,i}
\end{equation}
where $I$ is infiltration, $Sm$ is snow melt, $Tr_{herb}$ is herbaceous transpiration, $Tr_{woody}$ is woody plant transpiration and $Es$ is bare soikl evaporation. All right-hand terms are converted from $mm/day$ to instantaneous rates ($m/s$) for proper units in $S$.
