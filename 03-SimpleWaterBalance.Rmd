...
This chapter provides an overview of the water balance model described in @DeCaceres2015, which allows simulating the temporal variation in soil moisture and plant drought stress on a daily step basis for a given forest stand and for the period corresponding to input weather data. The model is executed using function `spwb()`, for a set of days, and function `spwb_day()` for a single day. While the information provided in this chapter should be enough to understand what the model does and when it can be useful, the following ones provide a deeper understanding on how the different processes are formulated.
...
There is a small set of functional parameters in the basic water balance model. They should be supplied for each plant cohort $i$. A first set of parameters relate to light extinction, water interception and leaf phenology processes (`paramsBase`):

| Symbol | Units | R  | Description                                  | 
|--------|-------|----| ---------------------------------------------|
| $k_{PAR,i}$ | | `kPAR` | PAR extinction coefficient |
...
A second set of parameters are related to transpiration (`paramsTransp`):
...
Since it simulates water balance for only one day, function `spwb_day()` returns a much more reduced output. This function is most useful with advanced water balance modelling (chapter \@ref(advancedwaterbalance)).
...
1. To monitor or forecast temporal variation in soil water content in particular stands or in multiple stands across a landscape. 
2. Monitor or forecast temporal variation of plant drought stress in particular stands or in multiple stands across a landscape (for example to anticipate mortality events).
