...
The set of steps corresponds to simulations if plant water pools *are not* considered, so that only the water balance of the overall soil matters. If plant water pools *are* considered, steps 1-2 are the same, because they apply to the whole forest stand (note that at present infiltration does not depend on antecedent conditions). However, steps 3 and 4 are performed for each water pool separately because they depend on antecedent soil moisture. Step 5 also involves defining woody plant water uptake from different plant water pools depending on the proportion of fine roots of each woody cohort in each pool (see \@ref(transpirationpools)). Soil water flows from these three steps are then averaged at the stand level. Step 6 is first performed on the soil corresponding to each pool and, finally, on the overall soil, using in each case the corresponding source/sink terms.
