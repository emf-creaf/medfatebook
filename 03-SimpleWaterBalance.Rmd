...
The set of steps corresponds to simulations if plant water pools *are not* considered, so that only the water balance of the overall soil matters. If plant water pools *are* considered, steps 1-3 are the same, because they apply to the whole forest stand (note that at present infiltration does not depend on antecedent conditions). However, step 4 is performed for each water pool separately, step 5 also involves defining woody plant water uptake from different plant water pools depending on the proportion of fine roots of each woody cohort in each pool (see \@ref(transpirationpools)). Step 6 is first performed on the soil corresponding to each pool and, finally, overall soil moisture is updated using pool proportions as weights. In this case, deep drainage for the overall soil is determined so as to force the water balance closure.
