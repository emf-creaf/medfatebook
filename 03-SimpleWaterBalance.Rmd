...
For every day to be simulated, the model performs the following steps:
  ...
  5. Decrease soil water content of the topmost soil layer due to bare soil evaporation ($Es$), if snow is not present (section \@ref(soilevaporation)), and herbaceous transpiration ($Tr_{herb}$; section \@ref(herbaceoustranspiration)).
  6. Determine woody plant transpiration ($Tr_{woody}$) and photosynthesis (chapter \@ref(transpirationgranier)).
...
+ `cavitationRefill [= "annual"]`: A string indicating how refilling of embolized conduits is done  (see section \@ref(irreversiblecavitation). Allowed values are: 
  + `"none"` - no refilling.
...
|`LeafPLC` | $PLC_{leaf,i}$ | [0-1] | Degree of leaf embolisation.|
|`StemPLC` | $PLC_{stem, i}$ | [0-1] | Degree of stem embolisation.|
