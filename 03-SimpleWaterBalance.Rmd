...
For every day to be simulated, the model performs the following steps:
  ...
  4. Define rainfall input as the sum of net rainfall ($Pr_{net}$) and surface runon ($Ro$). Divide rainfall input between infiltration and surface runoff ($Ru$) (section \@ref(runoff)). 
  ...
  7. Determine vertical water movement within the soil, while considering soil layer source/sinks from infiltration, bare soil evaporation and herbaceous transpiration adn woody plant extraction/redistribution (section \@ref(richards)). This updates soil moisture and lower boundary condition of water movement corresponds to deep drainage ($Dd$). 
...
Details of the formulation of each of these processes are provided within the referred chapters and sections. *If plant water pools are considered*, step 5 is performed for each plant water pool separately, step 6 involves defining extraction from different plant water pools depending on the proportion of fine roots of each cohort in each pool (see \@ref(transpirationpools)). Step 7 is also performed on the soil corresponding to each pool and, finally, overall soil moisture is updated using pool proportions as weights. In this case, deep drainage for the overall soil is determined so as to force the water balance closure.
