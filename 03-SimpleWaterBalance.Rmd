...
\begin{equation}
\Delta{V_{soil}} = Pr + Sm - In - Ru - Dd - Es - Tr_h - Tr_w
(\#eq:basicwaterbalanceequation)
\end{equation}

where $Pr$ is precipitation as rainfall, $Sm$ is water from melting the snow pack (if considered), $In$ is the interception loss (i.e., water evaporated after being intercepted by the canopy), $Ru$ is surface runoff, $Dd$ is deep drainage (i.e. water percolated to layers beyond soil depth), $Es$ is evaporation from soil, $Tr_h$ is the transpiration made by herbaceous plants and $Tr_w$ is the transpiration made by woody plants. 
...
For every day to be simulated, the model performs the following steps:
  ...
  5. Decrease soil water content of the topmost soil layer due to bare soil evaporation ($Es$), if snow is not present (section \@ref(soilevaporation)), and herbaceous transpiration ($Tr_h$).
  6. Determine woody plant transpiration ($Tr_w$) and photosynthesis (chapter \@ref(transpirationgranier)).
  7. Update plant water potential ($\Psi_{plant}$) and plant drought stress for each plant cohort and decrease soil water content due to woody plant transpiration.
