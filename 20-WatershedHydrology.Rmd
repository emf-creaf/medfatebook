...
For every day to be simulated in a watershed or subwatershed, the model performs the following steps:
 ...
 1. Calculation of soil lateral flows between adjacent cells, according to sub-steps if required by the user (see \@ref(soilsaturatedflows)) and determine the balance of soil lateral flows (i.e. determine $\Delta{S_{lat}}$) for each cell.
 2. Process cell vertical processes, including overland runoff-runon dynamics:
    ...
    b. In cells having soil (i.e. *wildland* or *agriculture*), cells are processed accounting for subsurface flows, runon from upslope cells, and capillarity from the groundwater level, which complete eqs.  \@ref(eq:cellsnowpackwaterbalanceequation) and \@ref(eq:cellsoilwaterbalanceequation). All processes mentioned in \@ref(basicwaterbalancescheduling) are taken into account. In cells that are watershed outlet or belong to channel capillarity from the aquifer is excluded to avoid the confusion (and instability) between local soil saturation excess and aquifer exfiltration.  
 ...
 3. Calculation of groundwater lateral flows between adjacent cells (see \@ref(groundwaterflows)) and apply changes in cell aquifer water content due to the balance of groundwater lateral flows ($\Delta{S_{base}}$ in eq. \@ref(eq:cellaquiferwaterbalanceequation)) and vertical flows between soils and the aquifer below, i.e. deep drainage and capillarity rise.
 4. Apply drainage losses from the aquifer towards a deeper aquifer not connected to watershed outlets (\@ref(deepaquiferloss)), completing eq. \@ref(eq:cellaquiferwaterbalanceequation).

Channel routing is optional, depending on whether a channel network has been supplied. Channel routing is performed after water balance simulations of the watershed (or its subwatersheds) are finished. For each day to be simulated and each channel cell, the model adds the water that reached the channel during water balance simulations to the volume of water in the network of the corresponding watershed outlet. After that the model applies channel routing, which can result in a channel backlog of water for the next day (see details \@ref(watershedexport)).
...
|                  |  `interflow`  |  Logical flag to include saturated soil flows in simulations. |
...
|                  | `baseflow`    |  Logical flag to include groundwater lateral flows in simulations. |
...
|                  | `free_drainage_outlets` | A boolean flag to prevent water table effects into local soil water balance of outlet/channel cells (included for numerical stability). |
...
| `outlet_export_m3s` | A matrix with daily runoff (in m3/s) at each of the outlet cells of the landscape. Each outlet drains its own subset of cells (sometimes including channel routing), so the overall daily watershed outflow corresponds to the sum of values in the corresponding row. |

Note that while simulations can be performed for sub-watersheds separately, at present the model does not produce results at the sub-watershed level. However, the partitioning of sub-watersheds is included in the output, so that users can cross it with `outlet_export_m3s` to get the discharge generated in each sub-watershed.

## Process details {#watershedbalancedetails}

### Soil saturated lateral flow  {#soilsaturatedflows}

Soil saturated lateral flows follow the saturated subsurface flow model of Wigmosta et al. [-@Wigmosta1994], which is based on a kinematic wave approximation. Each grid cell can exchange water with its eight adjacent neighbors. 

Local hydraulic gradients are approximated by local ground surface slopes, so that each cell will generally receive water from its upslope neighbors and discharge to its downslope neighbors. The rate of saturated flow from cell $i$ to its downgradient neighbors is equal to [@Wigmosta1999]:
\begin{eqnarray}
...
Soil saturated lateral flux inputs and outputs for a given cell $i$ are balanced to determine increase or decrease in soil moisture:
\begin{equation}
\Delta{S_{lat,i}} = S_{lat, in} - S_{lat, out} = \sum_{j}{q_{lat, ji}} - \sum_{j}{q_{lat, ij}}
\end{equation}
Hence, soil layers are not considered separately in the estimation of soil saturated flows and balance. For their source/sink effect on local soil water balance, however, the resulting lateral flow is divided between soil layers according to the water volume present (i.e. layers with larger amounts of water are assumed to exchange water more easily than layers that are drier, regardless of the soil volume they occupy). 

### Groundwater lateral flow  {#groundwaterflows}

Like the preceeding, groundwater lateral flows follow the saturated subsurface flow model of Wigmosta et al. [-@Wigmosta1994], which is based on a kinematic wave approximation. Each grid cell can exchange water with its eight adjacent neighbors. 

The transmissivity function for groundwater lateral flow in cell $i$ ($T_{base,i}$) is analogous to the soil saturated lateral flows: 
\begin{equation}
...
Groundwater lateral flux inputs and outputs for a given cell $i$ are balanced to determine increase or decrease of the aquifer balance:
\begin{equation}
\Delta{S_{base,i}} = S_{base, in} - S_{base, out} = \sum_{j}{q_{base, ji}} - \sum_{j}{q_{base, ij}}
\end{equation}

Aquifer update due to lateral flows is coupled to the vertical balance due to deep drainage or capillarity rise from/to the overlying soil. If the depth to aquifer becomes negative (i.e. the aquifer reaches the soil surface) for an outlet or channel cell, then exfiltration ($Exf$) occurs.
...
Every day, cells are processed in order from higher to lower elevation. For a given cell $i$, runoff is ($Ru_i$) includes infiltration excess and saturation excess (aquifer exfiltration is processed separately when dealing with groundwater flows). This runoff is divided among the neighboring cells at lower elevation. The runon of a neighbor $j$, $Ro_j$ is updated as:
