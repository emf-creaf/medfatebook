...
## Long-wave radiation balance {#longwaveradiationbalance}

Long-wave radiation (LWR) transfer within the canopy is based on the SWAW model [@Flerchinger2009]. Assume canopy layers are ordered from $j=1$ (in contact with the soil) to $j=J$ (in contact with the atmosphere above the boundary layer). The procedure first calculates downward LWR below each canopy layer, $L_{d,j}$,  from $j=J$ to $j=1$. The downward LWR below layer $j$ is calculated as:
\begin{equation}
L_{d,j} = \tau_{j} \cdot L_{d,j+1} + (1 - \tau_j) \cdot \epsilon_c \cdot \sigma \cdot (T_{air,j}+273.16)^{4}
\end{equation}
where $L_{d,j+1}$ is the downward LWR of the layer above ($L_{atm}$ in the case of $j=J$), $T_{air,j}$ is the air temperature of layer $j$, $\tau_j$ is the diffuse transmissivity of layer $j$, $\sigma = 5.67 \cdot 10^{-8}\,W \cdot K^{-4}\cdot m^{-2}$ is the Stephan-Boltzmann constant and $\epsilon_c = 0.97$ is the canopy emmissivity. Diffuse transmissivity values are defined by cohort $i$ and layer $j$ using:
\begin{equation}
\tau_{i,j} = \exp{(-k_{LWR} \cdot LAI^{all}_{i,j})}
\end{equation}
where $k_{LWR} = 0.7815$ is the extinction coefficient for LWR. The transmissivity of layer $j$ is simply $\tau_{j} = \sum_j {\tau_{i,j}}$. Once we know $L_{d,1}$ we can calculate the upwards LWR from the soil ($L_{u,s}$) using:
\begin{equation}
...
\end{equation}
The net LWR of cohort $i$ in layer $j$ is found using the relative proportion of light absorbed by the cohort (i.e. the complement of its diffuse transmissivity):
\begin{equation}
L_{net,i, j} = L_{net,j} \cdot \frac{1-\tau_{i,j}}{\sum_h{1-\tau_{i,h}}}
\end{equation}
whereas the net LWR of the whole canopy ($L_{net,ca}$) is simply the sum across layers of net LWR:
\begin{equation}
L_{net,ca} = \sum_j{L_{net,j}}
\end{equation}
and the net LWR of the soil ($L_{net,s}$) is:
\begin{equation}
