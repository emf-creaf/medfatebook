...
\begin{equation}
\Phi_{sca,i,j} = I_{b,j} \cdot k_{b} \left( \alpha_i^{0.5}\cdot \exp \left( \sum_{h}^{c}{-k_{b}\cdot \alpha_i \cdot 0.5\cdot LAI^{all}_{h,i}}\right) \\
...
\end{equation}
where $\beta$ is the solar elevation angle in radians, which changes throughout the day (see [section 4.3 of the reference manual](http://vegmod.ctfc.cat/meteolandbook/solarradiation.html#potential-radiation) for package **meteoland**). The amount of light absorbed by sunlit/shaded foliage of cohort $i$ in layer $j$ per leaf area unit ($\Phi^{sunlit}_{i,j}$ and $\Phi^{shade}_{i,j}$, respectively) is:
\begin{eqnarray}
