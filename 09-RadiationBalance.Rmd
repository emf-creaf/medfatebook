...
\end{equation}
where $\beta$ is the solar elevation angle in radians, which changes throughout the day (see [section 4.3 of the reference manual](http://vegmod.ctfc.cat/meteolandbook/solarradiation.html#potential-radiation) for package **meteoland**). The amount of light absorbed by shaded/sunlit foliage of cohort $i$ in layer $j$ per leaf area unit ($I_{SH,i,j}$ and $I_{SL,i,j}$, respectively) is:
\begin{eqnarray}
