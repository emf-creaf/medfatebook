...
\begin{eqnarray}
K_{abs,i,j}^{sunlit} &=& I^{sunlit}_{i,j} \cdot LAI^{sunlit}_{i,j} \\
K_{abs,i,j}^{shade} &=&  I^{shade}_{i,j} \cdot  LAI^{shade}_{i,j}
\end{eqnarray}
...
\end{eqnarray}
where $K_{beam}$ and $K_{dif}$ are the direct and diffuse irrradiance at the top of the canopy, $k_{b}$ is the extinction coefficient for direct light ($k_{b} = 0.8$) and $k_{d,i}$ is the extinction coefficient of cohort $i$ for diffuse SWR. From these, the SWR absorbed by the soil ($K_{abs,soil}$) is found by:
\begin{equation}
K_{abs,soil} = (1 - \gamma_{SWR, soil})\cdot (K_{beam, soil} + K_{dif, soil})
\end{equation}
