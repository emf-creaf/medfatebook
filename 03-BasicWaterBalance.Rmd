...
\begin{equation}
\Delta{V_{soil}} = Pr + Sm + Ro - In - Ru - Dd - Es - Tr_{herb} - Tr_{woody}
(\#eq:basicwaterbalanceequation)
\end{equation}

where $Pr$ is precipitation as rainfall or snow, $Sm$ is water from melting the snow pack (if snow-pack dynamics are considered), $In$ is the interception loss (i.e., water evaporated after being intercepted by the canopy), $Ro$ is surface runon (only relevant in spatially-explicit simulations), $Ru$ is surface runoff, $Dd$ is deep drainage (i.e. water percolated to layers beyond soil depth), $Es$ is evaporation from soil, $Tr_{herb}$ is the transpiration made by herbaceous plants and $Tr_{woody}$ is the transpiration made by woody plants. 

While the above describes the default water balance equations, variations can occur depending on specific control flags: 

a. *If snow dynamics are considered* [`snowpack`], the water balance of the snow pack ($\Delta{S_{snow}}$ in mm of snow water equivalents) is defined as:
\begin{equation}
\Delta{S_{snow}} = Ps - Sm
