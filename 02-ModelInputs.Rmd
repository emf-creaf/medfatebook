...
Weather input data must include variables calculated at the **daily** scale. Weather data should be arranged in a data frame with days in **rows** and variables in **columns**. Dates are needed in **medfate** to estimate parameters like solar declination or the day of the year ($DOY$). Importantly, *dates* should be included as **row names** (in format *year-month-day*) of the input meteorology data frame, or as a column named *dates* with objects of class `Date`.

Some weather variables must be present always, whereas others are optional, meaning that the package can estimate them if missing. The following table indicates the symbols, units, definitions and the variable name in R of input weather variables that are always **required**: 
...
Whereas the following table lists the same attributes for input weather variables that are **optional**: 
