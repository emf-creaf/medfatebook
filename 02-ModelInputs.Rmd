...
+ S3 class `soil` is a data frame that is initialized using a data frame of soil physical properties, but the table includes also soil parameters and state variables. Soil objects are described in subsection \@ref(soilinitialization).
...
The structure presented above for `forest` objects corresponds to the required data elements. A forest object can contain additional information when this is available. In particular, data frames `treeData` and `shrubData` can contain additional columns:
...
+ `ObsID`: A string identifying plant cohorts at the stage of forest sampling. Used to track the fate of particular plant cohorts through simulations. 
