...
Apart from its living plants, vegetation description can also include other elements, such as **cohorts of standing dead wood** (i.e. snags), or **leaf/branch/root litter** lying on the ground or found below the surface, or even the **seedling and seed banks**. These additional vegetation elements are described in sub-sections \@ref(deadplantelements) and \@ref(seedlingseedbanks) after the description of living plant cohorts.

### Structural attributes of living plant cohorts

A plant cohort $i$ is either a *tree cohort*, a *shrub cohort* or a *herb cohort*; and is defined using a set of structural attributes shown in the following table, where columns **spwb**, **growth** and **fordyn** indicate whether attributes are required by simulation functions `spwb()`, `growth()` and `fordyn()`, respectively:
...
$LAI$ variables refer to *one-sided leaf area* of plants per surface area of the stand. Leaves standing on branches of plants can be alive or dead. We will call *total leaf area index* of a plant cohort $i$ ($LAI^{all}_{i}$) to the sum of its dead and live (and unfolded) leaf area:
\begin{equation}
...
### Structural attributes of collective herbaceous layer
...
If there are $c$ plant cohorts, the leaf area index corresponding to living leaves, unfolded leaves and dead leaves can be aggregated across cohorts using:
...
We can also estimate the overall leaf area index of plant cohorts ($LAI^{all}$), including dead leaves:
...
While the previous section described the structural forest variables required to run simulations in **medfate**, forest data input in the package is easier if one uses a format that follows closely forest inventory plot descriptions. In this format, each forest stand is represented in an object of class `forest`, a list that contains several elements. Here we use an example forest stand provided with the package:
```{r}
data(exampleforest)
exampleforest
```

The most important items of `forest` are the two data frames describing woody plants: `treeData` (for trees) and `shrubData` for shrubs. While not required, an additional `herbData` item can be used to describe the herbs, with the same structure as for shrubs. Other items can be used to describe the seedling bank, the seed bank or the herbaceous layer collectively (see details using `?forest`). With the aim to help users in the task of constructing `forest` objects, package **medfate** includes functions to map user data in into tables `treeData` and `shrubData`.
...
Forest objects can be used to describe dead vegetation elements of forest stands. Dead elements can be either *standing* (e.g. tree snags), *downed litter* (e.g. branches, logs or leaf litter) or *belowground litter* (i.e. fine roots or coarse roots).

The optional data frame `snagData` contains information for dead plant cohorts, with the following columns:
...
Dead plant cohorts are characterized by species, diameter (for trees), height and age since death. Note that the description of dead cohorts quantifies the abundance of branches and large wood (i.e. stems), but not leaves (or twigs), which are assumed to fall before or soon after plant death. When dead plant cohorts are included, they can be used to simulate decomposition of standing dead elements and snag fall.

Downed and belowground litter can be described in an additional data frame `litterData`, containing the following columns:
...
Note that litter elements are characterized by species, so that the model can decompose litter elements at different speeds depending on species identity.

### Seedling and seed banks {#seedlingseedbanks}

### Update of vegetation during simulations

Vegetation characteristics stay constant during simulations using functions `spwb()`, although the actual (unfolded) leaf area and dead leaf area may vary depending on leaf phenology.
