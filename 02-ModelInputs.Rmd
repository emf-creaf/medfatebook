...
| $T_{min}$ | $^{\circ} \mathrm{C}$ | `MinTemperature` | Minimum temperature | Y | Y |
| $T_{max}$ | $^{\circ} \mathrm{C}$ | `MaxTemperature` | Maximum temperature | Y | Y |
| $RH_{min}$ | % | `MinRelativeHumidity` | Minimum relative humidity | Y | Y |
| $RH_{max}$ | % | `MaxRelativeHumidity` | Maximum relative humidity | Y | Y |
...
Dates are needed in **medfate** to estimate parameters like solar declination or the day of the year ($DOY$). Importantly, *dates* should be included as row names (in format *year-month-day*) of the input meteorology data frame. Since it allows producing data frames with appropriate variable units and row/column names, we recommend meteorological input to be generated using package **meteoland** [@DeCaceres2018], but other sources are possible.
