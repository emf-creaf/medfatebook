...
The following table indicates the symbols, units, definitions and the variable name in R.
...
| $u$ | $m \cdot s^{-1}$ | `WindSpeed` | Wind speed (optional) |
| $C_{atm}$ | $ppm$ | `CO2` | Atmospheric (above-canopy) $CO_2$ concentration (optional) |
...
+ `transpirationMode [="Granier"]` is an important control parameter, because it allows the user to switch between the *basic* and *advanced* water balance models (chapters \@ref(basicwaterbalance) and \@ref(advancedwaterbalance)). 
