...
The *water content* ($V_s$ in mm) of a soil layer $s$ is calculated from its water potential ($\Psi_s$) and water retention curve using:
\begin{equation}
V_s(\Psi_s) = d_s\cdot ((100-P_{rocks,s})/100)\cdot\theta_{s}(\Psi_s)
\end{equation}
where $d_s$ is the depth of the soil layer (in mm) and $P_{rocks,s}$ is the percentage of rock fragments. The overall water content of the soil is simply the sum of water content over soil layers:
\begin{equation}
V_{soil} = \sum_{s=1}^S{V_s(\Psi_s)}
\end{equation}

A number of fixed water contents are important to remember. 
...
Height ($H$) values refer to average height of individuals included in the cohort, and the same for crown ratio ($CR$) and diameter at breast height ($DBH$). While plant height is relatively easy to measure, it is normally quite hard to estimate leaf area indices for stands and species. Package **medfate** provides some utilities to provide estimates crown ratio and $LAI$ from forest inventory data (e.g. heights, DBH and density for measured trees), using allometric relationships calibrated for catalonia (see chapter \@ref(allometricmodels)). 
