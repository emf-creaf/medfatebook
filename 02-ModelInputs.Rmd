...
|--------|-------|----| ------------------------------------|-----|-----|-----|
| $SP$  |  | `Species` | Species identity |  Y  |  Y  |  Y  |
| $H$  | $cm$  | `Height` | Average plant height |  Y  |  Y  |  Y  |
| $CR$ |       | `CR`| Crown ratio (i.e. ratio between crown length and plant height) | Y | Y |  N  |
| $N$ | $ind · ha^{-1}$ | `N` | Density of individuals | N | Y |  Y  |
| $DBH$ | $cm$ | `DBH` | Tree diameter at breast height | N | Y |  Y  |
| $Cover$ | % | `Cover` | Projected percent cover | N | Y |  Y  |
| $LAI_{live}$ | $m^2 \cdot m^{-2}$ | `LAI_live` | (Maximum) leaf area index | Y | Y | N |
| $LAI_{dead}$ | $m^2 \cdot m^{-2}$ | `LAI_dead` | (Standing) dead leaf area index | Y | Y |  N  |
| $LAI_{\phi}$ | $m^2 \cdot m^{-2}$ | `LAI_expanded` | Current expanded leaf area index | Y | Y |  N  |
| $LAI_{NC}$ |$m^2 \cdot m^{-2}$ | `LAI_nocomp` | Leaf area index assuming no competition with other plants | N | Y |  N  |
| $Z_{50}$ | mm | `Z50` | Depth above which 50% of the fine root mass is located | Y | Y | Y |
| $Z_{95}$ | mm | `Z95` | Depth above which 95% of the fine root mass is located | Y | Y | Y |
...
#### Collective herbaceous layer {#collectiveherbs}
...
+ `LAI`: Leaf area index ($m^2 \cdot m^{-2}$), corresponding to $LAI_{live}$, avoiding the use of allometric relationships for leaf biomass in water balance simulations.
+ `LAIMistletoe`: Leaf area index ($m^2 \cdot m^{-2}$) of hemi-parasitic (mistletoe) plants.
+ `FoliarBiomass`: Standing dry biomass of leaves ($kg \cdot m^{-2}$), which together with $SLA$ will be used to estimate $LAI_{live}$, avoiding the use of allometric relationships for leaf biomass in water balance simulations.
...
| `SmallBranches` | Necromass of standing small dead branches (diameters between 0.635 cm an 7.5 cm; 10h and 100h fuels; in g C/m2) |
| `LargeWood` | Necromass of standing large dead wood (diameters > 7.5 cm; 1000h and 10000h fuels; in g C/m2). |
...
| `Twigs` | Necromass of twig  litter (diameters < 0.635; in g C/m2). |
| `SmallBranches` | Necromass of downed small dead branches (diameters between 0.635 cm an 7.5 cm; 10h and 100h fuels; in g C/m2). |
| `LargeWood` | Necromass of (downed) large dead wood (diameters > 7.5 cm; 1000h and 10000h fuels; in g C/m2). |
...
#### Soil organic carbon pools {#socpools}

Forest objects can include the description of soil organic matter pools, in an element called `SOCData`. Soil organic matter pools will depend on the decomposition model employed, being CENTURY the only available at present. `SOCData` for CENTURY is a named numerical vector with the following items:

| R items  | Description                        | 
|--------|------------------------------------|
| `SurfaceMetabolic` | Metabolic carbon in the surface (in g C/m2). |
| `SoilMetabolic` | Metabolic carbon in the soil (in g C/m2). |
| `SurfaceActive` | Active decomposition carbon pool in the surface (in g C/m2). |
| `SoilActive` | Active decomposition carbon pool in the soil (in g C/m2). |
| `SurfaceSlow` | Slow decomposition carbon pool in the surface (in g C/m2). |
| `SoilSlow` | Slow decomposition carbon pool in the soil (in g C/m2). |
| `SoilPassive` | Passive decomposition carbon pool in the soil (in g C/m2). |

### Leaf and root distribution

#### Leaf area index of a plant cohort

$LAI$ variables refer to *one-sided leaf area* of plants per surface area of the stand. Only living plant cohorts (or the collective herbaceous layer) contribute to the leaf area of the forest stand (i.e. seedlings or dead snags are assumed to not contribute significantly to leaf area). Leaves standing on branches of living plant cohorts can be alive or dead. We will call *total leaf area index* of a plant cohort $i$ ($LAI_{all,i}$) to the sum of its dead and live (and unfolded) leaf area:
\begin{equation}
LAI_{all,i} = LAI_{\phi,i}+LAI_{dead,i}
\end{equation}

For simulations not involving growth, winter-deciduous phenology is a prognosed variable, and the current level of leaves unfolded may be lower than the maximum (live) amount of leaves. In simulations involving growth, both $LAI_{\phi}$ and $LAI_{live}$ can change, the former due to leaf phenology (unfolding is simulated explicitly and has carbon costs) and the latter due to leaf allocation limits during plant development.

#### Leaf area index of the forest stand

If there are $c$ living plant cohorts, the leaf area index corresponding to living leaves, unfolded leaves, dead leaves and leaves of hemi-parasitic (mistletoe) plants can be aggregated across cohorts using:

\begin{eqnarray}
LAI_{live} &=& \sum_{i=1}^c{LAI_{live,i}} \\
LAI_{\phi} &=& \sum_{i=1}^c{LAI_{\phi,i}} \\
LAI_{dead} &=& \sum_{i=1}^c{LAI_{dead,i}} \\
LAI_{mistletoe} &=& \sum_{i=1}^c{LAI_{mistletoe,i}}
\end{eqnarray}

We can also estimate the overall leaf area index of living plant cohorts ($LAI_{all}$), including dead leaves:

\begin{eqnarray}
LAI_{all} &=& \sum_{i=1}^{c}{\left[LAI_{\phi,i}+LAI_{dead,i}\right]} = LAI_{\phi}+LAI_{dead}
\end{eqnarray}

Finally, we can also estimate the overall leaf area index of the whole stand ($LAI_{all,stand}$), which also takes into account the leaf area of mistletoes ($LAI_{mistletoe}$) and the collective herbaceous layer ($LAI_{herb}$), if defined:

\begin{eqnarray}
LAI_{all, stand} &=& LAI_{all} + LAI_{herb} = LAI_{\phi}+LAI_{dead} + LAI_{mistletoe} + LAI_{herb} 
\end{eqnarray}

#### Vertical leaf distribution {#leafdistribution}

The vegetation input structures allow the package to determine the vertical distribution of leaves in the stand. The leaf area of any woody plant cohort is assumed to be distributed vertically following a **truncated Gaussian function** whose standardized values -1.5 and 1.5 correspond to crown base height ($H_{crown}$; in $cm$) and total plant height ($H$), respectively. Crown base height is defined as the height corresponding to the first living branch. It is calculated from the crown ratio of the cohort ($CR$; a proportion between 0 and 1), which in turn can be estimated as explained in \@ref(eq:CrownHeight). 

Simulation models in **medfate** divide the vertical dimension into vertical layers (by default are 100 cm width, but see control parameter `verticalLayerSize`). Let us define $LAI_{all, i,j} = LAI_{\phi,i,j}+LAI_{dead,i,j}$ as the leaf area index of cohort $i$ in layer $j$ including both functional leaves and dead leaves standing on branches. The truncated Gaussian distribution defines the $LAI_{\phi,i,j}$ and $LAI_{dead,i,j}$ for all plant cohorts and vertical layers.
...
\begin{equation}
f_{pool,i} = \frac{LAI_{live,i}}{LAI_{live}}
\end{equation}
...
In contrast, growth simulation requires updating the structure of vegetation, i.e. plant heights, tree diameters, tree density and shrub cover. Therefore, function `growth()` can modify any of the structural attributes of living plant cohorts. 

Finally, function `fordyn()` modifies not only structural variables of plants cohorts but also involves many other changes: (a) the removal of dead (or cut) cohorts; (b) the modification of the seed bank and the seedling bank; (c) the recruitment of new cohorts during simulations; (d) the modification of snag data and litter data.
...
+ `rhizosphereOverlap [="partial"]` is also a relevant parameter, since changing its value allows considering that plants extract soil water from the same pools, or partially or totally unrelated water pools, as explained in \@ref(plantwaterpools).
