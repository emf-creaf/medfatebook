...
\end{equation}
where $LAI_i^{sunlit}$ and $LAI_i^{shade}$ are the cohorts LAI values for sunlit and shade leaves, from eq. \@ref(eq:sunlitshadelai). Note that profit maximization calculations for shade and sunlit leaves imply different amount of water extracted from the soil layers and different water potentials along the hydraulic pathway. To overcome this issue, the model uses the hydraulic supply function to find the transpiration rate ($E_i$), extraction flows from soil layers ($E_{i,s}$) and water potentials ($\Psi_{leaf}$, $\Psi_{stem}$, $\Psi_{rootcrown}$, ...) corresponding to  $E_i^{average}$. 

Instantaneous net photosynthesis per leaf area of sunlit and shade leaves (i.e. $A_{n,i}^{sunlit}$ and $A_{n,i}^{shade}$) is also averaged into an instantaneous net photosynthesis per leaf area unit of the plant cohort $i$:
\begin{equation}
 A_{n,i} = \frac{A_{n,i}^{sunlit} \cdot LAI_i^{sunlit} + A_{n,i}^{shade} \cdot LAI_i^{shade}}{LAI_i^{\phi}}
\end{equation}
