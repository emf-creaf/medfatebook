...
\begin{equation}
\theta(\Psi_{leaf}) = \left[ \frac{k_{c,max}-k_{c}(\Psi_{leaf})}{k_{c,max}-k_{crit}}\right]^{a_{\theta}}
\end{equation}
where $k_c(\Psi_{leaf}) = dE/d\Psi(\Psi)$ is the slope of the supply function corresponding to (leaf) water potential $\Psi_{leaf}$, $k_{c,max}$ is the maximum slope of the supply function (which occurs when $E_{leaf}=0$), i.e. the maximum whole-plant conductance for the current soil moisture conditions, and $k_{crit} = k_c(\Psi_{crit})$ is the slope of the supply function at $E_{leaf} = E_{crit}$ the critical flow beyond which hydraulic failure occurs. $a_{\theta}$ is an exponent that modifies the cost function, specified as a control parameter. In @Sperry2016a, this exponent was missing (i.e. $a_{\theta} = 1.0$), and this is the default in **medfate**, but we introduced it to allow some flexibility in the gain/cost comparison (see below).
...
PM11_s2 =transp_profitMaximization(supplyNetwork11, psi2A11, Gmin, Gmax, gainModifier = 0.5)
PM12_s2 =transp_profitMaximization(supplyNetwork12, psi2A12,Gmin, Gmax, gainModifier = 0.5)
PM13_s2 =transp_profitMaximization(supplyNetwork13, psi2A13, Gmin, Gmax, gainModifier = 0.5)
PM14_s2 =transp_profitMaximization(supplyNetwork14, psi2A14,Gmin, Gmax, gainModifier = 0.5)
PM21_s2 =transp_profitMaximization(supplyNetwork21, psi2A21,Gmin, Gmax, gainModifier = 0.5)
PM22_s2 =transp_profitMaximization(supplyNetwork22, psi2A22,Gmin, Gmax, gainModifier = 0.5)
PM23_s2 =transp_profitMaximization(supplyNetwork23, psi2A23,Gmin, Gmax, gainModifier = 0.5)
PM24_s2 =transp_profitMaximization(supplyNetwork24, psi2A24,Gmin, Gmax, gainModifier = 0.5)
PM31_s2 =transp_profitMaximization(supplyNetwork31, psi2A31, Gmin, Gmax, gainModifier = 0.5)
PM32_s2 =transp_profitMaximization(supplyNetwork32, psi2A32,Gmin, Gmax, gainModifier = 0.5)
PM33_s2 =transp_profitMaximization(supplyNetwork33, psi2A33, Gmin, Gmax, gainModifier = 0.5)
PM34_s2 =transp_profitMaximization(supplyNetwork34, psi2A34, Gmin, Gmax, gainModifier = 0.5)

PM11emb_s2 =transp_profitMaximization(supplyNetwork11emb, psi2A11emb, Gmin, Gmax, gainModifier = 0.5)
PM12emb_s2 =transp_profitMaximization(supplyNetwork12emb, psi2A12emb, Gmin, Gmax, gainModifier = 0.5)
PM13emb_s2 =transp_profitMaximization(supplyNetwork13emb, psi2A13emb, Gmin, Gmax, gainModifier = 0.5)
PM14emb_s2 =transp_profitMaximization(supplyNetwork14emb, psi2A14emb, Gmin, Gmax, gainModifier = 0.5)
PM21emb_s2 =transp_profitMaximization(supplyNetwork21emb, psi2A21emb, Gmin, Gmax, gainModifier = 0.5)
PM22emb_s2 =transp_profitMaximization(supplyNetwork22emb, psi2A22emb, Gmin, Gmax, gainModifier = 0.5)
PM23emb_s2 =transp_profitMaximization(supplyNetwork23emb, psi2A23emb, Gmin, Gmax, gainModifier = 0.5)
PM24emb_s2 =transp_profitMaximization(supplyNetwork24emb, psi2A24emb, Gmin, Gmax, gainModifier = 0.5)
PM31emb_s2 =transp_profitMaximization(supplyNetwork31emb, psi2A31emb, Gmin, Gmax, gainModifier = 0.5)
PM32emb_s2 =transp_profitMaximization(supplyNetwork32emb, psi2A32emb, Gmin, Gmax, gainModifier = 0.5)
PM33emb_s2 =transp_profitMaximization(supplyNetwork33emb, psi2A33emb, Gmin, Gmax, gainModifier = 0.5)
PM34emb_s2 =transp_profitMaximization(supplyNetwork34emb, psi2A34emb, Gmin, Gmax, gainModifier = 0.5)

```

The following figures illustrate the $\theta(\Psi_{leaf})$ curves corresponding to the supply functions:

(ref:costfunction-cap) Cost functions (i.e. $\theta(\Psi_{leaf})$) obtained for a hydraulic network, corresponding to fig. \@ref(fig:supplynetwork) and different soil textures and soil water potentials. Left/right panels show values for uncavitated/cavitated supply functions, respectively. In all cases $a_{\theta}=0$.

```{r costfunction, echo=FALSE, fig.width=8, fig.height=3.5, fig.align="center", fig.cap='(ref:costfunction-cap)'}
...
\begin{equation}
\beta(\Psi_{leaf}) = \left[\frac{A(\Psi_{leaf})}{A_{max}}\right]^{a_{\beta}}
\end{equation}
where $A_{max}$ is the instantaneous maximum (gross) assimilation rate estimated over the full $\Psi_{leaf}$ range (including values that imply a stomatal conductance larger than the maximum). Similarly to the cost function, $a_{\beta}$ is an exponent that modifies the gain function, specified as a control parameter. In @Sperry2016a, this exponent was missing (i.e. $a_{\beta} = 1.0$), and this is the default in **medfate**, but we introduced it to allow some flexibility in the gain/cost comparison.

The following figures illustrate the $\beta(\Psi_{leaf})$ curves corresponding to the supply and assimilation functions, for either  $a_{\beta} = 1.0$ (top row) or  $a_{\beta} = 0.5$ (bottom row):

(ref:gainfunction-cap) Gain function ($\beta(\Psi_{leaf})$) obtained for a hydraulic network, corresponding to fig. \@ref(fig:supplynetwork) and different soil textures and soil water potentials. Left/right panels show values for uncavitated/cavitated supply functions, respectively. Top and bottom panels correspond to $a_{\beta}=1.0$ and $a_{\beta}=0.5$, respectively. 

```{r gainfunction, echo=FALSE, fig.width=8, fig.height=7, fig.align="center", fig.cap='(ref:gainfunction-cap)'}
par(mar=c(4,4,3,1), mfrow=c(2,2))

plot(-supplyNetwork11$psiLeaf, PM11$Gain, type="l", col=col1, ylab="Gain function (a = 1.0)", xlab = "Canopy sap pressure (-MPa)",  xlim=c(0, 7), ylim=c(0,1), lwd=1, main = "Gain (original)")
...
plot(-supplyNetwork11emb$psiLeaf, PM11emb$Gain, type="l", col=col1, ylab="Gain function (a = 1.0)", xlab = "Canopy sap pressure (-MPa)",  xlim=c(0, 7), ylim=c(0,1), lwd=1, main = "Gain (after cavitation)")
...
plot(-supplyNetwork11$psiLeaf, PM11_s2$Gain, type="l", col=col1, ylab="Gain function (a = 0.5)", xlab = "Canopy sap pressure (-MPa)",  xlim=c(0, 7), ylim=c(0,1), lwd=1, main = "Gain (original)")
lines(-supplyNetwork12$psiLeaf, PM12_s2$Gain, lty=1, lwd=1, col=col2)
lines(-supplyNetwork13$psiLeaf, PM13_s2$Gain, lty=1, lwd=1, col=col3)
lines(-supplyNetwork14$psiLeaf, PM14_s2$Gain, lty=1, lwd=1, col=col4)
lines(-supplyNetwork21$psiLeaf, PM21_s2$Gain, lty=2, lwd=1, col=col1)
lines(-supplyNetwork22$psiLeaf, PM22_s2$Gain, lty=2, lwd=1, col=col2)
lines(-supplyNetwork23$psiLeaf, PM23_s2$Gain, lty=2, lwd=1, col=col3)
lines(-supplyNetwork24$psiLeaf, PM24_s2$Gain, lty=2, lwd=1, col=col4)
lines(-supplyNetwork31$psiLeaf, PM31_s2$Gain, lty=3, lwd=1, col=col1)
lines(-supplyNetwork32$psiLeaf, PM32_s2$Gain, lty=3, lwd=1, col=col2)
lines(-supplyNetwork33$psiLeaf, PM33_s2$Gain, lty=3, lwd=1, col=col3)
lines(-supplyNetwork34$psiLeaf, PM34_s2$Gain, lty=3, lwd=1, col=col4)
legend("bottomright", bty="n", legend=textures, lwd=1, lty=1:3, cex=0.8)


plot(-supplyNetwork11emb$psiLeaf, PM11emb_s2$Gain, type="l", col=col1, ylab="Gain function (a = 0.5)", xlab = "Canopy sap pressure (-MPa)",  xlim=c(0, 7), ylim=c(0,1), lwd=1, main = "Gain (after cavitation)")
lines(-supplyNetwork12emb$psiLeaf, PM12emb_s2$Gain, lty=1, lwd=1, col=col2)
lines(-supplyNetwork13emb$psiLeaf, PM13emb_s2$Gain, lty=1, lwd=1, col=col3)
lines(-supplyNetwork14emb$psiLeaf, PM14emb_s2$Gain, lty=1, lwd=1, col=col4)
lines(-supplyNetwork21emb$psiLeaf, PM21emb_s2$Gain, lty=2, lwd=1, col=col1)
lines(-supplyNetwork22emb$psiLeaf, PM22emb_s2$Gain, lty=2, lwd=1, col=col2)
lines(-supplyNetwork23emb$psiLeaf, PM23emb_s2$Gain, lty=2, lwd=1, col=col3)
lines(-supplyNetwork24emb$psiLeaf, PM24emb_s2$Gain, lty=2, lwd=1, col=col4)
lines(-supplyNetwork31emb$psiLeaf, PM31emb_s2$Gain, lty=3, lwd=1, col=col1)
lines(-supplyNetwork32emb$psiLeaf, PM32emb_s2$Gain, lty=3, lwd=1, col=col2)
lines(-supplyNetwork33emb$psiLeaf, PM33emb_s2$Gain, lty=3, lwd=1, col=col3)
lines(-supplyNetwork34emb$psiLeaf, PM34emb_s2$Gain, lty=3, lwd=1, col=col4)
abline(v=-psiCav, col="gray", lwd=1.5)
legend("bottomright", bty="n", legend=textures, lwd=1, lty=1:3, cex=0.8)

```

Note that setting $a_{\beta} = 0.5$ makes gain function steeper at the begining of the curve, hence making additional gains in photosynthesis less relevant compared to $a_{\beta} = 1.0$. This has an effect on the profit maximization strategy presented in the next subsection.
...
(ref:profitfunction-cap) Profit functions (i.e. $Profit(\Psi_{leaf})$) obtained for a hydraulic network, corresponding to fig. \@ref(fig:supplynetwork) and different soil textures and soil water potentials. Left/right panels show values for uncavitated/cavitated supply functions, respectively. Top and bottom panels correspond to $a_{\beta}=1.0$ and $a_{\beta}=0.5$, respectively. 
...
plot(-supplyNetwork11$psiLeaf, PM11$Profit, type="l", col=col1, ylab="Profit (a = 1)", xlab = "Leaf sap pressure (-MPa)",  xlim=c(0, 7), ylim=c(0,1), lwd=1, main="original")
...
plot(-supplyNetwork11emb$psiLeaf, PM11emb$Profit, type="l", col=col1, ylab="Profit (a = 1)", xlab = "Leaf sap pressure (-MPa)",  xlim=c(0, 7), ylim=c(0,1), lwd=1, main="after cavitation")
...
plot(-supplyNetwork11$psiLeaf, PM11_s2$Profit, type="l", col=col1, ylab="Profit (a = 0.5)", xlab = "Leaf sap pressure (-MPa)",  xlim=c(0, 7), ylim=c(0,1), lwd=1, main="original")
...
plot(-supplyNetwork11emb$psiLeaf, PM11emb_s2$Profit, type="l", col=col1, ylab="Profit (a = 0.5)", xlab = "Leaf sap pressure (-MPa)",  xlim=c(0, 7), ylim=c(0,1), lwd=1, main="after cavitation")
...
Differences between $a_{\beta}=1.0$ and $a_{\beta}=0.5$ can be more easily seen when plotting directly the optima in terms of both canopy sap pressure and flow rate:

(ref:compprofit-cap) Comparison of optimum flow and water potential values under profit functions built using $a_{\beta}=1.0$ and $a_{\beta}=0.5$, from the results of fig. \@ref(fig:profitfunction).
...
legend("topleft", bty="n", legend=c("Profit (a = 1) (original)", "Profit (a = 1) (cavitated)", "Profit (a = 2) (original)", "Profit (a = 2) (cavitated)"), pch=c(0,15, 1,19), cex=0.6)
...
When changing from $a_{\beta}=1.0$ to $a_{\beta}=0.5$ the optimum values are shifted towards less negative water potentials and, accordingly, lower transpiration rates. Hence, $a_{\beta}$ allows modifying the relative weight of gains and costs.
...
(ref:profitfunction-gscap) Profit function as a function of stomatal conductance, corresponding to fig. \@ref(fig:profitfunction) and different soil textures and soil water potentials. Left/right panels show values for uncavitated/cavitated supply functions, respectively. Top and bottom panels correspond to $a_{\beta}=1.0$ and $a_{\beta}=0.5$, respectively. 

```{r profitfunction-gs, echo=FALSE, warning=FALSE, fig.width=8, fig.height=7, fig.align="center", fig.cap = '(ref:profitfunction-gscap)'}
par(mar=c(4,4,2,1), mfrow=c(2,2))
plot(psi2A11$WaterVaporConductance, PM11$Profit, type="l", col=col1, ylab="Profit (a = 1)", xlab = "Stomatal conductance",  xlim=c(0.001, 0.5), log="x", ylim=c(0,1), lwd=1, main="original")
points(psi2A11$WaterVaporConductance[PM11$iMaxProfit+1],
         PM11$Profit[PM11$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A12$WaterVaporConductance, PM12$Profit, lty=1, lwd=1, col=col2)
points(psi2A12$WaterVaporConductance[PM12$iMaxProfit+1],
         PM12$Profit[PM12$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A13$WaterVaporConductance, PM13$Profit, lty=1, lwd=1, col=col3)
points(psi2A13$WaterVaporConductance[PM13$iMaxProfit+1],
         PM13$Profit[PM13$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A14$WaterVaporConductance, PM14$Profit, lty=1, lwd=1, col=col4)
points(psi2A14$WaterVaporConductance[PM14$iMaxProfit+1],
         PM14$Profit[PM14$iMaxProfit+1], col=col4, pch = 0)
lines(psi2A21$WaterVaporConductance, PM21$Profit, lty=2, lwd=1, col=col1)
points(psi2A21$WaterVaporConductance[PM21$iMaxProfit+1],
         PM21$Profit[PM21$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A22$WaterVaporConductance, PM22$Profit, lty=2, lwd=1, col=col2)
points(psi2A22$WaterVaporConductance[PM22$iMaxProfit+1],
         PM22$Profit[PM22$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A23$WaterVaporConductance, PM23$Profit, lty=2, lwd=1, col=col3)
points(psi2A23$WaterVaporConductance[PM23$iMaxProfit+1],
         PM23$Profit[PM23$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A24$WaterVaporConductance, PM24$Profit, lty=2, lwd=1, col=col4)
points(psi2A24$WaterVaporConductance[PM24$iMaxProfit+1],
         PM24$Profit[PM24$iMaxProfit+1], col=col4, pch = 0)
lines(psi2A31$WaterVaporConductance, PM31$Profit, lty=3, lwd=1, col=col1)
points(psi2A31$WaterVaporConductance[PM31$iMaxProfit+1],
         PM31$Profit[PM31$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A32$WaterVaporConductance, PM32$Profit, lty=3, lwd=1, col=col2)
points(psi2A32$WaterVaporConductance[PM32$iMaxProfit+1],
         PM32$Profit[PM32$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A33$WaterVaporConductance, PM33$Profit, lty=3, lwd=1, col=col3)
points(psi2A33$WaterVaporConductance[PM33$iMaxProfit+1],
         PM33$Profit[PM33$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A34$WaterVaporConductance, PM34$Profit, lty=3, lwd=1, col=col4)
points(psi2A34$WaterVaporConductance[PM34$iMaxProfit+1],
         PM34$Profit[PM34$iMaxProfit+1], col=col4, pch = 0)
legend("topright", bty="n", legend=textures, lwd=1, lty=1:3, cex=0.8)
abline(h=0,col="gray")
...
plot(psi2A11emb$WaterVaporConductance, PM11emb$Profit, type="l", col=col1, ylab="Profit (a = 1)", xlab = "Stomatal conductance",  xlim=c(0.001, 0.5), log="x", ylim=c(0,1), lwd=1, main="after cavitation")
points(psi2A11emb$WaterVaporConductance[PM11emb$iMaxProfit+1],
         PM11emb$Profit[PM11emb$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A12emb$WaterVaporConductance, PM12emb$Profit, lty=1, lwd=1, col=col2)
points(psi2A12emb$WaterVaporConductance[PM12emb$iMaxProfit+1],
         PM12emb$Profit[PM12emb$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A13emb$WaterVaporConductance, PM13emb$Profit, lty=1, lwd=1, col=col3)
points(psi2A13emb$WaterVaporConductance[PM13emb$iMaxProfit+1],
         PM13emb$Profit[PM13emb$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A14emb$WaterVaporConductance, PM14emb$Profit, lty=1, lwd=1, col=col4)
points(psi2A14emb$WaterVaporConductance[PM14emb$iMaxProfit+1],
         PM14emb$Profit[PM14emb$iMaxProfit+1], col=col4, pch = 0)
lines(psi2A21emb$WaterVaporConductance, PM21emb$Profit, lty=2, lwd=1, col=col1)
points(psi2A21emb$WaterVaporConductance[PM21emb$iMaxProfit+1],
         PM21emb$Profit[PM21emb$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A22emb$WaterVaporConductance, PM22emb$Profit, lty=2, lwd=1, col=col2)
points(psi2A22emb$WaterVaporConductance[PM22emb$iMaxProfit+1],
         PM22emb$Profit[PM22emb$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A23emb$WaterVaporConductance, PM23emb$Profit, lty=2, lwd=1, col=col3)
points(psi2A23emb$WaterVaporConductance[PM23emb$iMaxProfit+1],
         PM23emb$Profit[PM23emb$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A24emb$WaterVaporConductance, PM24emb$Profit, lty=2, lwd=1, col=col4)
points(psi2A24emb$WaterVaporConductance[PM24emb$iMaxProfit+1],
         PM24emb$Profit[PM24emb$iMaxProfit+1], col=col4, pch = 0)
lines(psi2A31emb$WaterVaporConductance, PM31emb$Profit, lty=3, lwd=1, col=col1)
points(psi2A31emb$WaterVaporConductance[PM31emb$iMaxProfit+1],
         PM31emb$Profit[PM31emb$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A32emb$WaterVaporConductance, PM32emb$Profit, lty=3, lwd=1, col=col2)
points(psi2A32emb$WaterVaporConductance[PM32emb$iMaxProfit+1],
         PM32emb$Profit[PM32emb$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A33emb$WaterVaporConductance, PM33emb$Profit, lty=3, lwd=1, col=col3)
points(psi2A33emb$WaterVaporConductance[PM33emb$iMaxProfit+1],
         PM33emb$Profit[PM33emb$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A34emb$WaterVaporConductance, PM34emb$Profit, lty=3, lwd=1, col=col4)
points(psi2A34emb$WaterVaporConductance[PM34emb$iMaxProfit+1],
         PM34emb$Profit[PM34emb$iMaxProfit+1], col=col4, pch = 0)
legend("topright", bty="n", legend=textures, lwd=1, lty=1:3, cex=0.8)
abline(h=0,col="gray")
abline(v=Gmin, col="gray", lwd=1.5)
...
plot(psi2A11$WaterVaporConductance, PM11_s2$Profit, type="l", col=col1, ylab="Profit (a = 0.5)", xlab = "Stomatal conductance",  xlim=c(0.001, 0.5), log="x", ylim=c(0,1), lwd=1, main="original")
points(psi2A11$WaterVaporConductance[PM11_s2$iMaxProfit+1],
         PM11_s2$Profit[PM11_s2$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A12$WaterVaporConductance, PM12_s2$Profit, lty=1, lwd=1, col=col2)
points(psi2A12$WaterVaporConductance[PM12_s2$iMaxProfit+1],
         PM12_s2$Profit[PM12_s2$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A13$WaterVaporConductance, PM13_s2$Profit, lty=1, lwd=1, col=col3)
points(psi2A13$WaterVaporConductance[PM13_s2$iMaxProfit+1],
         PM13_s2$Profit[PM13_s2$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A14$WaterVaporConductance, PM14_s2$Profit, lty=1, lwd=1, col=col4)
points(psi2A14$WaterVaporConductance[PM14_s2$iMaxProfit+1],
         PM14_s2$Profit[PM14_s2$iMaxProfit+1], col=col4, pch = 0)
lines(psi2A21$WaterVaporConductance, PM21_s2$Profit, lty=2, lwd=1, col=col1)
points(psi2A21$WaterVaporConductance[PM21_s2$iMaxProfit+1],
         PM21_s2$Profit[PM21_s2$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A22$WaterVaporConductance, PM22_s2$Profit, lty=2, lwd=1, col=col2)
points(psi2A22$WaterVaporConductance[PM22_s2$iMaxProfit+1],
         PM22_s2$Profit[PM22_s2$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A23$WaterVaporConductance, PM23_s2$Profit, lty=2, lwd=1, col=col3)
points(psi2A23$WaterVaporConductance[PM23_s2$iMaxProfit+1],
         PM23_s2$Profit[PM23_s2$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A24$WaterVaporConductance, PM24_s2$Profit, lty=2, lwd=1, col=col4)
points(psi2A24$WaterVaporConductance[PM24_s2$iMaxProfit+1],
         PM24_s2$Profit[PM24_s2$iMaxProfit+1], col=col4, pch = 0)
lines(psi2A31$WaterVaporConductance, PM31_s2$Profit, lty=3, lwd=1, col=col1)
points(psi2A31$WaterVaporConductance[PM31_s2$iMaxProfit+1],
         PM31_s2$Profit[PM31_s2$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A32$WaterVaporConductance, PM32_s2$Profit, lty=3, lwd=1, col=col2)
points(psi2A32$WaterVaporConductance[PM32_s2$iMaxProfit+1],
         PM32_s2$Profit[PM32_s2$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A33$WaterVaporConductance, PM33_s2$Profit, lty=3, lwd=1, col=col3)
points(psi2A33$WaterVaporConductance[PM33_s2$iMaxProfit+1],
         PM33_s2$Profit[PM33_s2$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A34$WaterVaporConductance, PM34_s2$Profit, lty=3, lwd=1, col=col4)
points(psi2A34$WaterVaporConductance[PM34_s2$iMaxProfit+1],
         PM34_s2$Profit[PM34_s2$iMaxProfit+1], col=col4, pch = 0)
legend("topright", bty="n", legend=textures, lwd=1, lty=1:3, cex=0.8)
abline(h=0,col="gray")
abline(v=Gmin, col="gray", lwd=1.5)
...
plot(psi2A11emb$WaterVaporConductance, PM11emb_s2$Profit, type="l", col=col1, ylab="Profit (a = 2)", xlab = "Stomatal conductance",  xlim=c(0.001, 0.5), log="x", ylim=c(0,1), lwd=1, main="after cavitation")
points(psi2A11emb$WaterVaporConductance[PM11emb_s2$iMaxProfit+1],
         PM11emb_s2$Profit[PM11emb_s2$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A12emb$WaterVaporConductance, PM12emb_s2$Profit, lty=1, lwd=1, col=col2)
points(psi2A12emb$WaterVaporConductance[PM12emb_s2$iMaxProfit+1],
         PM12emb_s2$Profit[PM12emb_s2$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A13emb$WaterVaporConductance, PM13emb_s2$Profit, lty=1, lwd=1, col=col3)
points(psi2A13emb$WaterVaporConductance[PM13emb_s2$iMaxProfit+1],
         PM13emb_s2$Profit[PM13emb_s2$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A14emb$WaterVaporConductance, PM14emb_s2$Profit, lty=1, lwd=1, col=col4)
points(psi2A14emb$WaterVaporConductance[PM14emb_s2$iMaxProfit+1],
         PM14emb_s2$Profit[PM14emb_s2$iMaxProfit+1], col=col4, pch = 0)
lines(psi2A21emb$WaterVaporConductance, PM21emb_s2$Profit, lty=2, lwd=1, col=col1)
points(psi2A21emb$WaterVaporConductance[PM21emb_s2$iMaxProfit+1],
         PM21emb_s2$Profit[PM21emb_s2$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A22emb$WaterVaporConductance, PM22emb_s2$Profit, lty=2, lwd=1, col=col2)
points(psi2A22emb$WaterVaporConductance[PM22emb_s2$iMaxProfit+1],
         PM22emb_s2$Profit[PM22emb_s2$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A23emb$WaterVaporConductance, PM23emb_s2$Profit, lty=2, lwd=1, col=col3)
points(psi2A23emb$WaterVaporConductance[PM23emb_s2$iMaxProfit+1],
         PM23emb_s2$Profit[PM23emb_s2$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A24emb$WaterVaporConductance, PM24emb_s2$Profit, lty=2, lwd=1, col=col4)
points(psi2A24emb$WaterVaporConductance[PM24emb_s2$iMaxProfit+1],
         PM24emb_s2$Profit[PM24emb_s2$iMaxProfit+1], col=col4, pch = 0)
lines(psi2A31emb$WaterVaporConductance, PM31emb_s2$Profit, lty=3, lwd=1, col=col1)
points(psi2A31emb$WaterVaporConductance[PM31emb_s2$iMaxProfit+1],
         PM31emb_s2$Profit[PM31emb_s2$iMaxProfit+1], col=col1, pch = 0)
lines(psi2A32emb$WaterVaporConductance, PM32emb_s2$Profit, lty=3, lwd=1, col=col2)
points(psi2A32emb$WaterVaporConductance[PM32emb_s2$iMaxProfit+1],
         PM32emb_s2$Profit[PM32emb_s2$iMaxProfit+1], col=col2, pch = 0)
lines(psi2A33emb$WaterVaporConductance, PM33emb_s2$Profit, lty=3, lwd=1, col=col3)
points(psi2A33emb$WaterVaporConductance[PM33emb_s2$iMaxProfit+1],
         PM33emb_s2$Profit[PM33emb_s2$iMaxProfit+1], col=col3, pch = 0)
lines(psi2A34emb$WaterVaporConductance, PM34emb_s2$Profit, lty=3, lwd=1, col=col4)
points(psi2A34emb$WaterVaporConductance[PM34emb_s2$iMaxProfit+1],
         PM34emb_s2$Profit[PM34emb_s2$iMaxProfit+1], col=col4, pch = 0)
legend("topright", bty="n", legend=textures, lwd=1, lty=1:3, cex=0.8)
abline(h=0,col="gray")
abline(v=Gmin, col="gray", lwd=1.5)
...
\begin{equation}
Ex_{i,s,t} = E_{i,s} \cdot LAI_i^{\phi} \cdot 10^{-3} \cdot 0.01802 \cdot \Delta t
\end{equation}
