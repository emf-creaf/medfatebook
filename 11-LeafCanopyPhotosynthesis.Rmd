...
Diffusive conductance to water vapor ($g_{w}$; in $mol\, H_2O \cdot s^{-1} \cdot m^{-2}$) and to carbon dioxide ($g_{c}$; in $mol\,CO_{2} \cdot s^{-1} \cdot m^{-2}$) are obtained for each value of transpiration flow $E$ (here expressed in $mol\, H_2O \cdot s^{-1} \cdot m^{-2}$) and $VPD_{leaf}$ using:
\begin{eqnarray}
g_{w} &=& E \cdot \frac{P_{atm}}{VPD_{leaf}}\\
g_{c} &=& g_{w}/1.6
\end{eqnarray}
Diffusive conductance to water vapor is assumed to be result of stomatal ($g_{sw}$), leaf boundary layer ($g_{bound}$) and canopy ($g_{canopy}$) conductances (all in $mol\, H_2O \cdot s^{-1} \cdot m^{-2}$). If we estimate $g_{bound}$ using:
\begin{equation}
g_{bound} = 0.397 \cdot (u_{leaf}/d)^{0.5}
\end{equation}
and $g_{canopy}$ as:
\begin{equation}
g_{canopy} = g_{canopy,0} \cdot u_{leaf}^{0.6}
\end{equation}
where $g_{canopy,0}$ is the reference canopy conductance given by control parameter `gCrown0`. Since diffusive resistance is the sum of all resistances, stomatal conductance is determined using:
\begin{equation}
g_{sw}^{-1} = g_{w}^{-1} - g_{bound}^{-1}  - g_{canopy}^{-1}  
\end{equation}
Ensuring here that $g_{sw} \geq 0.0$. The application of equations for $g_{w}$, $g_{bound}$, $g_{canopy}$ and $g_{sw}$ to the $VPD_{leaf}(\Psi_{leaf})$ curves yields the following stomatal conductance $g_{sw}(\Psi_{leaf})$ curves:
