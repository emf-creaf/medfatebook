...
As mentioned above, **medfate** uses a hydraulic network of $(S \times 2 + N + 1)$ resistance elements to represent the soil-plant continuum, with soil being represented in $S$ different layers. As before, the $S$ soil layers are in parallel up to the root crown and each soil layer requires at least a rhizosphere and a root segment. From the root crown there are $N$ stem xylem elements (normally $N = 1$) in series and a final leaf element. The whole hydraulic network is illustrated in the figure below.
