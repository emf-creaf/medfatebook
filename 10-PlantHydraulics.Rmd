...
To consider plant water storage compartments **medfate** uses two supply functions (black and red arrows in figure below): 
 ...
 1. The first supply function is made of ($S×2+1$) elements and draws water from the soil, pouring it into the stem xylem. This supply function is used to determine soil water uptake. 
 2. The second is made of ($1+2+1$) elements (representing plant resistances) and is used to determine the rate of water flowing from stem up to the leaves for transpiration and photosynthesis, as in the model without water comparments. 
 
This division of water input and output from the stem allows considering the intermediate node, $\Psi_{stem,1}$, as the water potential of an explicit xylem water compartment from which water is drawn towards the leaves and where soil water is poured. Following @Holtta2009, the xylem water compartment can be a source of water for transpiration when cavitation occurs. Following @Steppe2006, two symplasmic water reserve compartments are also considered, which can also provide water for transpiration. The xylem compartment is hydraulically connected with a stem symplastic compartment (whose moisture level is represented by the state variable $\Psi_{symp, stem}$) and another symplasmic compartment is considered at the leaf level (whose moisture level is represented by $\Psi_{symp, leaf}$) that adds or withdraws water from the transpirational stream depending on differences between leaf xylem and symplastic water potentials. As the supply function #2 assumes a steady state flow from the stem to the leaves (and atmosphere) the leaf symplastic water compartment is used to modulate how much of the steady-state flow will actually be drawn from the stem xylem compartment. In other words, the leaf lateral flow simply modifies the water demand from the leaves to the stem.
...
The larger these three compartments in volume, the more water the plant will be able to transpire without needing a replacement of water from the soil. In case of edaphic drought and very much reduced soil-root conductance, the plant can be disconnected hydraulically from the soil and still loose small amounts of water (through cuticular transpiration) from these compartments.


### Constitutive equations

The way the previous architecture is implemented is the following. Two supply functions are needed, one from soil nodes to $\Psi_{stem,1}$, and the other from $\Psi_{rootsurf}$ to $\Psi_{leaf}$. The first supply function defines an input instantaneous (steady-state) flow to the stem compartment, $E_{stem, in}$, whereas the second supply function defines an outflow from the stem, i.e. $E_{stem, out}$. The second (plant) supply function needs to be build from $\Psi_{rootsurf}$ to $\Psi_{leaf}$ (and not from $\Psi_{stem,1}$ to $\Psi_{leaf}$) because all plant resistances are needed to determine the relationship between $E_{stem, out}$ and $\Psi_{leaf}$.

Two instantaneous water flows, $F_{lat, stem}$, and $F_{lat, leaf}$ are defined:
\begin{eqnarray}
F_{lat,stem} &=& k_{lat,stem} \cdot (\Psi_{symp,stem} - \Psi_{stem, apo}) \\
F_{lat,leaf} &=& k_{lat,leaf} \cdot (\Psi_{symp,leaf} - \Psi_{leaf}) \\
\end{eqnarray}
where $k_{lat,stem}$ and $k_{lat,leaf}$ are the lateral (between symplastic tissues and xylem) water conductance for stem and leaves, respectively. These conductance values are defined as control parameters. The four instantaneous flows define a system of equations with the following volume changes:
\begin{eqnarray}
