...
The advanced water balance model of **medfate** implements steady-state hydraulic supply functions as described in @Sperry2015. The hydraulic network is described using $(N \times 2 + S + 1)$ resistance elements, with soil being represented in $N$ different soil layers and the stem by $S$ different stem segments. For each soil layer there is a rhizosphere element in series with a root xylem element. The $N$ soil layers are in parallel up to the root crown. From there there are $S$ stem xylem segments and a final leaf segment, all in series. 
