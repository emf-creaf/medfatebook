...
We devote this chapter to describe the background necessary to understand how plant hydraulics is implemented in the advanced water balance model of **medfate**. Section \@ref(hydraulicnetworks) introduces the hydraulic networks from which to choose, depending on the sub-model of plant hydraulics employed (i.e. either `transpirationMode = "Sperry"` or `transpirationMode = "Cochard"`). After that, in sections \@ref(vulnerabilitycurves) and  \@ref(plantwatercontent) we present the building blocks of models of plant hydraulics, i.e. how to represent in a model the water content of plant tissues and the water flow through vascular elements. In sections \@ref(supplyfunctions) and \@ref(hydraulicswithcompartments) we describe in detail the plant hydraulics sub-models presented in @DeCaceres2021. Finally, in  \@ref(hydraulicscochard) we describe the sub-model of plant hydraulics that was presented in @ruffault_sureau-ecos_2022 and was added to **medfate**.


## Hydraulic networks {#hydraulicnetworks}

The advanced water balance model of **medfate** implements different hydraulic networks depending on the sub-model of plant hydraulics:

+ If `transpirationMode = "Sperry"` and `capacitance = FALSE`, the hydraulic network is described using $(S \times 2 + 2 + 1)$ resistance elements, with soil being represented in $S$ different soil layers. For each soil layer there is a rhizosphere element in series with a root xylem element. The $S$ soil layers are in parallel up to the root crown. From there there are two stem xylem segments and a final leaf segment, all in series (Fig. \@ref(fig:hydraulicsnetwork)). 
...
+ If `transpirationMode = "Sperry"` and `capacitance = "TRUE"` To consider plant water storage compartments explicitly these need to be combined with supply functions in some way. The previous hydraulic network is divided into two parts: (1) the first is made of $(S \times 2 + 1)$ elements and draws water from the soil, pouring it into the stem xylem; and (2) the second is made of $(1+1)$ elements and draws water from stem xylem bringing it up to the leaves. Each part has its own supply function and the stem xylem segments exchange water with water storage compartments (Fig. \@ref(fig:hydraulicsnetworkcompartments)).


```{r hydraulicsnetworkcompartments, out.width='80%', fig.align="center", fig.cap="Schematic representation of hydraulics in a whole-plant network with water storage compartments", echo=FALSE}
knitr::include_graphics("figures/hydraulics_compartments.jpg")
```


+ If `transpirationMode = "Cochard"`, ... [TO BE WRITTEN]
...
\begin{equation}
PLC_{stem}(\Psi_{cav}) = 1.0 - \frac{k_{stem}(\Psi_{cav})}{k_{stem,max}} = 1.0 - e^{-((\Psi_{cav}/{d_{stem}})^{c_{stem}})}
\end{equation}
...
par(mar=c(4,4,2,1), mfrow=c(1,2))
psiMax = -7.0
hn11 <- list("psisoil" = psiSoilLayers1,
             "krhizomax" = krhizomaxvec1,"nsoil" = rep(n1,3), "alphasoil" = rep(alpha1,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS11 = hydraulics_supplyFunctionBelowground(hn11,maxNsteps = 400)
hn12 <- list("psisoil" = psiSoilLayers2,
             "krhizomax" = krhizomaxvec1,"nsoil" = rep(n1,3), "alphasoil" = rep(alpha1,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS12 = hydraulics_supplyFunctionBelowground(hn12, maxNsteps = 400)
hn13 <- list("psisoil" = psiSoilLayers3,
             "krhizomax" = krhizomaxvec1,"nsoil" = rep(n1,3), "alphasoil" = rep(alpha1,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS13 = hydraulics_supplyFunctionBelowground(hn13, maxNsteps = 400)
hn14 <- list("psisoil" = psiSoilLayers4,
             "krhizomax" = krhizomaxvec1,"nsoil" = rep(n1,3), "alphasoil" = rep(alpha1,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS14 = hydraulics_supplyFunctionBelowground(hn14, maxNsteps = 400)
hn21 <- list("psisoil" = psiSoilLayers1,
             "krhizomax" = krhizomaxvec2,"nsoil" = rep(n2,3), "alphasoil" = rep(alpha2,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS21 = hydraulics_supplyFunctionBelowground(hn21,maxNsteps = 400)
hn22 <- list("psisoil" = psiSoilLayers2,
             "krhizomax" = krhizomaxvec2,"nsoil" = rep(n2,3), "alphasoil" = rep(alpha2,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS22 = hydraulics_supplyFunctionBelowground(hn22,maxNsteps = 400)
hn23 <- list("psisoil" = psiSoilLayers3,
             "krhizomax" = krhizomaxvec2,"nsoil" = rep(n2,3), "alphasoil" = rep(alpha2,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS23 = hydraulics_supplyFunctionBelowground(hn23, maxNsteps = 400)
hn24 <- list("psisoil" = psiSoilLayers4,
             "krhizomax" = krhizomaxvec2,"nsoil" = rep(n2,3), "alphasoil" = rep(alpha2,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS24 = hydraulics_supplyFunctionBelowground(hn24, maxNsteps = 400)
hn31 <- list("psisoil" = psiSoilLayers1,
             "krhizomax" = krhizomaxvec3,"nsoil" = rep(n3,3), "alphasoil" = rep(alpha3,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS31 = hydraulics_supplyFunctionBelowground(hn31, maxNsteps = 400)
hn32 <- list("psisoil" = psiSoilLayers2,
             "krhizomax" = krhizomaxvec3,"nsoil" = rep(n3,3), "alphasoil" = rep(alpha3,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS32 = hydraulics_supplyFunctionBelowground(hn32, maxNsteps = 400)
hn33 <- list("psisoil" = psiSoilLayers3,
             "krhizomax" = krhizomaxvec3,"nsoil" = rep(n3,3), "alphasoil" = rep(alpha3,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS33 = hydraulics_supplyFunctionBelowground(hn33, maxNsteps = 400)
hn34 <- list("psisoil" = psiSoilLayers4,
             "krhizomax" = krhizomaxvec3,"nsoil" = rep(n3,3), "alphasoil" = rep(alpha3,3),
             "krootmax" = krootmaxvec,"rootc" = rootc, "rootd" = rootd)
supplyRS34 = hydraulics_supplyFunctionBelowground(hn34, maxNsteps = 400)
...
hn11 <- list("psisoil" = psiSoilLayers1,
             ...
             "PLCstem" =0.0)
supplyNetwork11 = hydraulics_supplyFunctionNetwork(hn11, maxNsteps = 400)
hn12 <- hn11
hn12$psisoil <- psiSoilLayers2
supplyNetwork12 = hydraulics_supplyFunctionNetwork(hn12, maxNsteps = 400)
hn13 <- hn11
hn13$psisoil <- psiSoilLayers3
supplyNetwork13 = hydraulics_supplyFunctionNetwork(hn13, maxNsteps = 400)
hn14 <- hn11
hn14$psisoil <- psiSoilLayers4
supplyNetwork14 = hydraulics_supplyFunctionNetwork(hn14, maxNsteps = 400)
hn21 <- list("psisoil" = psiSoilLayers1,
             "krhizomax" = krhizomaxvec2, "nsoil" = rep(n2,3), "alphasoil" = rep(alpha2,3),
             "krootmax" = krootmaxvec, "rootc" = rootc, "rootd" = rootd,
             "kstemmax" = kstemmax, "stemc" = stemc, "stemd" = stemd,
             "kleafmax" = kleafmax, "leafc" = leafc, "leafd" = leafd,
             "PLCstem" =0.0)
supplyNetwork21 = hydraulics_supplyFunctionNetwork(hn21, maxNsteps = 400)
hn22 <- hn21
hn22$psisoil <- psiSoilLayers2
supplyNetwork22 = hydraulics_supplyFunctionNetwork(hn22, maxNsteps = 400)
hn23 <- hn21
hn23$psisoil <- psiSoilLayers3
supplyNetwork23 = hydraulics_supplyFunctionNetwork(hn23, maxNsteps = 400)
hn24 <- hn21
hn24$psisoil <- psiSoilLayers4
supplyNetwork24 = hydraulics_supplyFunctionNetwork(hn24, maxNsteps = 400)
hn31 <- list("psisoil" = psiSoilLayers1,
             "krhizomax" = krhizomaxvec3, "nsoil" = rep(n3,3), "alphasoil" = rep(alpha3,3),
             "krootmax" = krootmaxvec, "rootc" = rootc, "rootd" = rootd,
             "kstemmax" = kstemmax, "stemc" = stemc, "stemd" = stemd,
             "kleafmax" = kleafmax, "leafc" = leafc, "leafd" = leafd,
             "PLCstem" =0.0)
supplyNetwork31 = hydraulics_supplyFunctionNetwork(hn31, maxNsteps = 400)
hn32 <- hn31
hn32$psisoil <- psiSoilLayers2
supplyNetwork32 = hydraulics_supplyFunctionNetwork(hn32, maxNsteps = 400)
hn33 <- hn31
hn33$psisoil <- psiSoilLayers3
supplyNetwork33 = hydraulics_supplyFunctionNetwork(hn33, maxNsteps = 400)
hn34 <- hn31
hn34$psisoil <- psiSoilLayers4
supplyNetwork34 = hydraulics_supplyFunctionNetwork(hn34, maxNsteps = 400)
...
supplyNetwork11emb = hydraulics_supplyFunctionNetwork(hn11emb, maxNsteps = 400)
hn12emb <- hn12
hn12emb$PLCstem <- plcCav
supplyNetwork12emb = hydraulics_supplyFunctionNetwork(hn12emb, maxNsteps = 400)
hn13emb <- hn13
hn13emb$PLCstem <- plcCav
supplyNetwork13emb = hydraulics_supplyFunctionNetwork(hn13emb, maxNsteps = 400)
hn14emb <- hn14
hn14emb$PLCstem <- plcCav
supplyNetwork14emb = hydraulics_supplyFunctionNetwork(hn14emb, maxNsteps = 400)
hn21emb <- hn21
hn21emb$PLCstem <- plcCav
supplyNetwork21emb = hydraulics_supplyFunctionNetwork(hn21emb, maxNsteps = 400)
hn22emb <- hn22
hn22emb$PLCstem <- plcCav
supplyNetwork22emb = hydraulics_supplyFunctionNetwork(hn22emb, maxNsteps = 400)
hn23emb <- hn23
hn23emb$PLCstem <- plcCav
supplyNetwork23emb = hydraulics_supplyFunctionNetwork(hn23emb, maxNsteps = 400)
hn24emb <- hn24
hn24emb$PLCstem <- plcCav
supplyNetwork24emb = hydraulics_supplyFunctionNetwork(hn24emb, maxNsteps = 400)
hn31emb <- hn31
hn31emb$PLCstem <- plcCav
supplyNetwork31emb = hydraulics_supplyFunctionNetwork(hn31emb, maxNsteps = 400)
hn32emb <- hn32
hn32emb$PLCstem <- plcCav
supplyNetwork32emb = hydraulics_supplyFunctionNetwork(hn32emb, maxNsteps = 400)
hn33emb <- hn33
hn33emb$PLCstem <- plcCav
supplyNetwork33emb = hydraulics_supplyFunctionNetwork(hn33emb, maxNsteps = 400)
hn34emb <- hn34
hn34emb$PLCstem <- plcCav
supplyNetwork34emb = hydraulics_supplyFunctionNetwork(hn34emb, maxNsteps = 400)
...
## Sperry's sub-model with water compartments {#hydraulicswithcompartments}

In **medfate** the relative water content of leaves and stems is tracked explicitly. When capacitance effects are not considered, the relative water content of both kinds of plant tissues follow the water potential changes dictated by supply functions and stomatal regulation. In contrast, when plant capacitance effects are considered, leaf and stem symplastic tissues have their own water potential as state variable, they exchange water with leaf and stem xylem, respectively.
...
### Constitutive equations  {#constitutiveequations}
...
\begin{equation}
\frac{\delta m_{stem}} {\delta t} = E_{stem, in} - E_{stem, out} + F_{lat, stem}
\end{equation}
