...
| Symbol | Units | R param | Description                        |
|--------|-------|---------|------------------------------------|
| $k_{PAR,i}$ | | `kPAR` | PAR extinction coefficient for cohort $i$ |
| $k_{SWR,i}$ | |  | SWR extinction coefficient for cohort $i$ |
| $k_{b}$ | | | Extinction coefficient for direct light  for cohort $i$|
| $k_{d,i}$ | | | Extinction coefficient for diffuse light for cohort $i$ (equal to $k_{PAR,i}$ or $k_{SWR,i}$) |
| $\alpha_{SWR,i}$ | | `alphaSWR` | Short-wave radiation absorbance coefficient  for cohort $i$|
| $\alpha_{PAR,i}$ | | | PAR absorbance coefficient  for cohort $i$|
| $\gamma_{SWR,i}$ | |`gammaSWR` | Short-wave radiation leaf reflectance (albedo) for cohort $i$ |
...
| $LE_{ca}$ | $W$ | `LEcan`| Energy released as latent heat from the canopy towards the atmosphere |
| $LE_{sa}$ | $W$ | `LEsoil` | Energy released as latent heat from the soil towards the atmosphere |
...
| Symbol | Units | R param | Description                        |
|--------|-------|---------|------------------------------------|
| $\theta_1$, $\theta_2$  | [0-1] | | Cost functions 1 and 2 |
| $\beta$  | [0-1] | | Gain function. |
| $Profit_1$, $Profit_2$  | [0-1] | | Profit functions 1 and 2 |
