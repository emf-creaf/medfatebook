...
Tree recruitment density, diameter and height are determined by parameters $N_{tree,recr}$, $DBH_{tree, recr}$  and $H_{tree, recr}$, respectively; whereas cover and height of shrub recruitment is determined by parameters `Cover_{recr}`  and $H_{shrub, recr}$, respectively. If stochastic simulation of recruitment is requested, then density or cover values are considered mean values of a Poisson distribution.

### Resprouting

Currently, resprouting only occurs in the model if plant cohorts have been cut or they died of desiccation (i.e. resprouting does not occur for baseline mortality or after plant starvation). The model first determines the density of tree resprouts originated after desiccation of above-ground parts, multiplying the density of desiccated trees by the value of $Resp_{dist}$ of the corresponding species, which should only be non-zero in the case the species has resprouting capacity. If management occurred, the model then estimates the density of resprouts originated after trees were cut, by multiplying the density cut trees by the value of $Resp_{clip}$ of the corresponding species. Tree resprouts have all a starting diameter $DBH_{tree, recr}$ and a height $H_{tree, recr}$, but the root system is the same of the *parent* tree cohort. In the case of shrub species, the same operations are done, but on cover values instead of density values. Shrub height of resprouts is assumed to be $H_{shrub, recr}$. Resprouting after fire is not yet implemented.
