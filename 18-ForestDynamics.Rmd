...
The `fordyn()` model divides the period to be simulated in years, which is the top-level time step of simulations. Given an input `forest` object, the function first initializes the input for function `growth()`. For each year to be simulated the model the performs the following steps:
 ...
 1. Calls function `growth()` to simulate daily water/carbon balance, growth and mortality processes (sub-daily processes may also be involved in `transpirationMode = "Sperry"`). See section \@ref(growthscheduling) for details of growth scheduling.
 ...
 3. Removes plant cohorts whose remaining density (ind. per hectare) is lower than a specified threshold (see control parameter `minimumCohortDensity`).
 4. If required, simulate recruitment (for species present in the stand or specified via control parameter `seedRain`).
...
+ `applyRecruitment [= TRUE]`: Boolean flag to indicate that recruitment is allowed.
