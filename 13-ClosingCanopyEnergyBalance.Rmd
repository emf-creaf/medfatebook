...
\begin{equation}
LE_{can} = \lambda_v(T_{can}) \cdot \left(\frac{In_{t} + \sum_{i=1}^{c}{Tr_{i,t}}}{\Delta_{step}} \right)
(\#eq:canopylatentheat)
\end{equation}
...
\begin{equation}
LE_{j} = \lambda_v(T_{air,j}) \cdot \left(\frac{In_{j,t} + \sum_{i=1}^{c}{Tr_{i,j,t}}}{\Delta t} \right)
\end{equation}
where $\lambda_v(T_{air,j})$ is the latent heat of vaporization at temperature $T_{air,j}$ (in $J·kg^{-1}$), $In_{j,t}$ is the intercepted water that is evaporated from the canopy layer and $Tr_{i,j,t}$ is the transpiration for cohort $i$ in layer $j$. $In_{j,t}$ is determined by dividing $In_{t}$ among canopy layers according to the fraction of total LAI in each one:
\begin{equation}
In_{j,t} = In_{t} \cdot \frac{\sum_{i=1}{LAI_{i,j}^{\phi}}}{LAI^{\phi}_{stand}}
\end{equation}
Similarly, $Tr_{i,j,t}$ is determined by dividing the transpiration of cohort $i$, $Tr_{i,t}$, among the canopy layers according to the proportion cohort's LAI in each one:
\begin{equation}
Tr_{i, j,t} = Tr_{i,t} \cdot \frac{LAI_{i,j}^{\phi}}{LAI_{i}^{\phi}}
...
\end{equation}
where $(\delta F_{[CO_2]}/\delta z)_j$ and $(\delta F_{[H_2O]}/\delta z)_j$ are the gradient of vertical flux of $CO_2$ or $H_2O$ in layer $j$, respectively; $S_{[H_2O],j}$ and $S_{[CO_2],j}$ are the source strengths for $[H_2O]$ (in $kg\cdot m^{-3} \cdot s^{-1}$) and $[CO_2]$ (in $mg \cdot m^{-3} \cdot s^{-1}$) in layer $j$, respectively, which derive from transpiration and photosynthesis:
\begin{eqnarray}
S_{[H_2O],j} &=&  \frac{In_{j,t} + \sum_{i=1}^{c}{Tr_{i,j,t}}}{\Delta t \cdot \Delta z} \\
S_{[CO_2],j} &=& - \frac{1000\cdot (44.01/12.0) \cdot \sum_{i=1}^c{A_{n,i,j,t}}}{\Delta t \cdot \Delta z}
\end{eqnarray}
where $Tr_{i,j,t}$ and $A_{n,i,j,t}$ are the transpiration and photosynthesis for cohort $i$ in layer $j$. The former is defined in eq. \@ref(eq:cohortlayertranspiration) whereas the latter is defined analogously:
\begin{equation}
A_{n,i, j,t} = A_{n,i,t} \cdot \frac{LAI_{i,j}^{\phi}}{LAI_{i}^{\phi}}
\end{equation}
being $A_{n,i,t}$ the amount of photosynthesis ($gC\cdot m^{-2}$) of cohort $i$ during time step $t$ \@ref(eq:cohortnetphotosynthesis).
