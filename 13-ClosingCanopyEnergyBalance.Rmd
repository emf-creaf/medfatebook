...
\end{eqnarray}
where $Tr_{i,j,t}$ and $A_{n,i,j,t}$ are the transpiration and photosynthesis for cohort $i$ in layer $j$. The former was already defined in eq. \@ref(eq:cohortlayertranspiration), whereas the latter is defined analogously:
\begin{equation}
A_{n,i, j,t} = A_{n,i,t} \cdot \frac{LAI_{i,j}^{\phi}}{LAI_{i}^{\phi}}
\end{equation}
being $A_{n,i,t}$ the amount of photosynthesis ($gC\cdot m^{-2}$) of cohort $i$ during time step $t$ (see \@ref(eq:cohortnetphotosynthesis)).
