...
\begin{equation}
\Delta T_{can} =  \Delta t_{step} \cdot \frac{K_{abs,can} + L_{net,ca} - LE_{can} - H_{ca} - H_{cs}}{TC_{can}}
\end{equation}
...
\begin{equation}
G_{0,1} =  K_{soil} + L_{net,soil} + H_{cs} - LE_{soil}
\end{equation}
...
### Latent heat
In a multi-layer canopy energy balance, the latent heat for a given layer $j$ is:
\begin{equation}
