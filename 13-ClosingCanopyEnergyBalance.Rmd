...
\begin{equation}
TC_{can} = TC_{LAI} \cdot \frac{0.8 \cdot LAI_{live} + 1.2 \cdot (LAI_{\phi} + \cdot LAI_{dead})}{2}
(\#eq:canopythermalcapacitance)
\end{equation}
where $TC_{LAI}$ is the thermal capacitance per LAI unit, which is specified by the control parameter `thermalCapacityLAI`. By using both the maximum leaf area of the stand, $LAI_{live}$, and its current live/dead leaf area ($LAI_{\phi}+LAI_{dead}$) it is assumed that part of thermal capacitance corresponds to stems and branches, so that capacitance does not drop to zero for deciduous canopies.
...
\begin{equation}
In_{j,t} = In_{t} \cdot \frac{\sum_{i=1}{LAI_{\phi,i,j}}}{LAI_{\phi}}
\end{equation}
and $E_{j,t}$ is the instantaneous transpiration rate in layer $j$ (in $kg\,H_2O\cdot m^{-2}\cdot s^{-1}$), given by:
\begin{equation}
E_{j,t} = 10^{-3} \cdot 0.01802\cdot \sum_{i=1}^{c}{E_{sunlit,i,t}\cdot LAI_{sunlit,i,j} + E_{shade,i,t}\cdot LAI_{shade,i,j}} 
(\#eq:cohortlayertranspiration)
\end{equation}
where $E_{sunlit, i,t}$ and $E_{sunlit,i,t}$ are the instantaneous transpiration rates (in $mmol\,H_2O\cdot m^{-2}\cdot s^{-1}$) of sunlit and shade leaves, respectively, of cohort $i$ during time step $t$; and $LAI_{sunlit,i,j}$ and $LAI_{shade,i,j}$ are the leaf area index of sunlit and shade leaves, respectively, of cohort $i$ in layer $j$ \@ref(eq:sunlitshadelai).
...
\begin{equation}
H_{leaf,j} = \sum_{i=1}^c { LAI_{sunlit,i,j} \cdot H_{sunlit,i,j}  + LAI_{shade,i,j} \cdot H_{shade,i,j}}
\end{equation}
where $LAI_{sunlit,i,j}$ and $LAI_{shade,i,j}$ are the leaf area index of sunlit and shade leaves of plant cohort $i$ in layer $j$, respectively (see eq. \@ref(eq:sunlitshadelai)). $H_{sunlit,i,j}$ and $H_{shade,i,j}$ are the sensible heat exchange between layer $j$ and sunlit or shade leaves of cohort $i$ [@Ma2019]:
\begin{eqnarray}
H_{sunlit,i,j} &=& -2 \cdot C_p \cdot (T_{leaf,sunlit,i} - T_{air,j})\cdot g_{Ha,i,j}\\
H_{shade,i,j} &=& -2\cdot C_p \cdot (T_{leaf,shade,i} - T_{air,j})\cdot g_{Ha,i,j}
\end{eqnarray}
where $T_{leaf,sunlit,i}$ and $T_{leaf,shade,i}$ are, respectively, the sunlit and shade leaf temperatures for cohort $i$ (determined using eq. \@ref(eq:leaftemperatureequation)); $T_{air,j}$ is the current air temperature in canopy layer $j$, $C_p = 29.37152\,J \cdot mol^{-1} \cdot ºC^{-1}$ is the specific heat capacity of dry air at constant pressure; and $g_{Ha,i,j}$ is the leaf boundary layer conductance for heat ($mol \cdot m^{-2} \cdot s^{-1}$) for leaves of cohort $i$ and wind speed in layer $j$ (see eq. \@ref(eq:leafboundaryconductances)).
...
\begin{equation}
TC_{j} = TC_{LAI} \cdot \frac{0.8 \cdot LAI_{live,j} + 1.2 \cdot (LAI_{\phi,j} + \cdot LAI_{dead,j})}{2}
\end{equation}
...
\begin{equation}
A_{n, j,t} = 10^{-3} \cdot 44.01 \cdot \sum_{i=1}^{c}{A_{n,sunlit,i,t}\cdot LAI_{sunlit,i,j} + A_{n,shade, i,t}\cdot LAI_{shade,i,j}}
\end{equation}
where $A_{n,sunlit,i,t}$ and $A_{n,shade,i,t}$ are the instantaneous net photosynthesis rates (in $\mu mol\, CO_2\cdot m^{-2}\cdot s^{-1}$) of sunlit and shade leaves, respectively, of cohort $i$ during time step $t$; and $LAI_{sunlit,i,j}$ and $LAI_{shade,i,j}$ are the leaf area index of sunlit and shade leaves, respectively of cohort $i$ in layer $j$ \@ref(eq:sunlitshadelai). For the special case of the lowest canopy layer, water vapor needs to include the evaporation from soil, i.e.:
\begin{equation}
