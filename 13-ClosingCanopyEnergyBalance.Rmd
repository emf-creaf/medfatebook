...
The latent heat exchange between the soil and the atmosphere comes either as vaporisation latent heat from water evaporated from the soil surface ($LE_{vap, soil}$; in $W\cdot m^{-2}$) or fusion latent heat due to snow melting ($LE_{fus, soil}$; in $W\cdot m^{-2}$). Both of them are estimated from water flows in $mm = kg \cdot m^{-2}$ using:
\begin{eqnarray}
LE_{vap, soil} &=& \frac{\lambda_v(T_{soil}) \cdot Es_{t}}{\Delta t_{step}} \\
LE_{fus, soil} &=& \frac{\lambda_{f} \cdot Sm_{t}}{\Delta t_{step}} 
(\#eq:soillatentheat)
\end{eqnarray}
...
Latent heat derived from soil water evaporation and snow melting ($LE_{vap, soil}$ and $LE_{fus, soil}$; in $W\cdot m^{-2}$) is estimated as in the single-canopy case, using eq. \@ref(eq:soillatentheat).
