...
Carbon pools and fluxes between them were described in the design section and illustrated in fig. \@ref(fig:decompositionpools). The following table provides the terminologic mapping between carbon pools in MEDFATE, which mainly follows Bonan et al. -@bonan_climate_2019, and the corresponding pools in CENTURY:
...
The first three carbon pools (marked with an asterisk) are those new to MEDFATE, in comparison with Bonan et al. -@bonan_climate_2019. In CENTURY, only leaf litter and fine root litter are those conforming structural pools, whereas dead wood (either fine branches, large wood or coarse roots) is considered separately.
...
df1 <- data.frame(pH = seq(4,8, by = 0.001), f = NA, Pool = "Surface/belowground metabolic/active")
df2 <- data.frame(pH = seq(4,8, by = 0.001), f = NA, Pool = "Surface/belowground structural/slow")
df3 <- data.frame(pH = seq(4,8, by = 0.001), f = NA, Pool = "Belowground passive")
for(i in 1:nrow(df1)) {
  df1$f[i] = decomposition_pHEffect(df1$pH[i], "SurfaceMetabolic")
  df2$f[i] = decomposition_pHEffect(df2$pH[i], "Leaves")
  df3$f[i] = decomposition_pHEffect(df3$pH[i], "BelowgroundPassive")
}
...
## Decomposition process of leaf and fine root litter, coarse woody debris and standing dead wood
