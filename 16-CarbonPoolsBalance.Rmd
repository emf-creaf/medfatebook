# Plant carbon pools and components of the labile carbon balance {#carbonpoolsbalance}

## Size of plant carbon pools {#estimationcarbonpools}

Biomass of leaves, twigs, sapwood and fine roots is needed in the model to estimate respiratory costs and the size of the C storage pools. 

### Leaf structural, dead, metabolic and storage biomass

The live leaf area $LA_{live}$ per individual (in $m^2$) is:
\begin{equation}
LA_{live} = 10000 \cdot LAI_{live} / N
(\#eq:maxleafarea)
\end{equation}
where $N$ is the density of the cohort and $LAI_{live}$ is the leaf area index. The actual (expanded) leaf area $LA_{\phi}$ is estimated analogously from $LAI_{\phi}$:
\begin{equation}
LA_{\phi} = 10000 \cdot LAI_{\phi} / N
(\#eq:leafarea)
\end{equation}
And so is the dead leaf area, $LA_{dead}$:
\begin{equation}
LA_{dead} = 10000 \cdot LAI_{dead} / N
(\#eq:deadleafarea)
\end{equation}
...
\begin{equation}
B_{leaf} = 1000 \cdot LA_{\phi} / SLA
\end{equation}
where a factor 1000 is used to convert from kg to g. Hence, only expanded leaf area has respiratory cost (i.e. winter resistance buds do not). An analogous equation is used to estimate dead leaf biomass per individual ($D_{leaf}$; $g\,dry·ind^{-1}$):
\begin{equation}
D_{leaf} = 1000 \cdot LA_{dead} / SLA
\end{equation}

Leaf storage volume for an individual ($V_{storage,leaf}$; in $L \cdot ind^{-1}$) is:
\begin{equation}
V_{storage, leaf} = LA_{\phi}  \cdot V_{leaf}
\end{equation}
...
### Twig structural and dead biomass

Structural twig biomass per individual ($B_{twig}$; $g\,dry·ind^{-1}$) is related to the biomass of leaves  ($B_{leaf}$), via the $r_{6.35}$ ratio, which is defined as the ratio between the biomass of leaves and twigs over the biomass of leaves:
\begin{equation}
B_{twig} = B_{leaf} / (r_{6.35} - 1)
\end{equation}
An analogous equation is used to estimate dead twig mass per individual ($D_{twig}$; $g\,dry·ind^{-1}$) from the mass of dead leaves ($D_{leaf}$):
\begin{equation}
D_{twig} = D_{leaf} / (r_{6.35} - 1)
\end{equation}

While $B_{twig}$ represents the structural twig biomass, it cannot be used to estimate twig respiration (see living sapwood biomass below). In addition, we assume that embolized parts of twigs do not contribute to twig respiration. Hence, twig living (respiratory) biomass is estimated using:
\begin{equation}
B_{living, twig} = B_{twig} \cdot (1 - PLC_{stem}) \cdot (1 - f_{conduits})
\end{equation}
...
\begin{equation}
B_{living, sapwood} = B_{sapwood} \cdot (1 - PLC_{stem}) \cdot (1 - f_{conduits})
\end{equation}
...
Total wood volume ($V_{wood}$; in $L \cdot ind^{-1}$) is defined, analogously as sapwood volume, as the product of stem section area and the sum of aboveground height and belowground coarse root length:
\begin{equation}
V_{wood} = 1000 \cdot SSA\cdot (H + \sum_{s}{FRP_s \cdot L_s})
\end{equation}
where $SSA$ is stem section area (in $cm^2$; derived from $DBH$), $H$ is plant height, $FRP_{s}$ is the proportion of fine roots in soil layer $s$ and $L_{s}$ is the coarse root length in layer $s$ (all lengths expressed here in $m$). Once total wood volume and sapwood volume are known, we can determine heartwood volume as their difference:
\begin{equation}
V_{heartwood} = V_{wood} - V_{sapwood}
\end{equation}
Heartwood biomass per individual ($D_{heartwood}$; $g\,dry·ind^{-1}$) represents the heartwood mass of trunks, branches and coarse roots. Analogously to sapwood biomass, is defined as the product of $V_{heartwood}$ and wood density  ($\rho_{wood}$; $g dry·cm^{-3}$):
\begin{equation}
D_{heartwood} = 1000 \cdot V_{heartwood} \cdot\rho_{wood}
\end{equation}
...
\begin{equation}
FRA = FRP_{s} \cdot LA_{\phi} \cdot RLR
\end{equation}
...
\begin{align*}
B_{total} &= B_{leaf} + B_{living, twig} + B_{living, sapwood} + B_{fineroot} + \\
...
Maintenance respiration (in $g\,gluc·ind^{-1}$) is calculated for each living compartment (leaves, twigs, sapwood, and fine roots) individually [@Mouillot2001], and differs slightly depending on the complexity of the transpiration submodel. 
...
### Growth costs and synthesis respiration

Construction costs per unit dry weight of new tissue ($CC_{leaf}$, $CC_{sapwood}$ and $CC_{fineroot}$ for leaves, sapwood and fine roots; in $g\,gluc \cdot g\,dry^{-1}$) are specified as control parameters (i.e. they are not species-specific). These unitary costs include both the carbon used in respiration during growth and structural carbon. Given an increase in leaf area $\Delta LA$, leaf carbon growth costs  (in $g\,gluc·ind^{-1}$) are:
\begin{equation}
GC_{leaf} = \Delta LA \cdot CC_{leaf} \cdot 1000 / SLA
(\#eq:leafconstructioncost)
\end{equation}
whereas the  growth costs for the corresponding twigs are:
\begin{equation}
GC_{twig} = \Delta LA \cdot CC_{sapwood} \cdot (r_{6.35} - 1) \cdot 1000 / SLA
(\#eq:twigconstructioncost)
\end{equation}
Analogously, given an increase in sapwood area $\Delta LA$, sapwood growth costs  (in $g\,gluc·ind^{-1}$) are:
\begin{equation}
GC_{sapwood} = \Delta SA \cdot CC_{sapwood} \cdot (H + \sum_{s}{FRP_s \cdot L_s}) \cdot \rho_{wood}
(\#eq:sapwoodconstructioncost)
\end{equation}
Finally, and given an increase in fine root biomass $\Delta B_{fineroot}$ has the following growth costs  (in $g\,gluc·ind^{-1}$) are:
\begin{equation}
GC_{fineroot} = \Delta B_{fineroot} \cdot CC_{fineroot}
\end{equation}

Synthesis respiration (in $g\,gluc·ind^{-1}$) associated to growth is less than growth costs, but can be found easily from growth costs, using construction costs per unit dry mass:
\begin{eqnarray}
SR_{leaf} &=& GC_{leaf} \cdot \frac{CC_{leaf} - 1}{CC_{leaf}} \\
SR_{twig} &=& GC_{twig} \cdot \frac{CC_{sapwood} - 1}{CC_{sapwood}} \\
SR_{sapwood} &=& GC_{sapwood} \cdot \frac{CC_{sapwood} - 1}{CC_{sapwood}} \\
SR_{fineroot} &=& GC_{fineroot} \cdot \frac{CC_{fineroot} - 1}{CC_{fineroot}} 
\end{eqnarray}

The plant-level total synthesis respiration (required for stand-level carbon balance) is $SR = SR_{leaf} + SR_{twig} + SR_{sapwood} + SR_{fineroot}$.
