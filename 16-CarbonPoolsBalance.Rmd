...
\begin{equation}
B = B_{leaf} + B_{sapwood} + B_{fineroot} + B_{sugar,leaf} + B_{starch,leaf}+ B_{sugar,sapwood} + B_{starch,sapwood}
\end{equation}
$B$ is used to express carbon balance components per dry weight of living biomass in the model output, which allows a better comparison across plant cohorts of different size and species.
