...
8. Distribute overland flows from cells having soil downhill (see \@ref(overlandflows)). If a cell is part of the channel network, its runoff is stored for latter processing of channel routing (see \@ref(watershedexport)). If a cell does not have downhill neighbors (i.e outlet cell) and is not in a channel, its runoff becomes water exported from the watershed (see \@ref(watershedexport)).
10. Apply vertical flows between soils and the aquifer below, i.e. deep drainage and capillarity rise, to the water balance of the aquifer.
11. Apply drainage losses from the aquifer towards a deeper aquifer not connected to watershed outlets (\@ref(deepaquiferloss)), completing eq. \@ref(eq:cellaquiferwaterbalanceequation).

Channel routing is optional, depending on whether a channel network has been supplied. Channel routing is performed after water balance simulations of the watershed (or its subwatersheds) are finished. For each day to be simulated and each channel cell, the model adds the water that reached the channel during water balance simulations to the backlog of the corresponding watershed outlet. After that the model advances one day the backlog of all network outlets to apply channel routing (see details \@ref(watershedexport)).
...
|                  | `channel_flow_speed` | Average flow speed in the channel (in m·s-1). |
...
| `watershed_soil_balance`|  Data frame with spatially-averaged components of the daily water balance over cells that have defined soils (i.e. *wildland* and *agriculture* cells). |
| `channel_export_m3s` | A matrix with daily values of runoff (in m3/s) reaching each of the channel cells of the landscape. |
| `outlet_export_m3` | A matrix with daily runoff (in m3/day) at each of the outlet cells of the landscape. Each outlet drains its own subset of cells (sometimes including channel routing), so the daily overall watershed export corresponds to the sum of row values. |
...
A special situation arises in overland runoff processing when processing cells that do not have downhill neighbors defined (i.e. where $q_{ij} = 0$ for all $j$), either because: (a) they are in a hollow surface (all neighbors have higher elevations); or (b) because they belong to a specified river channel network. In the first case (a), cells are called *outlet cells* in the model, and the runoff they generate contributes directly to watershed runoff exported ($Exp$), without any channel routing process. In the second case (b), cells are called *channel cells*, and channel routing is performed as explained in the next paragraph. 

At the time of initialization, the model determines for each channel cell which is the corresponding outlet cell (this is also a *channel*, but located in the boundary of the catchment) and how far it is in terms of the number of cells. Additionally, a numeric vector is created to store the backlog of every channel outlet (i.e. to store the water that is a number of days away from the outlet). Then, during overland runoff processing, the runoff of a channel cell is stored for subsequent processing of channel routing. 

Channel routing is performed after water balance simulations of the watershed (or its subwatersheds) are finished. For each day to be simulated and each channel cell, the model adds the water that reached the channel during water balance simulations to the backlog of the corresponding watershed outlet, at a position determined by its distance and the average channel flow speed (watershed control parameter `channel_flow_speed`). Once per day, the amount of water at the origin of the backlog vector of each outlet cell becomes watershed runoff exported ($Exp$), and the rest of values in the backlog vector are shifted one position towards the origin.
