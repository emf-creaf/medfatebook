...
For every day to be simulated, the model performs the following steps:
 ...
 5. Determine vertical cell flows by processing watershed cells (sequentially or in parallel), while accounting for subsurface flows and changes in the groundwater level, which complete eqs.  \@ref(eq:cellsnowpackwaterbalanceequation) and \@ref(eq:cellsoilwaterbalanceequation). For *wildland* or *agriculture* cells, determine snow-pack dynamics, rainfall interception loss, infiltration, saturation excess, capillarity rise, deep drainage and transpiration, soil evaporation processes as described in \@ref(basicwaterbalancescheduling). For *rock* and *artificial* cells, snow pack dynamics is still processed in these cells, but all rainfall input or snow melt becomes runoff to be passed downhill. In the case of *water* cells, snow pack dynamics are still processed, but all liquid water (from rain or snow melt) is poured (as deep drainage) down to the aquifer below.
6. Distribute overland flows among cells downhill (\@ref(overlandflows)). If a cell does not have downhill neighbors (i.e outlet cell) its runoff becomes water exported from the watershed (\@ref(watershedexport)).
7. Apply vertical flows between soils and the aquifer below, i.e. deep drainage and capillarity rise, to the water balance of the aquifer, completing eq. \@ref(eq:cellaquiferwaterbalanceequation).
