...
```{r packages, out.width='100%', fig.align="center", fig.cap="Set of packages conforming the medfate modelling framework", echo=FALSE}
...
Both packages have a user interface in R, but actual calculations have been implemented as C++ functions and linked to R via the **Rcpp** package. This means that you will need a compiler to install **medfate** and **medfateland** packages from sources (i.e. Rtools in Windows). Note that compilers will be a requirement when installing packages from GitHub. 
...
Importantly, `growth()` and `fordyn()` functions and the processes that they implements are still **under active development**.
...
Package **medfateland** allows simulating forest functioning and dynamics on sets forests stands distributed across space. Spatial information needs to be provided in `sf` objects, whose columns typically include spatial geometries, topographic features and lists of `forest` objects and `soil` objects. The actual columns required depend on the simulation function to be used. For example, if forest dynamics includes forest management, the user may supply management arguments for each spatial unit in an additional column.

### Dynamic simulation functions

Package **medfateland** offers sets of simulation functions that are analogous to those of package **medfate** but allow processing multiple forest stands sequentially or in parallel:
...
In addition, the package offers simulation functions where forest stands are related by spatial processes:
...
+ Function `fordyn_scenario()` allows simulating `fordyn()` on forest stands, while applying a demand-based forest management scenario.
