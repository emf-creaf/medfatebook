...
|  `carbon_*` | Biomass estimation and carbon balance |
|  `decomposition_*` | Dynamics of litter and soil organic carbon pools |
...
|  `regeneration_*` | Regeneration from seed germination or resprouting |
...
* `plant_*`: Cohort-level information (species name, id, leaf area, height, etc.).
* `species_*`: Cohort-level attributes aggregated by species (e.g. basal area, cover, density, etc.).
* `herb_*`: Attributes of the (aggregated) herbaceous layer (e.g. fuel density or leaf area index).
* `stand_*`: Stand-level attributes (e.g. stand basal area, fuel loading, etc.).
