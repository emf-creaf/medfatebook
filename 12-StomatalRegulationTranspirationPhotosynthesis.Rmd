...
The model does not try to simulate mistletoe hydraulics, because the focus is on estimating the effect of mistletoe transpiration on the host plant cohort. Mistletoe hydraulic conductance (or water potential drop) is not explicitly represented. Instead, the water potential driving transpiration in mistletoe is assumed to be *equal to the leaf water potential of the host plant*. In other words, it is assumed that the drop in water potential in mistletoe branches and leaves is comparable to the drop from the mistletoe branch insertion point to the host leaf (but see different water potential values in @sancho-knapik_viscum_2026). 
...
\begin{equation}
g_{sw, light, temp, mistletoe} = g_{sw, 0, mistletoe} + m_{wue, mistletoe} \cdot \frac{A_{n, mistletoe}}{C_{s, mistletoe}}
\end{equation}
where $m_{wue, mistletoe}$ is a dimensionless slope representing the water use efficiency, $A_{n, mistletoe}$ is net mistletoe photosynthesis as determined by Farquhar's model (which accounts for light and temperature effects on mistletoe photosynthetic capacity parameters), $g_{sw, 0, mistletoe}$ is the zero intercept when $A_{n, mistletoe} = 0$, here taken as $g_{sw,0, mistletoe} = g_{sw, night}$ and $C_{s, mistletoe}$ is the $CO_2$ concentration inside mistletoe stomata, i.e.:
\begin{equation}
...
\begin{equation}
\lambda_{mistletoe}(\Psi_{leaf,sym}) = 1 - \left(1 + e^{(slope_{gs,mistletoe}/25) \cdot (\Psi_{leaf,sym} - \Psi50_{gs, mistletoe})} \right)^{-1}
\end{equation}

with $\Psi50_{gs, mistletoe}$ and $slope_{gs,mistletoe}$ being parameters of the stomatal vulnerability curve described as control parameters.
