...
## Stomatal regulation and transpiration under Cochard's sub-model {#transpirationcochard}
...
Unlike $g_{leaf,cuti}$, $g_{stem, cuti}$ is assumed non temperature dependent.

### Stomatal conductance

$g_{stom}$ is the stomatal conductance taking into account the dependence of stomata on light, temperature and $CO_2$ concentration [missing currently], as well as water status: 

\begin{equation}
g_{stom} = \lambda(\Psi_{leaf,sym}) · g_{stom, jarvis} 
\end{equation}

where $g_{stom,jarvis}$ is the stomatal conductance without water stress and is determined as a function of light, temperature and CO2 concentration following Jarvis (1976). $\lambda$ is a regulation factor that varies between 0 and 1 to represent stomatal closure according to leaf symplasmic water potential ($\Psi_{leaf,sym}$) using sigmoid-type function:
...
## Cavitation and conduit refilling {#cavitationrefill}
...
\end{equation}
Where $r_{\max,refill}$ is the control parameter `refillMaximumRate` indicating a maximum refill rate. The right part of the equation normalizes the water potential, so that $r_{refill} = r_{refill,\max}$ if $\Psi_{symp,stem} = 0$ and $r_{refill} = 0$ if $\Psi_{symp,stem} <= -1.5MPa$. The proportion of conductance lost in leaves and stem are then updated using:
\begin{eqnarray}
