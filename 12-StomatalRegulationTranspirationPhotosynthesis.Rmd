...
## Stomatal regulation and transpiration under Sureau's sub-model 
...
\begin{equation}
g_{stom} = g_{sw} = \lambda(\Psi_{leaf,sym}) · g_{sw, light, temp} 
\end{equation}

where $g_{sw, light, temp}$ is the stomatal conductance value without water stress, and $\lambda$ is a regulation factor that varies between 0 and 1 to represent stomatal closure according to leaf symplasmic water potential ($\Psi_{leaf,sym}$) using sigmoid-type function:
...
\end{equation}
where $m_{wue}$ is a dimensionless slope representing the water use efficiency, $A_n$ is net photosynthesis as determined by Farquhar's model (which accounts for light and temperature effects), $g_{sw, 0}$ is the zero intercept when $A_{n} = 0$, here taken as $g_{sw,0} = g_{sw, night}$ and $C_{s}$ is the CO2 concentration inside stomata, i.e.:
\begin{equation}
C_{s} = C_{atm} - (A_{n}/g_{bound})
\end{equation}
where $C_{atm}$ is the atmospheric (inside the canopy) CO2 concentration and  $g_{bound}$ is here the boundary layer conductance for CO2.

## Cohort-level transpiration and photosynthesis {#scalingtranspirationphotosynthesis}

The amount of transpiration from the plant cohort in a given time step $t$ ($Tr_{i,t}$, in $l \cdot m^{-2}$ of ground area, i.e. mm) is:
\begin{equation}
