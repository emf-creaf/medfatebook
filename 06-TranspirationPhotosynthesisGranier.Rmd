...
Considering plant water pools (i.e. setting `rhizosphereOverlap = "partial"` or `rhizosphereOverlap = "total"` in the control parameters) involves partitioning the stand area into fractions corresponding to the abundance of each plant cohort. More specifically, the model defines as many plant water pools as plant cohorts, with proportions defined by their LAI values. For example, the fraction corresponding to plant cohort $j$ would be:
\begin{equation}
