...
Potential evapotranspiration ($PET$, in mm) is assumed to represent open water evaporation potential. Maximum canopy transpiration $Tr_{cohorts, \max}$ depends on both $PET$ and the amount of transpirating surface, i.e. the current cumulative $LAI$ of the vegetation, $LAI^{\phi}$. To estimate $Tr_{cohorts, \max}$ the model uses the empirical equation of @Granier1999, where $Tr_{cohorts, \max}/PET$ is a function of $LAI^{\phi}$:

\begin{equation}
\frac{Tr_{cohorts, \max}}{PET}= -0.006\cdot (LAI^{\phi})^2+0.134\cdot LAI^{\phi}+0.036
\end{equation}
This equation has already been adopted for Mediterranean biomes [@Fyllas2009; @Ruffault2013]. 

(ref:tmaxpet-cap) Empirical relationship between $Tr_{cohorts, \max}/PET$ and $LAI^{\phi}$
...
\begin{equation}
\frac{Tr_{cohorts, \max}}{PET}=  T_{max,LAI} \cdot LAI^{\phi} + T_{max,sqLAI} \cdot (LAI^{\phi})^2 
\end{equation}
