# Plant transpiration, photosynthesis and drought stress {#transpirationgranier}

This chapter describes how daily plant cohort transpiration ($Tr_{cohorts}$) is modelled in the basic water balance model, completing the flows needed to evaluate eq. \@ref(eq:basicwaterbalanceequation). The model uses the approach of BILJOU [@Granier1999] to estimate plant cohort maximum transpiration, and then divides this value among the contribution of plant cohorts and estimate actual transpiration depending on soil moisture and cohort functional parameters, which determine plant drought. The chapter also describes how is daily photosynthesis estimated, a process that in this model does not influence transpiration but is necessary for growth calculations (chapter \@ref(growthmodelling)). Finally, we include here the description of the estimation of plant water status, the effect of drought stress and how plants recover from drought legacies.
...
### Transpiration of hemi-parasitic plants {#mistletoetranspirationbasic}

When mistletoes are considered in water balance simulations, they are included in radiation extinction calculations. The maximum transpiration of mistletoe in cohort $i$ is ($Tr_{mistletoe, i, \max}$):

\begin{equation}
   Tr_{mistletoe, i, \max} =  PET \cdot (T_{max,LAI} \cdot LAI_{\phi} + T_{max,sqLAI} \cdot (LAI_{\phi})^2 ) \cdot \frac{f_i^{0.75}}{\sum_{j}{f_j^{0.75}}} \cdot \frac{LAI_{mistletoe,i}}{LAI_{\phi,i}+LAI_{mistletoe,i}}
\end{equation}

Maximum transpiration is then reduced to actual transpiration (i.e. $Tr_{mistletoe, i}$) according to the water potential of the plant cohort ($\Psi_{plant,i}$), following a sigmoid function with parameters  $\Psi50_{gs, bald, mistletoe}$  and $slope_{gs, bald, mistletoe}$ (defined in control parameters). Soil water extraction corresponding to mistletoe transpiration is added to extraction caused by the corresponding plant cohort. 
...
### Cavitation-induced defoliation and recovery {#cavitationinduceddefoliationbasic}

By default, the water-balance model assumes that high levels of leaf hydraulic impairment will lead to defoliation (although this process can be deactivated if `cavitationInducedDefoliation = FALSE`). When cavitation-induced defoliation is considered, the model assumes that crowns are made of a population of leaves whose hydraulic vulnerability follows a Gaussian distribution centered on the input water potential corresponding to 50% loss of conductance (which is dictated by $c_{leaf}$ and $d_{leaf}$) and with a known coefficient of variation set to 10 % (by default), a modelling strategy developed by Hervé Cochard.  Leaves are hydraulically disconnected and die when their embolism rate exceeds a critical value set to 88 % of percent loss of conductance, by default (see defoliation control parameters and function `hydraulics_proportionDefoliationWeibull()`). Ultimately, the assumed variability of hydraulic vulnerability and the current plant water potential lead to a proportion of cavitation-induced crown defoliation.

In the water-balance model, the proportion of crown defoliation leads to a corresponding decrease $LAI_{\phi}$, and recovery of foliage after drought will follow the conduit refilling process associated to decrease in $PLC_{leaf}$. In the growth model, the proportion of crown defoliation is used to determine leaf senescence and the recovery of foliage is simulated via explicit leaf growth (see chapter \@ref(growthsenescencemortality)).
