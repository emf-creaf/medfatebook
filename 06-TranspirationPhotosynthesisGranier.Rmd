...
\end{equation}
Where $r_{\max,refill}$ is the control parameter `refillMaximumRate` indicating a maximum refill rate. The right part of the equation normalizes the water potential, so that $r_{refill} = r_{refill,\max}$ if $\Psi_{plant,i} = 0$ and $r_{refill} = 0$ if $\Psi_{plant,i} <= -1.5MPa$. The proportion of conductance lost in stems and leaves is then updated using:
\begin{eqnarray}
