...
Sink limitations due to temperature and turgor effects on growth are modelled following @Cabon2019 and @Cabon2020. Cell relative expansion rate ($r_{cell}$) is defined as the relative time derivative of cell volume:
\begin{equation}
r_{cell} = \frac{dV}{Vdt}
\end{equation}
@Cabon2020 suggested to account for turgor and temperature limitations on growth using the following equation for $r_{cell}$:
\begin{equation}
r_{cell}(T, \Psi, \pi_0) = \phi_{max} \cdot (\Psi - \pi_0  - Y_{P}) \cdot \frac{f_{met}(T_K)}{f_{met}(288.15)} \cdot{f_{micro}(T, T_{thr})}
\end{equation}
where $T$ is temperature, $\Psi$ is the water potential, $\pi_0$ is the osmotic water potential at full turgor, $Y_P = 0.05$ MPa is the turgor yield threshold, $f_{met}(T_K)$ is a function modulating the effect of temperature $T_K$ in Kelvin, on metabolic rate, and $f_{micro}(T, T_{thr})$ is a sigmoidal function modulating the effect of temperature on microtubule stability, depending on a temperature threshold $T_{thr}=5$ ºC. Function  $f_{met}(T_K)$ is defined as: 
\begin{equation}
...
\end{equation}
$\Delta LA_{alloc}$ is the maximum leaf area increment allowed by the leaf area target set by the allocation rule, $LA^{target}$ in comparison with current leaf area $LA^{live}$:
\begin{equation}
\Delta LA_{alloc} = \max(LA^{target} - LA^{live},0)
\end{equation}
$Delta LA_{source}$ represents source limitations and is:
\begin{equation}
\Delta LA_{source} = s
\end{equation}
metabolic sugar availability - minimum growth and construction cost per LA unit
Finally, $\Delta LA_{sink}$ is the leaf area increment expected by taking into account leaf tissue growth rate ($RGR_{leaf}$) relative to sapwood area $SA$ and the relative cell expansion rate given $T_{day}$, $\Psi_{symp,leaf}$ and $\pi_{0,leaf}$; divided by the maximum relative cell expansion rate $r_{cell,max} = r_{cell}(25, 0, -2)$: 
\begin{equation}
\Delta LA_{sink} = SA \cdot RGR_{leaf} \cdot \frac{r_{cell}(T_{day}, \Psi_{leaf}, \pi_{0,leaf})}{r_{cell,max}}
\end{equation}


growth cost as explained above.
