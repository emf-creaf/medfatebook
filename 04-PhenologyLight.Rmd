...
whereas for evergreen species $LAI_{i}^{\phi}=LAI^{live}_i$ (or equivalently $\phi_i = 1$ at all times). Leaf budburst in deciduous species occurs when daily temperature exceeds $T_{base}$ and $\phi_i$ increases linearly from 0 to 1 as function of the degree days above $T_{base}$, until the functional parameter $S_{GDD,i}$ is surpassed (i.e. until $GDD > S_{GDD,i}$). In autumn, $\phi_i$ drops to 0 when average daily temperature falls again below $T_{base}$ [@Sitch2003]. 
...
Although simulations will normally start in winter and $GDD$ will be zero at the beginning, the user can specify a starting non-zero value for $GDD$ in the object created by function `spwbInput()`. Function `pheno_updateLeaves()` updates the status of expanded leaves and dead leaves in a simulation object.
