...
\begin{eqnarray}
R_{sen,d}(Ph_d, T_d) &=& 0  \,\, &if& \,\, T_d \geq T_{sen} \,\, or \,\, Ph_d>Ph_{sen} \\
              &=& (T_d - T_{sen})^2 \cdot (Ph_d/Ph_{sen})^2  \,\, &if& \,\, T_d < T_{sen} \,\,and \,\, Ph_d<Ph_{sen}\\
\end{eqnarray}
where $Ph_{sen}$ is the maximum photoperiod to start counting the senescence. The state of development of senescence $S_{sen,d}$ is defined
\begin{equation} 
S_{sen,d} = \sum R_{sen,d}
\end{equation}
and when $S_{sen,d} = S_{sen}^*$ then expanded leaves are assumed to suddenly die. This model is implemented in function `pheno_leafSenescenceStatus()` and returns $\phi_{i,d}=1$ for all days until $S_{sen,d} = S_{sen}^*$ when it returns $\phi_{i,d}=0$. Parameters $T_{sen}$, $Ph_{sen}$ and $S_{sen}^*$ are plant-specific parameters in `spwbInput`.  
