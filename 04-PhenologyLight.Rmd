...
\begin{equation}
L^{PAR}_{herb}=e^{-\sum_{i=1}^{c}{k_{PAR,i} \cdot LAI_{all,i}}}
\end{equation}
where $k_{PAR,i}$ is the PAR extinction coefficient of plant cohort $i$. If we add the extinction caused by the herbaceous layer, we have that $L^{PAR}_{ground}$, the proportion of PAR that reaches the ground, is calculated as:
\begin{equation}
...
\begin{equation}
f_j=1 - e^{-\sum_{i=1}^{c}{k_{SWR,i} \cdot LAI_{all, i,j}}}
\end{equation}
...
\begin{equation}
f_{ij} = f_j \cdot \frac{k_{SWR,i}\cdot LAI_{\phi, i,j}}{\sum_{h=1}^{c}{k_{SWR,h} \cdot LAI_{all, h,j}}}
\end{equation}
