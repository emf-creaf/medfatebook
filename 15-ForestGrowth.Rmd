...
Formation of new leaf, sapwood and fine root tissues is determined taking into account both source (i.e. sapwood storage carbon availability) and sink (i.e. biophysical constraints on plant tissue expansion) limitations [@Fatichi2014; @Guillemot2015; @Korner2015]. Temperature and turgor sink limitations are implemented following @Cabon2019 and @Cabon2020. These authors developed their approach for tracheid production and enlargement only, whereas we apply it to different kinds of tissues. Growth controls include also the effect of detecting low levels of metabolic carbon in the stem, which allows stopping growth to use available carbon for maintenance respiration. 
...
Dynamic vegetation models implement mortality processes in very different ways, from purely-empirical to process-based [@Hawkes2000; @Bugmann2019]. The fact that proximal causes of mortality are complex, co-occurring, interrelated and variable has prompted the question of whether a mechanistic approach will improve prediction ability [@Adams2013].

In medfate, woody plants are assumed to die due to unknown mechanisms at a constant basal rate. Mortality rates are assumed to increase whenever physiological thresholds known to cause plant mortality are surpassed. Plant cohorts ultimately can die from either *starvation* (if metabolic carbon is exhausted) or *dessication* (tissue dehydration) [@McDowell2008; @McDowell2011]. 

With respect to intra-cohort variability, the model can be run assuming that all individuals within plant cohorts are functionally equal, so that mortality applies to the entire plant cohort. 
...
+ `equilibriumOsmoticConcentration [= c(leaf = 0.8, sapwood = 0.6)]`: Equilibrium osmotic concentrations ($mol \cdot L^{-1}$) for leaf and sapwood cells. The difference between leaf and sapwood values helps maintaining phloem transport. The equilibrium sugar concentration is `equilibriumOsmoticConcentration - nonSugarConcentration` defaults to `[= c(leaf = 0.5, sapwood = 0.3)]`.
+ `minimumSugarForGrowth [= 0.25]`:  Minimum concentration of sapwood sugar ($mol \cdot L^{-1}$) needed for growth to occur in different tissues. This value should always be larger than `mortalitySugarThreshold [=0.1]` (so that growth stops before starvation) and smaller than the equilibrium sugar concentration for sapwood (so that growth occurs under normal metabolic status).
...
+ `mortalitySugarThreshold [= 0.1]`: Threshold of stem sugar concentration resulting in increased mortality rate/probability whenever levels are below.
...
| `SugarTransport`| $F_{floem}$| $mol\,gluc \cdot s^{-1}$ | Average instantaneous rate of carbon transferred between leaves and stem compartments via floem. |
