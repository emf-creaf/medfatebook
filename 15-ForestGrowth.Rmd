...
The information provided in the current chapter should be enough to understand what the growth model does, but the reading previous chapters and the following ones provide a more detailed understanding on how the different processes are designed and formulated.

## Design principles {#growthdesign}

### Overview

Like in the preceding models, the physical structure of the stand is represented in one (vertical) dimension (i.e. the horizontal coordinates of plants are not explicit). The model uses *cohort-based* representation of living plants, meaning that similar plant individuals are represented using a single entity with average characteristics (e.g. tree height and diameter at breast height) and a density variable is used to scale from (an average) **individual level** to the **cohort level**. Additionally, the model keeps track of carbon pools for **dead standing plants** (i.e. snags), **litter** (aboveground or belowground) and **soil organic matter**, represented at cohort-level (for snags), species-level (for litter) or aggregated level (for soil organic matter), respectively. 

Processes in the forest stand are modeled either at the cohort level (water balance, photosynthesis, or plant mortality) or at the individual level (carbon balance, tissue growth and senescence). Energy balance, water balance and plant photosynthesis processes follow the design of the previous models and will not be repeated here (see chapters \@ref(basicwaterbalance) and \@ref(advancedwaterbalance)). 

At the *individual level*, the growth model tries to reproduce the phenological, physiological and physical processes that modulate live and dead leaf area changes, sapwood area changes and fine root area changes of an *average individual* of the plant cohort. Formation of new sapwood leads to diameter and height increases, so that physiological processes involve **plant structural changes**. Since the model does not implement all processes and drivers (such as nutrient availability) potentially affecting growth, user input parameters are used to empirically constrain growth rates and plant size, i.e. maximum relative sapwood area growth rates, maximum plant height and diameter-height ratios. 
...
Finally, the model also simulates the **decomposition** of organic matter produced by senescence and mortality processes (root exudation, litter fall and plant mortality). Plant litter (i.e. fine roots, leaves, branches, etc.) is allocated into different structural carbon pools depending on the species that produced it. Decomposition of organic matter in those structural pools leads to fluxes towards surface and soil carbon matter pools for further decomposition. Decomposition processes involve heterotrophic respiration, which releases $CO_2$ back to the atmosphere. 
...
Details of the carbon pools in different compartments are as follows (Fig. \@ref(fig:carbonpools)):
 ...
 + **Metabolic carbon** is a labile carbon form assumed to correspond to **sugars** (e.g., glucose or fructose) used to sustain cell functioning (i.e. provide carbon for maintenance respiration). Metabolic carbon occurs in leaves and sapwood compartments only. *Leaf metabolic carbon* is generated from photosynthesis or leaf starch degradation and used in different functions, including leaf maintenance respiration and export to sapwood via phloem transport. Sugar concentration in the phloem is assumed to be (passively) in equilibrium with leaf metabolic sugar concentration. *Sapwood metabolic carbon* comes from phloem transport, or sapwood starch degradation, and is used to directly sustain maintenance respiration of sapwood and fine roots. 
...
### Growth phenology {#growthphenologydesign}

Whereas in the water balance models expanded leaf area corresponded to the portion of live leaf area that was unfolded according to the phenological status, in the growth model leaf area changes result from explicit simulation of growth. However, the leaf phenology submodel still controls the duration of phenophases corresponding to leaf organogenesis, leaf elongation and leaf senescence. 

Growth phenology types are defined by combining leaf phenology types (i.e. evergreen, winter-deciduous or winter-semideciduous) with the degree of determinacy of growth, which describes how growth in the current year occurs either [@baumgarten_determinacy_2026]: (i) from elongating organs preformed the preceding year and contained within persistent buds: *determined growth*; (ii) from continuous meristematic activity forming new shoots *indetermined growth*; or (iii) from both strategies: *intermediate growth*. The following five combinations of leaf phenology and growth determinacy are distinguished (see Fig. \@ref(fig:phenology)):

  (a) *Evergreen / Indetermined growth*: Indetermined evergreen plants are assumed to shed old leaves at the same time as new leaves appear. Leaf senescence and leaf formation can occur continuously throughout the year, and leaf growth occurs without bud phase.
  (b) *Evergreen / Determinate growth*: Determinate evergreen plants are assumed to form persistent buds before winter and concentrate budburst, leaf elongation and leaf senescence during the same period in spring. The amount of leaf area to be flushed in spring is decided during the persistent bud formation period, assumed to happen at the end of leaf elongation.
  (c) *Evergreen / Intermediate growth*: Evergreen plants with intermediate growth determinacy have a mixed growth strategy. Like determinate plants, persistent buds are formed the preceding year, and a first set of leaves is flushed out in spring. Additionally, the plant can continue forming new leaves after these initial ones have finished elongation, in the same way as a indeterminate evergreen plants, until there is a decrease in photoperiod. From that moment on, persistent buds are formed for the next year. 
  (d) *Winter (semi-)deciduous / Determined growth*: In winter deciduous and semi-deciduous plants with determined growth leaf bud formation occurs at the end of the leaf elongation. Buds remain under para- or eco-dormancy until next spring, when budburst and elongation occurs. Winter deciduous species shed their all their leaves at the time of leaf senescence in autumn, whereas semi-deciduous species keep dead leaves in the crown during winter until budburst of new leaves.
  (e) *Winter (semi-)deciduous / Intermediate growth*: Intermediate winter (semi-)deciduous species are the analogue of intermediate evergreen species but with winter (semi-)deciduous phenology. Budburst and elongation of preformed leaves can be followed by new leaf formation during the growth season. Like in intermediate evergreen growth plants, bud formation is set to occur between the decrease of photoperiod and the start of ecodormancy. 
  
```{r phenology, out.width='100%', fig.align="center", fig.cap="Schematic representation of processes and timings for different leaf phenology types", echo=FALSE}
knitr::include_graphics("figures/leaf_phenology.png")
```


### Tissue growth and senescence processes {#growthsenescencedesign}

The forest growth model allows simulating the variation of leaf area, sapwood area and fine root area via the following growth and senescence processes (see fig. \@ref(fig:allocationdiagram)):

 i. **Leaf/twig growth**:  Leaf area increase occurs during *leaf elongation* periods (after budburst) or during **indetermined (free) growth**. Organogenesis during persistent bud formation does not consume any carbon, but leaf elongation or indetermined (free) leaf growth does consume carbon. Moreover, leaf growth includes an additional carbon cost in *twig growth*, according to the biomass ratio between both.
 ii. **Leaf/twig senescence and leaf fall**: Leaf/twig senescence (and the corresponding increases in dead leaf area) can occur in the model due to *leaf aging* in evergreen species, due to *programmed leaf senescence* in deciduous species, or due to *leaf dessication* (i.e. cavitation-induced senescence). Leaf senescence increases the leaf area index corresponding to dead leaves, thereby increasing the leaf and twig dead carbon pools. *Leaf fall* is the process that transforms dead leaf area into leaf and twig litter, which triggers decomposition processes (see fig. \@ref(fig:carbonpools)). In addition, fires can induce *foliage burning*, which also leads to a decrease in dead and live leaf area (but where non-structural carbon reallocation is not possible) and produces carbon emissions due to leaf and twig combustion.
 iii. **Sapwood growth**: The model assumes that the hormonal signals controlling cambium division have been mainly synthesized in leaves. Auxins are known to be primarily generated in the plant's apical meristems, young leaves, and flower buds; from where they are transported throughout the plant via the phloem [@bajguz_biosynthetic_2023]. More specifically, we assume that the levels of secondary growth hormones (auxines) depend linearly on the *current amount of foliage relative to the expected maximum foliage of the same plant without competition*. This entails that: (a) leaf phenological status of deciduous species will affect sapwood growth rates; (b) for a given plant size (i.e. cambium surface) the amount of foliage will be higher or lower depending on the shading created by taller plants and, hence, understory plants or densely-crowded plants will have lower sapwood growth rates than plants that are isolated or overtop the other plants; (c) defoliation events (e.g. from drought impacts) will have an effect on sapwood growth rates, as observed empirically [@javoy_impact_2025]. Sapwood formation corresponds mostly to secondary growth, but primary growth occurs at the same time due to diameter-height relationships.
...
### Source and sink limitations to growth {#sourcesinkdesign}

The growth rates leaf/twig, sapwood and fine root compartments are primarily determined by parameters specifying *maximum daily growth rates*. Maximum growth rates are then modulated after taking into account both **source** (i.e. carbon availability) and **sink** (i.e. biophysical constraints on plant tissue expansion) limitations [@Fatichi2014; @Guillemot2015; @Korner2015] (see fig. \@ref(fig:allocationdiagram)). 

Carbon availability limitations to growth apply to leaf and fine root growth because storage carbon levels need to be at more than 10\% of their capacity. In contrast, sapwood formation is only allowed if storage carbon levels are above a given species-specific threshold which are normally much higher (e.g. 50\%) than required for leaf or fine root growth. Thus, when carbon levels are relatively low, sapwood growth rates fall to zero, whereas replacement of leaves and fine roots continues to sustain metabolism. This design was chosen to stop sapwood growth much before emptying storage carbon reserves, so that maintenance respiration can be sustained longer whenever photosynthesis is low, as in shadow-tolerant species. 

Temperature and turgor sink limitations to growth are implemented following @Cabon2019 and @Cabon2020. These authors developed their approach for tracheid production and enlargement only, whereas we apply it to different kinds of tissues. Bud formation and leaf elongation of preformed leaves can both be reduced due to turgor limitations during the corresponding organogenesis or expansion process, but these processes do not take into account temperature effects. In contrast, the rate of indetermined leaf area growth is affected by both temperature and turgor conditions. In the case of fine roots, differences in soil water potential between layers can lead to different degrees of turgor limitation for the growth of fine roots, resulting in acclimation of fine root distribution to soil moisture dynamics. 


### Growth coordination {#growthcoordinationdesign}

Leaf area target limits the maximum amount of leaves formed during organogenesis and this target is determined by plant size (i.e. tree diameter) and stand structure (i.e. competition for light) via allometric equations of foliar biomass (see fig. \@ref(fig:allocationdiagram)). This limits tissue growth to the replacement of aging tissues, unless there are changes in forest structure, i.e. competition is released or the target tree increases in diameter. Sapwood growth targets are determined from functional relationships (e.g. target Huber value or whole-plant conductance) with leaf area targets (see below). Since sapwood formation leads to tree diameter increases, and therefore to forest structural changes, maximum sapwood growth rate indirectly determine the increase in leaf area and fine root targets, hence influencing the whole-plant maximum growth rate. 
...
Two options are possible regarding the functional coordination between structural carbon of leaves and sapwood (see fig. \@ref(fig:allocationdiagram)):
...
\end{equation}
where $MR = MR_{leaf} + MR_{twig} +MR_{sapwood} + MR_{fineroot}$ is plant-level maintenance respiration and $GC = GC_{leaf} + GC_{twig} +GC_{sapwood} + GC_{fineroot}$ is plant-level growth costs.

### Plant biomass balance

In addition to changes in labile carbon pools, the model also implements changes in the size of structural and dead carbon compartments, as a result of the interplay between growth, senescence and foliage burning processes, as well as cohort-level biomass balance once mortality processes are taken into account.

**Live biomass balance**
For the balance of structural carbon pools, we can define the following structural equations (all variables in $g\,dry$):
\begin{eqnarray}
\Delta{B_{leaves}} &=& B_{leaves, growth} - B_{leaves, senescence} - B_{leaves, burnt}\\
\Delta{B_{twigs}} &=& B_{twigs, growth} - B_{twigs, senescence} - B_{twigs, burnt}\\
...
\end{eqnarray}
where $B_{leaves, growth}$, $B_{twigs, growth}$, $B_{sapwood, growth}$ and $B_{fineroot, growth}$ are the biomass increase due to growth for leaves, twigs, sapwood and fineroots, respectively;  $B_{leaves, senescence}$, $B_{twigs, senescence}$, $B_{sapwood, senescence}$ and $B_{fineroot, senescence}$ are the biomass loss due to senescence for leaves, twigs, sapwood and fineroots, respectively; $B_{leaves, burnt}$ and $B_{twigs, burnt}$ are the biomass burnt of leaves and twigs. 

The following equation defines the biomass balance for (live) structural pools (in $g\,dry$):
...
Senescence processes increase carbon for dead tissue pools, whereas litter fall and foliage burn decrease them. Hence, we can define the following equations (all variables in $g\,dry$):
\begin{eqnarray}
\Delta{D_{leaves}} &=& B_{leaves, senescence} - D_{leaves, fall} - D_{leaves, burnt}\\
\Delta{D_{twigs}} &=& B_{twigs, senescence} - D_{twigs, fall} - D_{twigs, burnt}\\
\Delta{D_{heartwood}} &=& B_{sapwood, senescence}\\
(\#eq:deadstructurecarbonbalanceequations)
\end{eqnarray}

Where $D_{leaves, fall}$ and $D_{twigs, fall}$ are the mass of dead leaves and twigs, respectively, that falls into litter compartments; and $D_{leaves, burnt}$ and $D_{twigs, burnt}$ are the mass of dead leaves and twigs, respectively, that are burnt. For trees, sapwood senescence ($B_{sapwood, senescence}$) results in a corresponding increase in heartwood ($D_{heartwood}$). In contrast, for shrubs heartwood is not considered and sapwood senescence ($B_{sapwood, senescence}$) results in dead standing branch (snag) creation. 


**Total biomass balance**

The overall mass balance at the **individual level** is completed if we add the labile biomass balance to the structural and dead tissue mass balances (in $g\,dry$):
\begin{equation}
\Delta{B_{plant}} = \Delta{B_{structure}} + \Delta{S_{labile}} + \Delta{D_{leaves}} + \Delta{D_{twigs}} + \Delta{D_{heartwood}} 
(\#eq:planttotalcarbonbalanceequation)
\end{equation}

To evaluate biomass balance at the cohort level, we need to take into account that some individuals will die, so that the biomass balance needs to incorporate mortality losses (all variables in $g\,dry \cdot m^{-2}$):
\begin{equation}
\Delta{B_{cohort}} = \Delta{B_{plant}} \cdot N_{cohort} - D_{above, dead} - D_{below, dead}
(\#eq:cohorttotalcarbonbalanceequation)
\end{equation}
where $N_{cohort}$ is the initial cohort density (before mortality occurred), $D_{above, dead}$ is the aboveground (leaves, twigs, small branches and large wood) biomass loss due to mortality of individuals and $D_{below, dead}$ is the belowground (coarse roots and fine roots) biomass loss due to mortality of individuals. The latter two are calculated using:
\begin{eqnarray}
 D_{above, dead} &=& (B_{plant, above} + \Delta{B_{plant, above}}) \cdot N_{dead} \\
 D_{below, dead} &=& (B_{plant, below} + \Delta{B_{plant, belowground}}) \cdot N_{dead, nonresp}
(\#eq:mortalitycarbonlossequation)
\end{eqnarray}
where $B_{plant, above}$ is the initial plant aboveground biomass (leaves, twigs, small branches and large wood), $B_{plant, below}$ is the initial plant belowground biomass (coarse roots and fine roots); $N_{dead}$ is the density of dead individuals and $N_{dead,noresp}$ is the density of individuals that are dead and will not resprout next year (resprouting is handled by the forest dynamic model). In turn, $D_{above, dead}$ is divided between standing wood ($D_{wood,non-burnt}$) and wood emissions due to wildfire combustion ($D_{wood,burnt}$):
\begin{equation}
D_{above, dead} = D_{wood,non-burnt} + D_{wood,burnt}
\end{equation}
Note that $D_{above,dead}$ should include dead biomass of leaves and twigs, that is not shown here for simplicity.

### Mass balance of decomposition pools

Let $\Delta SN$, $\Delta LI$ and $\Delta SOC$ be the change in the amount of carbon in snag, litter and soil organic carbon pools ($g\,C\cdot m^{-2}$). Now let $\Delta DEC$ be the sum of mass changes in all pools taken together:
\begin{equation}
\Delta DEC = \Delta SN + \Delta LI +  \Delta SOC 
\end{equation}
$\Delta DEC$  is equal to litter and root exudation inputs minus heterotrophic respiration and snag/litter combustion:
\begin{equation}
\Delta DEC =  \sum_{i}{(LI_{cohort,i}  + RE_i)} - SN_{burnt} - LI_{burnt} - HR
\end{equation}
where $RE_i$ is root exudation for cohort $i$, $SN_{burnt}$ is the mass of snags (small branches) burnt, $LI_{burnt}$ is the mass of litter burnt and $HR$ is heterotrophic respiration, all expressed in $g\,C \cdot m^{-2}$ units. Finally, $LI_{cohort,i}$ is the sum of litter inputs from cohort $i$, i.e.:
\begin{equation}
LI_{cohort,i} = D_{leaves, fall,i} + D_{twigs,fall,i}  + B_{fineroot, senescence,i} + D_{below, dead,i}
\end{equation}
where $D_{leaves, fall, i}$ and $D_{twigs, fall, i}$ is the mass of fallen leaves, respectively, $B_{fineroot,senescence,i}$ is the mass of dead fine roots due to senescence (all after multiplication by cohort density, $N_{cohort,i}$, and carbon content), $D_{below, dead, i}$ is the mass of coarse and fine roots entering decomposition after belowground mortality. The litter inputs corresponding to small branches and large wood are not included here because they are created by the model of forest dynamics as a result of snag fall.

### Fire combustion

Fire effects led to carbon emissions ($FC$; $g\,C \cdot m^{-2}$) corresponding the combustion of live and dead biomass from plant cohorts, snags and litter:

\begin{equation}
FC = \sum_{i}{B_{burnt,i}} + SN_{burnt} + LI_{burnt}
\end{equation}
where $SN_{burnt}$ is the mass of snags burnt (small branches), $LI_{burnt}$ is the mass of litter burnt (only leaves, twigs and small branches) and $B_{burnt,i}$ is the biomass burnt for cohort $i$:
\begin{equation}
B_{burnt,i} = B_{leaves, burnt,i} + B_{twigs, burnt,i} + D_{leaves, burnt,i} + D_{twigs, burnt,i} + D_{wood, burnt,i}
\end{equation}
where $B_{leaves, burnt,i}$ and $B_{twigs, burnt,i}$ are the mass of live leaves and twigs, respectively, that are burnt; $D_{leaves, burnt,i}$ and $D_{twigs, burnt,i}$ are the mass of dead leaves and twigs, respectively, that are burnt; $D_{wood,burnt,i}$ is the mass of burnt wood for cohort $i$.
...
The model estimates several carbon fluxes at the stand level, all expressed in units of $g\,C\cdot m^{-2}$:
  ...
  NPP = GPP - MR - SR 
  \end{equation}
  Note that net primary production is in some models equal to the carbon allocated to new plant structures (i.e. growth), but in medfate NPP can be used to replenish labile carbon pools.
  ...
  * **Net ecosystem production** ($NEP$) is the net carbon source of the forest ecosystem, i.e. the net primary production minus heterotrophic respiration and fire combustion:
\begin{equation}
NEP = \Delta{C}_{stand} = NPP - HR - FC 
(\#eq:NEPequation)
\end{equation}

$NEP$ expresses the overall carbon balance of the forest stand ($\Delta{C}_{stand}$), i.e.:
\begin{equation}
\Delta{C}_{stand} = \Delta SN + \Delta LI +  \Delta SOC + \sum_{i}{(\Delta B_{cohort,i} + D_{snags,i})}
(\#eq:standcarbonbalanceequation)
\end{equation}
where $D_{snags,i}$ stands for standing dead wood from cohort $i$ that that will become snag cohorts (managed by the model of forest dynamics) and includes non-burnt aboveground mortality ($D_{wood, nonburnt, i}$) and shrub sapwood senescence ($B_{sapwood, senescence, i}$):
\begin{equation}
D_{snags,i} = D_{wood, nonburnt, i} + B_{sapwood, senescence, i}
\end{equation}

## Process scheduling {#growthscheduling}

Growth simulations are conducted in **daily** time steps. For every day to be simulated, the model performs the following blocks of steps:

**A. Water and energy balance** 

Water and energy balance calculations are performed by calling the basic or the advanced water balance model (\@ref(basicwaterbalancescheduling)). 

**B. Individual-level carbon balance, growth and senescence**

The model performs the following steps for each plant cohort:

  1. Determine the amount of live structural biomass (in leaves, sapwood and fine root compartments), storage volume (in leaves and sapwood), storage capacity (\@ref(estimationcarbonpools)) and the construction costs per unit tissue (area or dry weight). 
  2. If $LA_{\phi} > 0$, estimate gross photosynthesis per individual ($A$) from cohort-level photosynthesis ($A_g$).
  3. Estimate maintenance respiration for leaves ($MR_{leaf}$), twigs ($MR_{twig}$), sapwood ($MR_{sapwood}$) and fine roots ($MR_{fineroot}$) (\@ref(carbonbalancecomponents)).
  4. If phenology indicates leaf unfolding, evaluate leaf temperature and turgor sink limitations, leaf and twig growth costs ($GC_{leaf}$ and $GC_{twig}$, respectively), leaf and twig synthesis respiration ($SR_{leaf}$ and $SR_{twig}$, respectively) and leaf area increase (\@ref(leafgrowth)). 
  5. If sapwood growth is permitted, evaluate sapwood temperature and turgor sink limitations, sapwood growth costs ($GC_{sapwood}$), sapwood synthesis respiration ($SR_{sapwood}$) and sapwood area increase (\@ref(sapwoodgrowth)).
  6. If fineroot biomass is lower than the target biomass, evaluate fineroot temperature and turgor sink limitations, fineroot growth costs ($GC_{fineroot}$), fineroot synthesis respiration ($SR_{fineroot}$) and fineroot biomass increase (\@ref(finerootgrowth)).
  7. Update changes in sugar and starch compartments due to photosynthesis, maintenance respiration and growth. Apply transfer from leaf metabolic to sapwood metabolic carbon (\@ref(phloemtransport)) and conversions between storage and metabolic carbon (\@ref(sugarstarchdynamics)).  
  8. Determine leaf senescence due to aging and leaf/bud senescence due to cavitation effects (\@ref(leafsenescence)). 
  9. Determine sapwood senescence due to aging. For shrubs, convert sapwood senescence into mass of standing dead branches (\@ref(sapwoodsenescence)). 
  10. Determine fine root senescence by aging (\@ref(finerootsenescence)). Divide dead fine root mass between metabolic soil carbon pool and fine root litter pool. 
  11. Evaluate labile carbon translocation due to leaf and sapwood senescence ($TS_{leaf}$, $TT_{leaf}$ and $TS_{sapwood}$). If leaf storage carbon is surpassed, add the excess to $TT_{leaf}$. Add the sum of translocated carbon to the starch sapwood pool.
  12. Move any surplus in sapwood storage carbon to root exudation ($RE$). This completes the daily carbon balance for leaf labile pools \@ref(eq:leafcarbonbalanceequations), sapwood labile pools \@ref(eq:sapwoodcarbonbalanceequations), and plant-level labile carbon \@ref(eq:plantcarbonbalanceequation). The model checks at this point the plant-level labile carbon balance.
  13. Update (expanded) leaf area, dead leaf area, sapwood area, fine root biomass and derived variables (e.g., leaf area index and Huber value) (\@ref(updateplanttraits)). 
  ...
  15. Translate sapwood area growth into structural variables (i.e., plant height, tree DBH, tree crown ratio and shrub cover), (\@ref(structuralupdate)),
  16. Update leaf, sapwood and fine root tissue targets and update storage volumes and the concentration of labile pools.

Steps 2-7 can be conducted at sub-daily time steps if the advanced model has been used for water balance (see control option `subdailyCarbonBalance`).

**C. Fire occurrence and behavior**

Then, the model evaluates fire occurrence and, if so, predicts fire behavior (see chapter \@ref(fuelandfirebehavior)). 

**D. Mortality and fire severity effects**

D1. Determine if a wildfire stroke the forest stand and, if so, fire severity effects (\@ref(fireseverity)).
D2. Determine if plant death by starvation or desiccation occurred (\@ref(plantmortality)).
...
| $RGR_{bud}$ |  $m^2 \cdot m^{-2}$| `RGRbud` | Bud (potential leaf area) daily growth rate, relative to leaf area growth |
