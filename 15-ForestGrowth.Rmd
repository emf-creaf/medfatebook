...
1. Update the expanded leaf area of living plants according to the phenology of species and the day of the year and perform soil water balance, transpiration and photosynthesis calculations by calling the water balance submodel (see \@ref(basicwaterbalancescheduling) and \@ref(advancedwaterbalancescheduling)). 

After dealing with water fluxes and photosynthesis, the model performs the following steps:

2. Determine the current amount of respiratory biomass (in leaves, sapwood and fine root compartments) and maximum starch storage capacity for leaves and sapwood (depending on their volume). 
3. Estimate leaf respiration ($R_{leaf}$) and evaluate $SS_{leaf}$ after accounting for gross photosynthesis. If the balance produces a negative value $SS_{leaf}$, leaf senescense occurs until the balance becomes zero.
4. The remaining components of leaf carbon balance (leaf growth, floem transport and sugar-starch dynamics) are evaluated sequentially.
5. Floem transport and respiration components of sapwood carbon balance are evaluated first. If the balance becomes negative the aboveground part of the plant is assumed to die (but it could resprout from the root collar if starch is still present).
6. The remaining components of the sapwood carbon balance (sapwood growth, fine root growth and sugar-starch dynamics) are evaluated sequentially.
7. After evaluating carbon balance, the model determines variations in sapwood area (and sapwood volume) originated due to conversion from sapwood to heartwood, growth or drought-induced cavitation.

Once a year (or by the end of the simulated period) the model translates cumulated sapwood area growth into structural variables (i.e., plant height, tree DBH, tree crown ratio and shrub cover). 
