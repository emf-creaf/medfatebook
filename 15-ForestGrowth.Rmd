...
Constant Huber value strategy is the only one available when running the growth model along with the basic water balance, whereas the two strategies are allowed when simulation growth with the advance water balance. Fine root area growth is only modelled when simulating growth with advance water balance. Fine root area growth is coordinated with the formation of the other tissues via the need to ensure an *average resistance fraction in the rhizosphere*, which is used to define the fine root area target and, hence, the allocation to fine roots. 

Growth rates of all three compartments are regulated by parameters specifying maximum relative growth rates (i.e. the daily growth area relative to the amount of tissue present). However, since leaf and fine root area targets are constrained by sapwood area via allocation rules, the relative growth rate for sapwood area indirectly determines the growth rates of leaves and fine roots. 

### Tree diameter, tree height and shrub cover updates

Update of stand structure is conducted once every simulated year. Tree diameter increment follows the creation of sapwood area, whereas sapwood area decreases do not affect diameter, as they imply conversion to heartwood. Tree height increments are estimated from diameter increments following diameter-height ratios that are modulated by the proportion of light available to the cohort, as frequently done in forest gap models [@Lindner1997]. Tree crown ratios are updated using static allometric relationships, whereas shrub height and cover are estimated from new leaf area values using leaf biomass allometries.
...
```{r, out.width='100%', fig.align="center", fig.cap="Schematic representation of relationships between plant hydraulics (advanced water balance) and carbon balance", echo=FALSE}
...
Several cohort-level structural variables that were treated parameters in the water balance models become state variables here (for each cohort $i$):
...
There are functional state variables necessary to know the physiological status of plants, defined at the individual level:
...
Belowground variables (fine root area, fine root biomass, fine root distribution and radial length of coarse roots) are only considered dynamic when simulating growth with advance water balance.
...
\end{eqnarray}
where $A$ is (gross) photosynthesis, $R_{leaf}$ is leaf respiration, $F_{phloem}$ is the sugar flow via phloem towards the stem and roots, $G_{leaf}$ is the carbon lost via new leaf growth, $SC_{leaf}$ is the conversion from leaf sugar to leaf starch and $RE_{leaf}$ is the root exudation of leaf carbon. Leaf senescence has no impact on dissolved sugar and starch compartments of leaves (but it affects leaf biomass, volume and leaf area) as it is assumed that these non-structural carbon sources are recycled before necrosis.
...
\end{eqnarray}
where $F_{phloem}$ is the sugar inflow via phloem transport, $R_{sapwood}$ is the sum of respiration of branches, stem and coarse roots, $R_{fineroots}$ is the respiration of fine roots, $G_{sapwood}$ and $G_{fineroots}$ are the carbon expenses of growth of new sapwood tissue or fine roots (respectively), $SC_{sapwood}$ is the conversion of sapwood sugar to sapwood starch and $RE_{sapwood}$ is the root exudation of sapwood carbon. Analogously to leave senescence, when sapwood is converted into heartwood, non-structural carbon in the protoplasm of parenchyma cells is assumed to be absorbed by neighboring living cells.
...
1. Performs soil water balance, transpiration and photosynthesis calculations by calling the basic water balance model (see \@ref(basicwaterbalancescheduling)).
2. Determine the current amount of live structural biomass (in leaves, sapwood and fine root compartments), storage volume (in leaves and sapwood) and carbon construction costs per unit tissue (leaf or sapwood) area. 
3. Calculate gross photosynthesis per individual ($A$), maintenance respiration costs for leaves ($R_{leaf}$), sapwood ($R_{sapwood}$) and fine roots ($R_{fineroot}$).
4. If phenology indicates leaf unfolding, evaluate temperature and turgor sink limitations for growth of leaves, estimate growth costs ($G_{leaf}$) and record leaf area increases. 
5. If sapwood growth is permitted, evaluate temperature and turgor sink limitations for growth of sapwood, estimate growth costs ($G_{sapwood}$) and record sapwood area increases. 
6. Apply changes to labile carbon compartments, including the transfer from leaf metabolic to sapwood metabolic carbon (via phloem) and conversions between storage and metabolic carbon. Record surplus in storage carbon as root exudation. This completes the carbon balance of $SS_{leaf}$ \@ref(eq:leafcarbonbalanceequations) and $SS_{sapwood}$ \@ref(eq:sapwoodcarbonbalanceequations) for the daily time step.
7. Determine leaf area dynamics, including growth and senescence (aging, starvation and embolism effects).
8. Determine sapwood area dynamics, including growth and senescence (aging and embolism effects).
9. Update derived variables (e.g., leaf area index and Huber value) and evaluate if death by starvation or plant desiccation occurred.

### Advanced scheduling
For every day to be simulated, the model performs the following steps:

1. Performs soil water balance, energy balance, transpiration and photosynthesis calculations by calling the advanced water balance model (\@ref(advancedwaterbalancescheduling)).
2. Determine the current amount of live structural biomass (in leaves, sapwood and fine root compartments), storage volume (in leaves and sapwood) and carbon construction costs per unit tissue (leaf or sapwood) area. 
3. For every **subdaily** step perform carbon balance as follows:
    a. Calculate gross photosynthesis per individual ($A$) and maintenance respiration costs for leaves ($R_{leaf}$), sapwood ($R_{sapwood}$) and fine roots ($R_{fineroot}$), taking into account both live structural biomass and labile sugar biomass.
    b. If phenology indicates leaf unfolding, evaluate temperature and turgor sink limitations for growth of leaves, estimate growth costs ($G_{leaf}$) and record leaf area increases. 
    c. If sapwood growth is permitted, evaluate temperature and turgor sink limitations for growth of sapwood, estimate growth costs ($G_{sapwood}$) and record sapwood area increases. 
    d. Evaluate temperature and turgor sink limitations for growth of fine roots, estimate growth costs ($G_{fineroot}$) and record fine root biomass increases. 
    d. Update $SS_{leaf}$ and $SS_{sapwood}$ taking into account photosynthesis, maintenance respiration and growth respiration.
    e. Evaluate phloem transport ($F_{phloem}$) and sugar-starch dynamics in the leaf and sapwood compartments **at 1s time steps**, including diversion towards root exudation if maximum starch capacity is attained. This completes the carbon balance of $SS_{leaf}$ \@ref(eq:leafcarbonbalanceequations) and $SS_{sapwood}$ \@ref(eq:sapwoodcarbonbalanceequations) for the subdaily time step.
  
4. If the balance of $SS_{leaf}$ is negative, apply leaf senescence as the exceeding leaf area according to respiration costs. Otherwise, a constant leaf senescence rate is applied according to leaf lifespan. 
5. Apply a constant rate of sapwood senescence (conversion to heartwood), which does not affect sapwood C storage.
6. Increase sapwood area ($SA$) and leaf area ($LAI^{live}$) according to cumulative increases obtained from carbon balance.
7. Update Huber values, hydraulic conductance of stems and roots according to current sapwood and leaf area values. 
8. Update osmotic potential at full turgor of leaves and sapwood according to current sugar concentrations (a constant concentration of salts is assumed).
9. If non-stomatal photosynthesis limitations are considered, apply them.
...
| `RootExudation`|$R_{g,i}$ | $g\,gluc \cdot g\,dry^{-1} \cdot day^{-1}$ | Daily root exudation per dry weight of living biomass. |
