...
## Design principles (basic water balance model)
...
*Constant Huber value* (basic water balance model). The Pipe model [@Shinozaki1964] is adopted to link increments of leaf area, sapwood area and fine root biomass. The model assumes a constant, species-specific Huber value, but allows deviations from the pipe model caused by drought-related leaf area reductions. 
...
Three kinds of carbon pools are considered: **structural**, **metabolic** and **storage**: 
...
Carbon balance of leaf metabolic and storage carbon is perfomed by considering photosynthesis, sugar-starch conversion and floem tranport. Plant respiration is calculated at the individual level, by estimating the respiration of leaves, stem and fine root compartments. While fine root respiration is proportional to leaf respiration, and hence to expanded leaf area, stem respiration depends on plant size.
...
Sapwood area of individuals is another important state variable. Two options are possible regarding the relationship between leaf area and sapwood area (Huber values):


  2. *Dynamic Huber value* (advanced water balance model): Ratios of leaf area to sapwood area (Huber value) can vary within species, due to environmental conditions [@Mencuccini1995]. Huber value variations arise as a result of the dynamic equilibrium of growth/senescence in leaf area and sapwood area, both constrained by plant hydraulics. Growth in leaves is hierarchically prioritized because carbon for growth comes from the leaf metabolic carbon (sugars), so that it may be used before being exported towards the stem. Excess of leaf growth leds to a decrease in stem and fine root conductance (per leaf area), which may feedback negatively to leaf growth via a decreased leaf turgor. This can only be compensated by increasing again stem/root conductance via sapwood secondary growth and fine root conductance via fine root growth. 
...
After dealing with water fluxes and photosynthesis, the model performs the following steps:

1. Determine the current amount of respiratory biomass (in leaves, sapwood and fine root compartments) and maximum starch storage capacity for leaves and sapwood (depending on their volume). 
2. Estimate leaf respiration ($R_{leaf}$) and evaluate $SS_{leaf}$ after accounting for gross photosynthesis. If the balance produces a negative value $SS_{leaf}$, leaf senescense occurs until the balance becomes zero.
3. The remaining components of leaf carbon balance (leaf growth, floem transport and sugar-starch dynamics) are evaluated sequentially.
4. Floem transport and respiration components of sapwood carbon balance are evaluated first. If the balance becomes negative the aboveground part of the plant is assumed to die (but it could resprout from the root collar if starch is still present).
5. The remaining components of the sapwood carbon balance (sapwood growth, fine root growth and sugar-starch dynamics) are evaluated sequentially.
6. After evaluating carbon balance, the model determines variations in sapwood area (and sapwood volume) originated due to conversion from sapwood to heartwood, growth or drought-induced cavitation.
