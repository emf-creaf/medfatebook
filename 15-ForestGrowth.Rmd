...
For every day to be simulated, the model first executes soil water balance, transpiration and photosynthesis calculations by calling the basic water balance model (\@ref(basicwaterbalancescheduling)). Then, the model evaluates fire occurrence and, if so, predicts fire behavior. After that, the model performs the following steps for each plant cohort:
...
11. Move any surplus in sapwood storage carbon to root exudation ($RE$). This completes the daily carbon balance for leaf labile pools \@ref(eq:leafcarbonbalanceequations), sapwood labile pools \@ref(eq:sapwoodcarbonbalanceequations), and plant-level labile carbon. \@ref(eq:plantcarbonbalanceequation).
