...
knitr::include_graphics("Allocation.drawio.png")
```
...
Finally, the model implements an additional source of mortality for young individual trees, meant to represent a self-thinning process occurring between small trees (e.g. 1 cm DBH) resulting from seed establishment (or resprouting) and trees achieving the size (e.g. 7.5 cm) corresponding to inclusion as individual in forest inventories (e.g. ingrowth size). This mortality process is most relevant for simulations of forest dynamics (see \@ref(dynamicsmodelling)).

Details of the implementation of plant mortality are given in section \@ref(plantmortality).

### Fire occurrence and severity

The occurrence of fires is represented in the growth model in a forcing variable that describes the daily probability that a wildfire strikes the stand. Whenever a fire occurs, the model will first simulate fire behavior according to the conditions of the stand (see chapter \@ref(fuelandfirebehavior)) and then fire severity will be evaluated. 

The degree to which vegetation is impacted by fire depends on the heat fluxes incident on various plant parts, which is an outcome of various fire behavior variables such as fireline intensity and residence time. Fire effects on trees can be classified as two types: first- and second-order effects [@bar_fire_2019]. First-order effects comprise the immediate impacts of heat transfer on plant tissues. Nonlethal first-order heat injuries can trigger second-order effects, such as physiological limitations in carbon and water relations or increased susceptibility to insect attacks and pathogenic infections. 

The growth model currently deals with first-order effects only. The design of first-order fire effects on vegetation follows, with some modifications, the model proposed by @michaletz_biophysical_2008 for surface fires, and it is outlined in fig. \@ref(fig:carbonwaterlinks). The surface fire behavior variables required for severity evaluation are the residence time and fireline intensity. With them, the model estimates the air temperature gradient of the plume, which is then compared to the distribution of foliage for the determination of the proportion of the crown foliage and crown buds that die due to heat accumulation. In addition, the model determines the effects on the cambium by estimating the radial necrosis depth and comparing it with bark thickness. If enough heat reaches the cambium, the tree is considered girdled, which leads to above-ground mortality. Otherwise, the degree of crown bud necrosis will determine whether the plant can resprout from the crown (and rebuild the leaf area lost) or not. 
...
If 100\% of buds have died but the cambium is alive and the species has dormant buds beneath the bark, then it can still resprout epicormically, but other for other species the above-ground structures of the plant will be considered dead. At this point, the cohort may resprout from the root crown only, but is dealt in the forest dynamics model (see chapter \@ref(dynamicsmodelling)).
...
knitr::include_graphics("FireSeverity.drawio.png")
```
