...
The forest growth model allows simulating the variation of leaf area, sapwood area and fine root area via the following processes (see fig. \@ref(fig:allocationdiagram)):
 ...
 iii. **Sapwood growth**: The model assumes that the hormonal signals controlling cambium division have been mainly synthesized in leaves and, hence, formation of new sapwood will only occur if unfolded leaves are present. More specifically, the levels of growth hormones depend linearly on the current amount of foliage relative to the maximum foliage of the same plant expected without competition. This means that: (a) leaf phenological status of deciduous species will affect sapwood growth rates; (b) for a given plant size (i.e. cambium surface) the amount of foliage will be higher or lower depending on the shading created by taller plants; (c) in particular, understory plants or densely-crowded plants will have lower sapwood growth rates than plants that are isolated or overtop the other plants; (d) defoliation events will have an impact on sapwood growth rates, as observed empirically [@javoy_impact_2025]. Sapwood formation corresponds mostly to secondary growth (but primary growth occurs at the same time due to diameter-height relationships), and is only allowed if storage carbon levels are above a given species-specific threshold (e.g. 50\%). These levels are much higher than required for leaf or fine root growth. Thus, when carbon levels are low, sapwood growth rates fall to zero, whereas replacement of leaves and fine roots continues to sustain metabolism. This design was chosen to stop sapwood growth much before emptying storage carbon reserves, so that maintenance respiration can be sustained longer whenever photosynthesis is low, as in shadow-tolerant species. 
...
### Fire occurrence and severity {#fireseveritydesign}

The occurrence of fires is represented in the growth model in a forcing variable that describes the daily probability that a wildfire strikes the stand. Whenever a fire occurs, the model first simulates fire behavior according to the conditions of the stand (see chapter \@ref(fuelandfirebehavior)) and then fire severity is evaluated. 
...
Many state variables are inherited by the corresponding water balance model and depend on its level of complexity (see  \@ref(statevariablesbasic) and \@ref(statevariablesadvanced)). Several *cohort-level state variables* were treated as fixed parameters in the water balance models. For each plant cohort:
 ...
 + The radial length of coarse roots $L_{radial, s}$ in each soil layer $s$.
