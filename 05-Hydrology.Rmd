...
Forest hydrological processes are key for soil and plant water balances. This chapter details the design and implementation of vertical hydrological processes included in package **medfate**. These processes determine the value of the water flows mentioned in eq. \@ref(eq:basicwaterbalanceequation). The only process that is not described here is plant transpiration ($Tr$), which will be covered in detail in chapter \@ref(transpirationgranier).
...
\begin{eqnarray}
In = C_{canopy}\cdot P_G+C_{canopy}\cdot(E/R)\cdot(Pr-P_G) \: {if}\: Pr > P_G \\
