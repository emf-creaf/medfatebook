...
Forest hydrological processes are key for soil and plant water balances. This chapter details the design and implementation of vertical hydrological processes included in the package. These processes determine the value of the water flows mentioned in eq. \@ref(eq:basicwaterbalanceequation). The only process that is not described here is plant transpiration ($Tr$), which will be covered in detail in chapter \@ref(transpirationgranier).

While each process is implemented in specific sub-model functions, function `hydrology_verticalInputs()` allows simulating several soil water input processes altogether, including canopy water interception, snow accumulation/melt, soil infiltration and percolation.
